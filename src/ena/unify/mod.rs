...
use crate::ena::snapshot_vec::{self as sv, UndoLog};
use crate::ena::undo_log::{UndoLogs, VecLog};
...
/// that implements the `UnifyKey` trait. Unification tables have one mode - upstream's
/// `persistent` mode went with the `dogged` dependency when this copy was vendored:
///
