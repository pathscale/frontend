...
mod prune {
    ...
    mod should_simplify {
        ...
        #[test]
        fn seq_1() {
            let layout: Tree<Def, crate::Never, crate::Never> = Tree::def(Def::NoSafetyInvariants).then(Tree::byte(0x00));
            assert_eq!(layout.prune(&|d| matches!(d, Def::HasSafetyInvariants)), Tree::byte(0x00));
        }

        #[test]
        fn seq_2() {
            let layout: Tree<Def, crate::Never, crate::Never> =
    ...
    mod should_reject {
        ...
        #[test]
        fn invisible_def() {
            let layout: Tree<Def, crate::Never, crate::Never> = Tree::def(Def::HasSafetyInvariants);
            assert_eq!(
                layout.prune(&|d| matches!(d, Def::HasSafetyInvariants)),
                Tree::uninhabited()
            );
        ...
        fn invisible_def_in_seq_len_3() {
            let layout: Tree<Def, crate::Never, crate::Never> = Tree::def(Def::NoSafetyInvariants)
                .then(Tree::byte(0x00))
