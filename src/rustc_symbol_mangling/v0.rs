...
pub(crate) fn push_ident(ident: &str, output: &mut String) {
    ...
    let ident = if use_punycode {
        ...
        // `punycode_no_std` is our own RFC 3492 encoder. It replaced the `punycode` crate,
        // which was `std`, and it settles the FIXME that stood here asking for exactly
        // that. The identifier cannot contain a `-` (the byte scan above rejects one), so
        // the only `-` the encoder can emit is the delimiter, which is what the `rfind`
        // below relies on.
        let mut punycode_bytes = match crate::punycode_no_std::encode(ident) {
            Ok(s) => s.into_bytes(),
