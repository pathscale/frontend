    ...
    fn abi_stable_hash(&self, tcx: TyCtxt<'tcx>, hasher: &mut StableHasher) {
        match self.kind() {
            ...
            ty::Adt(adt_def, args) => {
                ...
                // `as_slice()` rather than relying on the deref to `[T]`: the impl above is for
                // `[T]`, and autoderef cannot reach it through `RawList`, which ends in an extern
                // type. See the note beside the inherent slice methods in `ty/list.rs`.
                args.as_slice().abi_stable_hash(tcx, hasher);
            }
