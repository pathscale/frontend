...
// `#![no_std]`: `Vec` arrives from the prelude under `std` and not under `core`.
use alloc::vec::Vec;
...
pub(crate) fn antijoin<'me, Key: Ord, Val: Ord, Result: Ord>(
    ...
    let results = input1
        .recent()
        .iter()
        // `ref` removed from both patterns. This crate is edition 2018 upstream and is compiled
        // at 2024 here, where an explicit `ref` inside a pattern that is already in a
        // by-reference default binding mode is an error rather than a redundancy. The bindings
        // were already references; nothing about them changes.
        .filter(|(key, _)| {
            tuples2 = gallop(tuples2, |k| k < key);
            tuples2.first() != Some(key)
        })
        .map(|(key, val)| logic(key, val))
        .collect::<Vec<_>>();
