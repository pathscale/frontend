// Vendored from datafrog 2.0.1, converted to `no_std`. See `vendor/datafrog/VENDORED.md`.
//
// The conversion is a rename: every `std::` path in this crate was `core`- or `alloc`-reachable.
// `Rc` moved to `alloc::rc`, everything else to `core::`, and each module names `Vec` (and in
// `lib.rs`, `Box` and `String`) explicitly because they arrive from the prelude under `std` and
// not under `core`. No algorithm, no data layout, and no public API is touched.
...
// `#![no_std]`: prelude names, invisible to a `std::` search.
use alloc::boxed::Box;
use alloc::rc::Rc;
...
mod map;
mod treefrog;
pub use crate::datafrog::join::JoinInput;
pub use crate::datafrog::treefrog::{
...
impl<Tuple: Ord> Relation<Tuple> {
    ...
    pub fn merge(self, other: Self) -> Self {
        ...
        if elements1[0] > elements2[0] {
            core::mem::swap(&mut elements1, &mut elements2);
        }
    ...
    ///
    /// Same as the `from_iter` method from `core::iter::FromIterator` trait.
...
impl<Tuple: Ord> core::ops::Deref for Relation<Tuple> {
    type Target = [Tuple];
