...
use crate::rustc_errors::{Applicability, Diag, ErrorGuaranteed, StashKey, msg};
...
use crate::rustc_middle::ty::{self, FnSig, GenericArgsRef, Ty, TyCtxt, TypeVisitableExt, Unnormalized};
...
use crate::rustc_hir_typeck::method::TreatNotYetDefinedOpaques;
use crate::rustc_hir_typeck::method::confirm::ConfirmContext;
use crate::rustc_hir_typeck::method::probe::{IsSuggestion, Mode};
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    pub(crate) fn check_call_abi(&self, abi: ExternAbi, span: Span) {
        ...
        match canon_abi {
            ...
            | CanonAbi::Interrupt(_) => {
                let err = crate::rustc_hir_typeck::diagnostics::AbiCannotBeCalled { span, abi };
                self.tcx.dcx().emit_err(err);
            }

            // This is an entry point for the host, and cannot be called directly.
            CanonAbi::GpuKernel => {
                let err = crate::rustc_hir_typeck::diagnostics::GpuKernelAbiCannotBeCalled { span };
                self.tcx.dcx().emit_err(err);
            }
    ...
    fn report_invalid_callee(
            ...
            && call_expr.span.contains(callee_expr.span)
        {
            ...
            unit_variant =
                Some((removal_span, descr, crate::rustc_hir_pretty::qpath_to_string(self, qpath)));
        }
