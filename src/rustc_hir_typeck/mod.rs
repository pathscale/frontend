...
// One real std use remains: `PathBuf` in `method/suggest.rs`, which names the file a `--emit=metadata` suggestion would point at. Real std, and it stays spelled `std::` so
// the ratchet keeps counting that file.
...
use crate::rustc_errors::{Applicability, Diag, ErrorGuaranteed, struct_span_code_err};
...
use crate::rustc_infer::traits::{ObligationCauseCode, ObligationInspector, TraitEngine, WellFormedLoc};
use crate::rustc_middle::middle::codegen_fn_attrs::CodegenFnAttrFlags;
use crate::rustc_middle::query::Providers;
use crate::rustc_middle::ty::{self, FnSigKind, Ty, TyCtxt, Unnormalized};
...
use crate::rustc_hir_typeck::check::check_fn;
use crate::rustc_hir_typeck::coercion::CoerceMany;
use crate::rustc_hir_typeck::diverges::Diverges;
use crate::rustc_hir_typeck::expectation::Expectation;
use crate::rustc_hir_typeck::fn_ctxt::LoweredTy;
use crate::rustc_hir_typeck::gather_locals::GatherLocalsVisitor;

#[macro_export]
macro_rules! type_error_struct {
    ($dcx:expr, $span:expr, $typ:expr, $code:expr, $($message:tt)*) => ({
        let mut err = crate::struct_span_code_err!($dcx, $span, $code, $($message)*);
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    fn report_unexpected_variant_res(
        ...
        let path_str = crate::rustc_hir_pretty::qpath_to_string(self, qpath);
        let mut err = tcx
            .dcx()
...
pub use crate::type_error_struct;
