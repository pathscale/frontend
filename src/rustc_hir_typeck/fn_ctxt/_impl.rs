...
use crate::rustc_lint_defs::builtin::{SELF_CONSTRUCTOR_FROM_OUTER_ITEM, UNREACHABLE_CODE};
...
use crate::rustc_hir_typeck::callee::{self, DeferredCallResolution, SplatLoweringInfo};
use crate::rustc_hir_typeck::diagnostics::{self, CtorIsPrivate};
use crate::rustc_hir_typeck::method::{self, MethodCallee};
use crate::rustc_hir_typeck::{BreakableCtxt, Diverges, Expectation, FnCtxt, LoweredTy};
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    pub(crate) fn transform_args_for_inherent_type_const(
        ...
        tcx.mk_args_from_iter(
            core::iter::once(ty::GenericArg::from(self_ty)).chain(own_args.iter().copied()),
        )
    }
    ...
    pub(crate) fn instantiate_value_path(
        ...
        let (res, implicit_args) = if let Res::Def(DefKind::ConstParam, def) = res {
            ...
            if core::iter::successors(Some(self.body_def_id.to_def_id()), |&def_id| {
