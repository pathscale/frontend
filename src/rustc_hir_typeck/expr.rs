...
use crate::rustc_hir::{ExprKind, HirId, QPath, find_attr, is_range_literal};
use crate::rustc_hir_analysis::diagnostics::{NoFieldOnType, NoVariantNamed};
use crate::rustc_hir_analysis::hir_ty_lowering::HirTyLowerer as _;
...
use crate::rustc_middle::ty::{self, AdtKind, GenericArgsRef, Ty, TypeVisitableExt, Unnormalized};
...
use crate::rustc_hir_typeck::op::contains_let_in_chain;
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    fn check_expr_struct_fields(
        ...
        match *base_expr {
            ...
            crate::rustc_hir::StructTailExpr::NoneWithError(guaranteed) => {
            ...
            crate::rustc_hir::StructTailExpr::None => {
    ...
    fn report_private_fields(
        ...
        if let ty::Adt(def, _) = adt_ty.kind() {
            ...
            let suggestion = |name, args| {
                format!(
                    "::{name}({})",
                    core::iter::repeat_n("_", args).collect::<Vec<_>>().join(", ")
                )
            };
            match &items[..] {
                [] => {}
