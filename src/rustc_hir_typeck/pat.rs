...
use crate::rustc_lint_defs::builtin::NON_EXHAUSTIVE_OMITTED_PATTERNS;
use crate::rustc_middle::traits::PatternOriginExpr;
use crate::rustc_middle::ty::{self, Pinnedness, Ty, TypeVisitableExt, Unnormalized};
...
use crate::rustc_hir_typeck::gather_locals::DeclOrigin;
use crate::rustc_hir_typeck::{FnCtxt, diagnostics};
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    fn check_pin_projection(
            ...
            && !adt.is_pin()
        {
            ...
            self.dcx().emit_err(crate::rustc_hir_typeck::diagnostics::ProjectOnNonPinProjectType {
                span: pat.span,
                def_span,
                sugg_span,
            });
    ...
    fn error_tuple_variant_index_shorthand(
            ...
            (variant.ctor_kind(), &pat.kind)
        {
            ...
            if has_shorthand_field_name {
                let path = crate::rustc_hir_pretty::qpath_to_string(self, qpath);
                let mut err = struct_span_code_err!(
                    self.dcx(),
                    pat.span,
                    E0769,
        ...
        fields
            ...
            .map(|field| {
                match self.tcx.sess.source_map().span_to_snippet(field.pat.span) {
                    ...
                    Err(_) => crate::rustc_hir_pretty::pat_to_string(self, field.pat),
                }
            })
            .collect::<Vec<String>>()
            .join(", ")
        ...
        impl<'a, 'b, 'c, 'tcx> Diagnostic<'a, ()> for FieldsNotListed<'b, 'c, 'tcx> {
            fn into_diag(self, dcx: DiagCtxtHandle<'a>, level: Level) -> Diag<'a, ()> {
                ...
                Diag::new(dcx, level, "some fields are not explicitly listed")
                    .with_span_label(pat_span, format!("field{} {} not listed", crate::pluralize!(unmentioned_fields.len()), joined_patterns))
                    .with_help(
