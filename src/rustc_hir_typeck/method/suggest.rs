...
use crate::rustc_hir_typeck::diagnostics::{self, CandidateTraitNote, NoAssociatedItem};
use crate::rustc_hir_typeck::expr_use_visitor::expr_place;
use crate::rustc_hir_typeck::method::probe::UnsatisfiedPredicates;
use crate::rustc_hir_typeck::{Expectation, FnCtxt};
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    fn create_missing_writer_err(
        ...
        mut long_ty_path: Option<String>,
    ) -> Diag<'_> {
        let mut err = struct_span_code_err!(
            self.dcx(),
            rcvr_expr.span,
    ...
    fn handle_unsatisfied_predicates(
        ...
        for (p, parent_p, cause) in unsatisfied_predicates {
            ...
            match self.tcx.hir_get_if_local(item_def_id) {
                ...
                Some(Node::Item(hir::Item {
                    kind: hir::ItemKind::Trait { is_auto: crate::rustc_ast::ast::IsAuto::Yes, .. },
                    span: item_span,
                    ..
                })) => {
                ...
                if let Node::Expr(parent_expr) = parent {
                    ...
                    let lang_item = match parent_expr.kind {
                        ...
                        ExprKind::Call(func, _) => match func.kind {
                            // `..=` desugars into `::core::ops::RangeInclusive::new(...)`.
                            ExprKind::Path(qpath)
    ...
    fn suggest_bounds_for_range_to_method(
        ...
        let Some(end_field) = fields.iter().find(|f| f.ident.name == crate::rustc_span::sym::end) else {
            return;
        };
        ...
        let end_is_negative = is_integral
            && matches!(end_field.expr.kind, hir::ExprKind::Unary(crate::rustc_ast::UnOp::Neg, _));
        ...
        let insert_span = rcvr_expr
            .span
            .with_lo(rcvr_expr.span.lo() + crate::rustc_span::BytePos(offset as u32))
            .shrink_to_lo();
    ...
    fn set_label_for_method_error(
        ...
        if tcx.ty_is_opaque_future(rcvr_ty) && item_ident.name == sym::poll {
            ...
            err.help(format!(
                ...
                see documentation for `core::pin::Pin`"
            ));
            err.help(
                    ...
                    && inputs_len.is_some_and(|inputs_len| pick.item.is_fn() && self.tcx.fn_sig(pick.item.def_id).skip_binder().skip_binder().inputs().len() == inputs_len)
                {
                    ...
                    match self.tcx.parent_hir_node(expr.hir_id) {
                            ...
                            err.multipart_suggestion(
                                ...
                                vec![
                                    (
                                        ...
                                        format!(
                                            "let mut pinned = core::pin::pin!({code});\n{indent}"
                                        ),
                                    ),
                                    (
                            ...
                            err.multipart_suggestion(
                                ...
                                vec![
                                    (
                                        ...
                                        "let mut pinned = core::pin::pin!(".to_string(),
                                    ),
                                    (
                        ...
                        _ => {
                            ...
                            err.span_help(
                                rcvr.span,
                                "consider pinning the expression with `core::pin::pin!()` and \
