...
impl<'tcx> crate::rustc_mir_transform::MirPass<'tcx> for ReorderBasicBlocks {
...
impl<'tcx> crate::rustc_mir_transform::MirPass<'tcx> for ReorderLocals {
    fn policy(&self, _session: &Session) -> PassPolicy {
        PassPolicy::optional_non_optimization(false)
    }
...
fn permute<I: crate::rustc_index::Idx + Ord, T>(data: &mut IndexVec<I, T>, map: &IndexSlice<I, I>) {
    ...
    let mut enumerated: Vec<_> = core::mem::take(data).into_iter_enumerated().collect();
