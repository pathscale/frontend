...
impl<'tcx> crate::rustc_mir_transform::MirPass<'tcx> for SimplifyConstCondition {
    ...
    fn run_pass(&self, tcx: TyCtxt<'tcx>, body: &mut Body<'tcx>) {
        ...
        'blocks: for (bb, block) in body.basic_blocks.iter_enumerated() {
            ...
            for (statement_index, stmt) in block.statements.iter().enumerate() {
                ...
                } else if let StatementKind::Assign(ref assign) = stmt.kind
                    && let (lhs, Rvalue::Use(Operand::Constant(c), _)) = &**assign
                {
                    let lhs = *lhs;
