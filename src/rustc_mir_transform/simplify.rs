...
impl<'tcx> crate::rustc_mir_transform::MirPass<'tcx> for SimplifyCfg {
    fn name(&self) -> &'static str {
        self.name()
    }
...
impl<'tcx> crate::rustc_mir_transform::MirPass<'tcx> for SimplifyLocals {
    fn name(&self) -> &'static str {
        match &self {
    ...
    fn visit_statement(&mut self, statement: &Statement<'tcx>, location: Location) {
        match statement.kind {
            ...
            StatementKind::Assign(ref assign) => {
                let (ref place, ref rvalue) = **assign;
        ...
        for data in body.basic_blocks.as_mut_preserves_cfg() {
            ...
            for statement in data.statements.iter_mut() {
                let keep_statement = match &statement.kind {
                    ...
                    StatementKind::Assign(assign) => used_locals.is_used(assign.0.local),
