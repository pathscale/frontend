...
// One real std use remains, incidental rather than computation, and it is spelled `std::` so
// the ratchet keeps counting the file that holds it: `thread_local!` memoises the
// self-profiler's snake-case pass names in `pass_manager.rs`. `eko::thread::
// ThreadLocal` is not a drop-in for it - it is a type with a `with(init, f)` method, not a
// macro producing a value with `with`/`take` - so that one waits on a shape that fits.
// The pass-name table below was the other, and now uses `eko::thread::LazyLock`;
// it goes entirely when the table is built once up front.
...
use crate::rustc_const_eval::check_consts::{self, ConstCx};
use crate::rustc_const_eval::util;
