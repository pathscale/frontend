...
use crate::rustc_lint_defs::builtin::AMBIGUOUS_DERIVE_HELPERS;
...
fn parse_derive_like(
    ...
    if let Some(attrs) = items.next() {
        ...
        for attr in attr_list.mixed() {
            ...
            if crate::rustc_feature::is_builtin_attr_name(ident.name) {
                cx.emit_lint(
                    AMBIGUOUS_DERIVE_HELPERS,
                    crate::rustc_attr_parsing::diagnostics::AmbiguousDeriveHelpers,
                    ident.span,
                );
            }
