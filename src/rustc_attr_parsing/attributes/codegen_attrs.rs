...
use crate::rustc_span::edition::Edition::Edition2024;
use crate::rustc_structures::SanitizerSet;
...
use crate::rustc_attr_parsing::target_checking::Policy::AllowSilent;
...
impl SingleAttributeParser for RustcObjcClassParser {
    const PATH: &[crate::rustc_span::Symbol] = &[sym::rustc_objc_class];
...
impl SingleAttributeParser for RustcObjcSelectorParser {
    const PATH: &[crate::rustc_span::Symbol] = &[sym::rustc_objc_selector];
    const ALLOWED_TARGETS: AllowedTargets<'_> =
        AllowedTargets::AllowList(&[Allow(Target::ForeignStatic)]);
...
impl AttributeParser for NakedParser {
    ...
    fn finalize(self, cx: &FinalizeContext<'_, '_>) -> Option<AttributeKind> {
        ...
        const ALLOW_LIST: &[crate::rustc_span::Symbol] = &[
            // testing (allowed here so better errors can be generated in `crate::rustc_builtin_macros::test`)
            sym::test,
            sym::ignore,
            sym::should_panic,
            sym::bench,
...
impl SingleAttributeParser for PatchableFunctionEntryParser {
    ...
    fn convert(cx: &mut AcceptContext<'_, '_>, args: &ArgParser) -> Option<AttributeKind> {
        ...
        for item in meta_item_list.mixed() {
            ...
            let crate::rustc_ast::LitKind::Int(val, _) = value.value_as_lit().kind else {
