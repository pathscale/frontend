...
use crate::rustc_ast_pretty::pp::Breaks::{Consistent, Inconsistent};
use crate::rustc_ast_pretty::pp::{self, BoxMarker, Breaks};
use crate::rustc_ast_pretty::pprust::state::fixup::FixupContext;
...
fn gather_comments(sm: &SourceMap, path: FileName, src: String) -> Vec<Comment> {
    ...
    if let Some(shebang_len) = crate::rustc_lexer::strip_shebang(text) {
    ...
    for token in crate::rustc_lexer::tokenize(&text[pos..], crate::rustc_lexer::FrontmatterAllowed::Yes) {
        ...
        match token.kind {
            ...
            crate::rustc_lexer::TokenKind::BlockComment { doc_style, .. } => {
            ...
            crate::rustc_lexer::TokenKind::Frontmatter { .. } => {
                code_to_the_left = false;
                comments.push(Comment {
                    style: CommentStyle::Isolated,
                    lines: vec![token_text.to_string()],
...
pub trait PrintState<'a>: core::ops::Deref<Target = pp::Printer> + core::ops::DerefMut {
    ...
    fn maybe_print_shebang(&mut self) {
        if let Some(cmnt) = self.peek_comment() {
            ...
            // See [crate::rustc_lexer::strip_shebang] and [gather_comments] from pprust/state.rs for details
...
impl<'a> State<'a> {
    ...
    pub fn print_ty_pat(&mut self, pat: &ast::TyPat) {
        match &pat.kind {
            ...
            crate::rustc_ast::TyPatKind::NotNull => self.word("!null"),
