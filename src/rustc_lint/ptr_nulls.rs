...
use crate::rustc_hir::{BinOpKind, Expr, ExprKind, TyKind, find_attr};
use crate::rustc_lint_defs::{declare_lint, declare_lint_pass};
use crate::rustc_middle::ty::RawPtr;
use crate::rustc_span::{Span, sym};

use crate::rustc_lint::diagnostics::{InvalidNullArgumentsDiag, UselessPtrNullChecksDiag};
use crate::rustc_lint::utils::peel_casts;
        ...
        match expr.kind {
            ...
            ExprKind::Binary(op, left, right) if matches!(op.node, BinOpKind::Eq) => {
                ...
                match to_check.kind {
                    ...
                    // Catching:
                    // (fn_ptr as *<const/mut> <ty>) == core::ptr::null()
                    ExprKind::Call(path, [])
                        if let ExprKind::Path(ref qpath) = path.kind
                            && let Some(def_id) = cx.qpath_res(qpath, path.hir_id).opt_def_id()
                            && let Some(diag_item) = cx.tcx.get_diagnostic_name(def_id)
