...
use crate::rustc_lint::diagnostics::{DropGlue, DropTraitConstraintsDiag};
use crate::rustc_lint::{LateContext, LateLintPass, LintContext};

declare_lint! {
    /// The `drop_bounds` lint checks for generics with `core::ops::Drop` as
    /// bounds.
    ///
    /// ### Example
    ///
...
declare_lint! {
    /// The `dyn_drop` lint checks for trait objects with `core::ops::Drop`.
    ///
    /// ### Example
    ///
    /// ```rust
...
declare_lint_pass!(
    ...
    /// indicate an attempt to emulate `core::mem::needs_drop`.
