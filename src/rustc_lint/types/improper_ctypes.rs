...
use crate::rustc_errors::{DiagMessage, msg};
...
use crate::rustc_lint::diagnostics::{ImproperCTypes, UsesPowerAlignment};
    ...
    fn visit_indirection(
        ...
        match indirection_kind {
            ...
            IndirectionKind::Ref | IndirectionKind::RawPtr => {
                ...
                // opaqueness with an extern type (opaque caller-side) or a alloc::ffi::c_void
                    ...
                    AdtKind::Struct | AdtKind::Union => {
                            ...
                            && !self.base_ty.is_mutable_ptr()
                        {
                            return FfiUnsafe {
                                ...
                                help: Some(msg!(
                                    "consider passing a `*const alloc::ffi::c_char` instead, and use `CStr::as_ptr()`"
                                )),
                            };
                        }
                        self.visit_struct_or_union(state, ty, def, args)
