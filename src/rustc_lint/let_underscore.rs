...
use crate::rustc_lint::diagnostics::{NonBindingLet, NonBindingLetSub};
...
declare_lint! {
    ...
    /// calling `core::mem::drop` on the expression is clearer and helps convey
    /// intent.
    ...
    fn check_local(&mut self, cx: &LateContext<'_>, local: &hir::LetStmt<'_>) {
        ...
        local.pat.walk_always(|pat| {
            ...
            let sub = NonBindingLetSub {
                ...
                is_assign_desugar: matches!(local.source, crate::rustc_hir::LocalSource::AssignDesugar),
            };
            if is_sync_lock {
