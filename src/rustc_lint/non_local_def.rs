...
use crate::rustc_errors::{MultiSpan, msg};
use crate::rustc_hir::def::{DefKind, Res};
use crate::rustc_hir::intravisit::{self, Visitor, VisitorExt};
use crate::rustc_hir::{Body, HirId, Item, ItemKind, Node, Path, TyKind, find_attr};
use crate::rustc_lint_defs::{declare_lint, impl_lint_pass};
use crate::rustc_middle::ty::TyCtxt;
use crate::rustc_span::def_id::{DefId, LOCAL_CRATE};
use crate::rustc_span::{ExpnKind, Span, kw};

use crate::rustc_lint::diagnostics::{NonLocalDefinitionsCargoUpdateNote, NonLocalDefinitionsDiag};
...
///
/// ```text
///    core::convert::PartialEq<Foo<Bar>>
///    ^^^^^^^^^^^^^^^^^^^^^^^
/// ```
#[inline]
