...
declare_lint! {
    /// The `dropping_references` lint checks for calls to `core::mem::drop`
...
declare_lint! {
    /// The `forgetting_references` lint checks for calls to `core::mem::forget` with a reference
    ...
    /// ```rust
    /// let x = Box::new(1);
    /// core::mem::forget(&x); // Should have been forget(x), x will still be dropped
    /// ```
    ///
    /// {{produces}}
    ///
    ...
    Warn,
    "calls to `core::mem::forget` with a reference instead of an owned value"
}

declare_lint! {
    /// The `dropping_copy_types` lint checks for calls to `core::mem::drop`
    /// and `core::ptr::drop_in_place` where the dropped value implements the `Copy` trait.
    ///
    /// ### Example
    ///
    /// ```rust
    /// let x: i32 = 42; // i32 implements Copy
    /// core::mem::drop(x); // A copy of x is passed to the function, leaving the
    ///                    // original unaffected
    /// ```
    ///
    /// {{produces}}
    ...
    /// ### Explanation
    ///
    /// Calling `core::mem::drop` [does nothing for types that
...
declare_lint! {
    /// The `forgetting_copy_types` lint checks for calls to `core::mem::forget` with a value
    ...
    /// ```rust
    /// let x: i32 = 42; // i32 implements Copy
    /// core::mem::forget(x); // A copy of x is passed to the function, leaving the
    ///                      // original unaffected
    /// ```
    ///
    /// {{produces}}
    ...
    /// ### Explanation
    ///
    /// Calling `core::mem::forget` [does nothing for types that
    /// implement Copy](https://doc.rust-lang.org/std/mem/fn.drop.html) since the
    /// value will be copied and moved into the function on invocation.
    ///
    /// An alternative, but also valid, explanation is that Copy types do not
    /// implement the Drop trait, which means they have no destructors. Without a
    /// destructor, there is nothing for `core::mem::forget` to ignore.
    pub FORGETTING_COPY_TYPES,
    Warn,
    "calls to `core::mem::forget` with a value that implements Copy"
}

declare_lint! {
    /// The `undropped_manually_drops` lint check for calls to `core::mem::drop`
    /// and `core::ptr::drop_in_place` where the dropped value is `core::mem::ManuallyDrop`
    /// which doesn't drop.
    ///
    /// ### Example
    ///
    /// ```rust,compile_fail
    /// struct S;
    /// drop(core::mem::ManuallyDrop::new(S));
    /// ```
    ///
    /// {{produces}}
    ///
    /// ### Explanation
    ///
    /// `ManuallyDrop` does not drop it's inner value so calling `core::mem::drop` will
    /// not drop the inner value of the `ManuallyDrop` either.
    pub UNDROPPED_MANUALLY_DROPS,
    Deny,
    "calls to `drop` and `drop_in_place` where the dropped value is `core::mem::ManuallyDrop`"
}
