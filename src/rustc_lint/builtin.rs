...
//! AST visitor. Also see `crate::rustc_lint_defs::builtin`, which contains the
...
use crate::rustc_ast_pretty::pprust::expr_to_string;
use crate::rustc_attr_parsing::AttributeParser;
use crate::rustc_errors::{Applicability, Diagnostic, msg};
use crate::rustc_feature::GateIssue;
use crate::rustc_hir::attrs::lang_items::LangItem;
use crate::rustc_hir::attrs::{AttributeKind, DocAttribute};
...
use crate::rustc_hir::{self as hir, Body, FnDecl, ImplItemImplKind, PatKind, PredicateOrigin, find_attr};
// Lints from rustc_lint_defs
pub use crate::rustc_lint_defs::builtin::*;
use crate::rustc_lint_defs::{declare_lint, declare_lint_pass, fcw, impl_lint_pass};
use crate::bug;
...
use crate::rustc_span::{DUMMY_SP, Ident, InnerSpan, Span, Spanned, Symbol, kw, sym};
...
impl InvalidNoMangleItems {
    fn check_no_mangle_on_generic_fn(
        ...
        if generics.requires_monomorphization(cx.tcx) {
            cx.tcx.dcx().emit_err(crate::rustc_lint::diagnostics::BuiltinNoMangleGeneric {
...
declare_lint! {
    ...
    /// ```rust,compile_fail
    /// unsafe {
    ///     let y = core::mem::transmute::<&i32, &mut i32>(&5);
    /// }
    /// ```
    ///
    /// {{produces}}
...
pub mod soft {
    ...
    pub fn lint_vec() -> crate::rustc_lint::LintVec {
        vec![
            WHILE_TRUE,
            NON_SHORTHAND_FIELD_PATTERNS,
            UNSAFE_CODE,
...
declare_lint! {
    ...
    /// # #![allow(unused)]
    /// unsafe {
    ///     let x: &'static i32 = core::mem::zeroed();
    /// }
    /// ```
    ///
    /// {{produces}}
    ...
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &hir::Expr<'_>) {
        ...
        fn is_null_ptr(cx: &LateContext<'_>, expr: &hir::Expr<'_>) -> bool {
            ...
            if let Ok(layout) = cx.tcx.layout_of(cx.typing_env().as_query_input(*pointee)) {
                if layout.layout.size() == crate::rustc_abi::Size::ZERO {
                    return false;
                }
            }
...
impl<'tcx> LateLintPass<'tcx> for InternalEqTraitMethodImpls {
    fn check_impl_item(&mut self, cx: &LateContext<'tcx>, item: &'tcx crate::rustc_hir::ImplItem<'tcx>) {
