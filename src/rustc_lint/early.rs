...
use crate::rustc_ast::{self as ast, AttrVec, HasAttrs};
use crate::rustc_errors::{BufferedEarlyLint, LintBuffer};
use crate::rustc_feature::Features;
use crate::rustc_lint_defs::LintPass;
use crate::rustc_middle::ty::RegisteredTools;
use crate::rustc_session::Session;
use crate::rustc_span::{Ident, Span};
use tracing::debug;

use crate::rustc_lint::DiagAndSess;
use crate::rustc_lint::context::{EarlyContext, EarlyLintPassFactory, LintContext, LintStore};
use crate::rustc_lint::passes::{EarlyLintPass, EarlyLintPassObject};
...
impl LintPass for RuntimeCombinedEarlyLintPass {
    ...
    fn get_lints(&self) -> crate::rustc_lint::LintVec {
        panic!()
    }
}
...
pub fn check_ast_node<'a>(
    ...
    let context = if pre_expansion {
        let builtin_lints = crate::rustc_lint::BuiltinCombinedPreExpansionLintPass::new();
        let passes = &lint_store.pre_expansion_lint_passes;
        run_passes(check_node, context, builtin_lints, passes)
    } else {
        let builtin_lints = crate::rustc_lint::BuiltinCombinedEarlyLintPass::new();
