...
use crate::rustc_ast::{Pat, PatKind, Path};
use crate::rustc_hir as hir;
use crate::rustc_hir::def::Res;
use crate::rustc_hir::def_id::DefId;
use crate::rustc_hir::{Expr, ExprKind, HirId, find_attr};
use crate::rustc_lint_defs::{declare_lint_pass, declare_tool_lint};
use crate::rustc_middle::ty::{self, ClausePolarity, GenericArgsRef};
...
use crate::rustc_lint::{EarlyContext, EarlyLintPass, LateContext, LateLintPass, LintContext};

declare_tool_lint! {
    /// The `default_hash_type` lint detects use of [`alloc::collections::HashMap`] and
    /// [`alloc::collections::HashSet`], suggesting the use of `FxHashMap`/`FxHashSet`.
    ///
...
declare_tool_lint! {
    ...
    /// non-glob imports of module `crate::rustc_type_ir::inherent`.
    pub rustc::NON_GLOB_IMPORT_OF_TYPE_IR_INHERENT,
    Allow,
    "non-glob import of `crate::rustc_type_ir::inherent`",
    report_in_external_macro: true
}

declare_tool_lint! {
    /// The `usage_of_type_ir_inherent` lint detects usage of `crate::rustc_type_ir::inherent`.
    ///
    /// This module should only be used within the trait solver.
    pub rustc::USAGE_OF_TYPE_IR_INHERENT,
    Allow,
    "usage `crate::rustc_type_ir::inherent` outside of trait system",
    report_in_external_macro: true
}

declare_tool_lint! {
    /// The `usage_of_type_ir_traits` lint detects usage of `crate::rustc_type_ir::Interner`,
...
impl<'tcx> LateLintPass<'tcx> for TypeIr {
    ...
    fn check_item(&mut self, cx: &LateContext<'tcx>, item: &'tcx hir::Item<'tcx>) {
        ...
        // Final path resolutions, like `use crate::rustc_type_ir::inherent`
...
impl EarlyLintPass for BadUseOfFindAttr {
    fn check_arm(&mut self, cx: &EarlyContext<'_>, arm: &crate::rustc_ast::Arm) {
...
impl<'tcx> LateLintPass<'tcx> for RustcMustMatchExhaustively {
    ...
    fn check_stmt(&mut self, cx: &LateContext<'tcx>, stmt: &'tcx crate::rustc_hir::Stmt<'tcx>) {
        match stmt.kind {
            crate::rustc_hir::StmtKind::Let(let_stmt) => {
