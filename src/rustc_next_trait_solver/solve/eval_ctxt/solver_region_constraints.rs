...
use crate::rustc_type_ir::ClauseKind::*;
use crate::rustc_type_ir::inherent::*;
use crate::rustc_type_ir::outlives::{Component, push_outlives_components};
#[cfg(not(feature = "nightly"))]
use crate::rustc_type_ir::region_constraint::TransitiveRelationBuilder;
use crate::rustc_type_ir::region_constraint::{
...
use crate::rustc_next_trait_solver::solve::{Certainty, EvalCtxt, Goal, NoSolution};
    ...
    I: Interner,
{
    ...
    pub(super) fn region_assumptions_for_placeholders_in_universe(
        ...
        let clauses = crate::rustc_type_ir::elaborate::elaborate(
            self.cx(),
    ...
    #[instrument(level = "debug", skip(self), ret)]
    pub(in crate::rustc_next_trait_solver::solve) fn destructure_type_outlives(
        &mut self,
        ty: I::Ty,
        r: Region<I>,
    ) -> RegionConstraint<I> {
    ...
    fn destructure_alias_outlives(
        ...
        let item_bounds =
            crate::rustc_type_ir::outlives::declared_bounds_from_definition(self.cx(), alias)
        ...
        let mut components = Default::default();
        crate::rustc_type_ir::outlives::compute_alias_components_recursive(
            self.cx(),
            alias,
            &mut components,
        );
