...
use crate::rustc_span::{BytePos, DUMMY_SP, Ident, Span, SpanSnippetError, Spanned, Symbol, kw, sym};
...
use crate::rustc_parse::parser::{FnContext, IsDotDotDot};
...
impl<'a> Parser<'a> {
    ...
    /// ```text
    /// impl<S: Into<alloc::borrow::Cow<'static, str>> From<S> for Canonical {}
    ///                                          // ^ missing > here
    /// ```
    ...
    pub(super) fn parameter_without_type(
        ...
        require_name: bool,
        first_param: bool,
        fn_parse_mode: &crate::rustc_parse::parser::FnParseMode,
    ) -> Option<Ident> {
                ...
                || self.token == token::CloseParen)
        {
            let maybe_emit_anon_params_note = |this: &mut Self, err: &mut Diag<'_>| {
                ...
                if matches!(fn_parse_mode.context, crate::rustc_parse::parser::FnContext::Trait)
