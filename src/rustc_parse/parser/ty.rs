...
use crate::rustc_errors::{Applicability, Diag, E0516, PResult};
...
use crate::rustc_parse::parser::{FnContext, FnParseMode, FrontMatterParsingMode};
use crate::rustc_parse::{exp, maybe_recover_from_interpolated_ty_qpath};
...
fn can_begin_dyn_bound_in_edition_2015(t: Token) -> bool {
    if t.is_path_start() {
        ...
        // well-formed (consider: `use core::ops::Fn as dyn;`). Instead, we treat it as a trait
...
impl<'a> Parser<'a> {
    ...
    fn parse_ty_fn_ptr(
        ...
        let mode = crate::rustc_parse::parser::FnParseMode {
            req_name: |_, _| false,
    ...
    fn recover_fn_ptr_with_generics(
        ...
        let sugg = if !lifetimes.is_empty() {
            let snippet =
                lifetimes.iter().map(|param| param.ident.as_str()).separated_by(", ").collect();
    ...
    fn recover_fn_trait_with_lifetime_params(
        ...
        *fn_path_segment = ast::PathSegment {
        ...
        let snippet = format!(
            "for<{}> ",
            lifetimes.iter().map(|lt| lt.ident.as_str()).separated_by(", ").collect::<String>(),
        );
