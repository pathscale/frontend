...
use crate::rustc_parse::{new_parser_from_source_str, source_str_to_stream, unwrap_or_emit_fatal};
...
fn create_test_handler(theme: OutputTheme) -> (DiagCtxt, Arc<SourceMap>, Arc<Mutex<String>>) {
    let output = Arc::new(Mutex::new(String::new()));
    let source_map = Arc::new(SourceMap::new(FilePathMapping::empty()));
    // `PlainEmitter` writes plain lines, so the capture is a `fmt::Write` sink rather than the
...
pub(crate) fn matches_codepattern(a: &str, b: &str) -> bool {
    ...
    loop {
        let (a, b) = match (a_iter.peek(), b_iter.peek()) {
            ...
            (None, _) => return false,
            (Some(&a), None) => {
                if crate::rustc_lexer::is_whitespace(a) {
        ...
        if crate::rustc_lexer::is_whitespace(a) && crate::rustc_lexer::is_whitespace(b) {
            // Skip whitespace for `a` and `b`.
            scan_for_non_ws_or_end(&mut a_iter);
            scan_for_non_ws_or_end(&mut b_iter);
        } else if crate::rustc_lexer::is_whitespace(a) {
    ...
    a_iter.all(crate::rustc_lexer::is_whitespace)
}

/// Advances the given peekable `Iterator` until it reaches a non-whitespace character.
fn scan_for_non_ws_or_end<I: Iterator<Item = char>>(iter: &mut Peekable<I>) {
    while iter.peek().copied().is_some_and(crate::rustc_lexer::is_whitespace) {
        iter.next();
    }
}
...
/// A `fmt::Write` sink shared with the test that reads it back.
struct Shared {
    data: Arc<Mutex<String>>,
}

impl core::fmt::Write for Shared {
    ...
    create_default_session_globals_then(|| {
        for (theme, expected_output) in [
            ...
            for span_label in &span_labels {
                ...
                eko::println!("span: {:?} label: {:?}", span, span_label.label);
                eko::println!("text: {:?}", source_map.span_to_snippet(span));
            }
