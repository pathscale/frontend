    ...
    fn eat_keyword_case(&mut self, exp: ExpKeywordPair, case: Case) -> bool {
            ...
            && ident.as_str().eq_ignore_ascii_case(exp.kw.as_str())
        {
            ...
            let case = match (is_upper, is_lower) {
                ...
                (true, false) => crate::rustc_parse::diagnostics::Case::Upper,
                (false, true) => crate::rustc_parse::diagnostics::Case::Lower,
                (false, false) => crate::rustc_parse::diagnostics::Case::Mixed,
            };

            self.dcx().emit_err(crate::rustc_parse::diagnostics::KwBadCase { span: ident.span, kw, case });
            self.bump();
            true
        } else {
            false
    ...
    pub fn parse_field_name(&mut self) -> PResult<'a, Ident> {
        if let token::Literal(token::Lit { kind: token::Integer, symbol, suffix }) = self.token.kind
        {
            if let Some(suffix) = suffix {
                self.dcx().emit_err(crate::rustc_parse::diagnostics::InvalidLiteralSuffixOnTupleIndex {
