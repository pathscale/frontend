...
use crate::rustc_ast::token::{self, Delimiter, MetaVarKind, TokenKind};
...
use crate::rustc_errors::{Applicability, PResult, StashKey, msg, struct_span_code_err};
...
impl<'a> Parser<'a> {
            ...
            if !matches!(vis.kind, VisibilityKind::Inherited) {
                ...
                if let Some((ident, _)) = this.token.ident()
                    ...
                    && let Some((similar_kw, is_incorrect_case)) = ident
                        .name
                        .find_similar(&crate::rustc_span::symbol::used_keywords(|| ident.span.edition()))
                {
    ...
    fn emit_error_attr_in_use_tree(
        ...
        self.dcx().emit_err(crate::rustc_parse::diagnostics::AttrInUseTree {
            attr_span,
            sub: Some(crate::rustc_parse::diagnostics::AttrInUseTreeSugg {
    ...
    fn report_invalid_macro_expansion_item(&self, args: &DelimArgs, path: Option<&Path>) {
        ...
        if !span.from_expansion() {
            ...
            {
                let replace =
                    if path.span.hi() + crate::rustc_span::BytePos(1) < open.lo() { "" } else { " " };
