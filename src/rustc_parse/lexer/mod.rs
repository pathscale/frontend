...
use crate::rustc_parse::lexer::diagnostics::TokenTreeDiagInfo;
use crate::rustc_parse::lexer::unicode_chars::UNICODE_ARRAY;
...
crate::static_assert_size!(crate::rustc_lexer::Token, 12);
...
pub(crate) fn lex_token_trees<'psess, 'src>(
    ...
    match strip_tokens {
        StripTokens::Shebang | StripTokens::ShebangAndFrontmatter => {
            if let Some(shebang_len) = crate::rustc_lexer::strip_shebang(src) {
                src = &src[shebang_len..];
...
impl<'psess, 'src> Lexer<'psess, 'src> {
    ...
    fn next_token_from_cursor(&mut self) -> (Token, bool) {
        ...
        loop {
            ...
            if let crate::rustc_lexer::TokenKind::Semi
            | crate::rustc_lexer::TokenKind::LineComment { .. }
            | crate::rustc_lexer::TokenKind::BlockComment { .. }
            | crate::rustc_lexer::TokenKind::CloseParen
            | crate::rustc_lexer::TokenKind::CloseBrace
            | crate::rustc_lexer::TokenKind::CloseBracket = token.kind
            {
            ...
            // Now "cook" the token, converting the simple `crate::rustc_lexer::TokenKind` enum into a
            // rich `crate::rustc_ast::TokenKind`. This turns strings into interned symbols and runs
            // additional validation.
            let kind = match token.kind {
                crate::rustc_lexer::TokenKind::LineComment { doc_style } => {
                ...
                crate::rustc_lexer::TokenKind::BlockComment { doc_style, terminated } => {
                    if !terminated {
                ...
                crate::rustc_lexer::TokenKind::Frontmatter { has_invalid_preceding_whitespace, invalid_infostring } => {
                    self.validate_frontmatter(start, has_invalid_preceding_whitespace, invalid_infostring);
                    preceded_by_whitespace = true;
                    continue;
                }
                crate::rustc_lexer::TokenKind::Whitespace => {
                    preceded_by_whitespace = true;
                    continue;
                }
                ...
                crate::rustc_lexer::TokenKind::UnknownPrefix => {
                    self.report_unknown_prefix(start);
                    self.ident(start)
                }
                crate::rustc_lexer::TokenKind::UnknownPrefixLifetime => {
                    self.report_unknown_prefix(start);
                ...
                // 2021.
                crate::rustc_lexer::TokenKind::Literal {
                    ...
                    self.psess.buffer_lint(
                        ...
                        span,
                        ast::CRATE_NODE_ID,
                        crate::rustc_parse::diagnostics::ReservedPrefixLint {
                            subject: "this".into(),
                            kind,
                ...
                crate::rustc_lexer::TokenKind::GuardedStrPrefix => {
                    self.maybe_report_guarded_str(start, str_before)
                }
                crate::rustc_lexer::TokenKind::Literal { kind, suffix_start } => {
                    let suffix_start = start + BytePos(suffix_start);
                    let (kind, symbol) = self.cook_lexer_literal(start, suffix_start, kind);
                    let suffix = if suffix_start < self.pos {
                        let string = self.str_from(suffix_start);
                        if string == "_" {
                            self.dcx().emit_err(crate::rustc_parse::diagnostics::UnderscoreLiteralSuffix {
                ...
                crate::rustc_lexer::TokenKind::Lifetime { starts_with_number } => {
                    // Include the leading `'` in the real identifier, for macro
                    // expansion purposes. See #12512 for the gory details of why
                    // this is necessary.
                ...
                crate::rustc_lexer::TokenKind::RawLifetime => {
                    self.last_lifetime = Some(self.mk_sp(start, start + BytePos(1)));
                ...
                crate::rustc_lexer::TokenKind::Semi => token::Semi,
                crate::rustc_lexer::TokenKind::Comma => token::Comma,
                crate::rustc_lexer::TokenKind::Dot => token::Dot,
                crate::rustc_lexer::TokenKind::OpenParen => token::OpenParen,
                crate::rustc_lexer::TokenKind::CloseParen => token::CloseParen,
                crate::rustc_lexer::TokenKind::OpenBrace => token::OpenBrace,
                crate::rustc_lexer::TokenKind::CloseBrace => token::CloseBrace,
                crate::rustc_lexer::TokenKind::OpenBracket => token::OpenBracket,
                crate::rustc_lexer::TokenKind::CloseBracket => token::CloseBracket,
                crate::rustc_lexer::TokenKind::At => token::At,
                crate::rustc_lexer::TokenKind::Pound => token::Pound,
                crate::rustc_lexer::TokenKind::Tilde => token::Tilde,
                crate::rustc_lexer::TokenKind::Question => token::Question,
                crate::rustc_lexer::TokenKind::Colon => token::Colon,
                crate::rustc_lexer::TokenKind::Dollar => token::Dollar,
                crate::rustc_lexer::TokenKind::Eq => token::Eq,
                crate::rustc_lexer::TokenKind::Bang => token::Bang,
                crate::rustc_lexer::TokenKind::Lt => token::Lt,
                crate::rustc_lexer::TokenKind::Gt => token::Gt,
                crate::rustc_lexer::TokenKind::Minus => token::Minus,
                crate::rustc_lexer::TokenKind::And => token::And,
                crate::rustc_lexer::TokenKind::Or => token::Or,
                crate::rustc_lexer::TokenKind::Plus => token::Plus,
                crate::rustc_lexer::TokenKind::Star => token::Star,
                crate::rustc_lexer::TokenKind::Slash => token::Slash,
                crate::rustc_lexer::TokenKind::Caret => token::Caret,
                crate::rustc_lexer::TokenKind::Percent => token::Percent,

                crate::rustc_lexer::TokenKind::Unknown | crate::rustc_lexer::TokenKind::InvalidIdent => {
                    ...
                    self.dcx().emit_err(crate::rustc_parse::diagnostics::UnknownTokenStart {
                        ...
                        repeat: if repeats > 0 {
                            ...
                            Some(crate::rustc_parse::diagnostics::UnknownTokenRepeat { repeats })
                        } else {
                            None
                        },
                    });
                ...
                crate::rustc_lexer::TokenKind::Eof => token::Eof,
            };
            let span = self.mk_sp(start, self.pos);
            ...
            self.psess.dyn_buffer_lint(
                ...
                move |dcx, level| {
                    ...
                    let characters = spans
                        .iter()
                        .map(|&(c, span)| crate::rustc_parse::diagnostics::UnicodeCharNoteSub {
                            span,
                            c_debug: format!("{c:?}"),
                        })
                        .collect();
                    let suggestions = (!spans.is_empty()).then_some(
                        crate::rustc_parse::diagnostics::UnicodeTextFlowSuggestion {
                            spans: spans.iter().map(|(_c, span)| *span).collect(),
                        },
                    );

                    crate::rustc_parse::diagnostics::UnicodeTextFlow {
                        comment_span: span,
                        characters,
                        suggestions,
    ...
    fn report_text_direction_codepoint(
        ...
        let count = spans.len();
        let labels =
            point_at_inner_spans.then_some(crate::rustc_parse::diagnostics::HiddenUnicodeCodepointsDiagLabels {
                spans: spans.clone(),
            });
        let sub = if point_at_inner_spans && !spans.is_empty() {
            crate::rustc_parse::diagnostics::HiddenUnicodeCodepointsDiagSub::Escape { spans }
        } else {
            crate::rustc_parse::diagnostics::HiddenUnicodeCodepointsDiagSub::NoEscape { spans, is_doc_comment }
        };
        ...
        self.psess.buffer_lint(
            ...
            span,
            ast::CRATE_NODE_ID,
            crate::rustc_parse::diagnostics::HiddenUnicodeCodepointsDiag {
                label,
                count,
                span_label: span,
                labels,
    ...
    fn validate_frontmatter(
        ...
        if has_invalid_preceding_whitespace {
            ...
            self.dcx().emit_err(crate::rustc_parse::diagnostics::FrontmatterInvalidOpeningPrecedingWhitespace {
                span,
        ...
        if invalid_infostring {
            ...
            self.dcx().emit_err(crate::rustc_parse::diagnostics::FrontmatterInvalidInfostring { span });
        }
        ...
        if let Some(cr_offset) = content.find('\r') {
            ...
            self.dcx().emit_err(crate::rustc_parse::diagnostics::BareCrFrontmatter { span });
        }
        ...
        if !last_line_trimmed.starts_with("---") {
            ...
            self.dcx().emit_err(crate::rustc_parse::diagnostics::FrontmatterUnclosed {
        ...
        if last_line_trimmed.len() != last_line.len() {
            ...
            self.dcx().emit_err(crate::rustc_parse::diagnostics::FrontmatterInvalidClosingPrecedingWhitespace {
                span,
                note_span: label_span,
            });
        }
        ...
        if len_close != len_opening {
            ...
            self.dcx().emit_err(crate::rustc_parse::diagnostics::FrontmatterLengthMismatch {
                span,
                opening,
                close,
                len_opening,
        ...
        if u8::try_from(len_opening).is_err() {
            self.dcx().emit_err(crate::rustc_parse::diagnostics::FrontmatterTooManyDashes { len_opening });
        }

        if !rest.trim_matches(is_horizontal_whitespace).is_empty() {
            let span = self.mk_sp(last_line_start_pos, self.pos);
            self.dcx().emit_err(crate::rustc_parse::diagnostics::FrontmatterExtraCharactersAfterClose { span });
        }
    }
    ...
    fn cook_doc_comment(
        ...
        if content.contains('\r') {
            for (idx, _) in content.char_indices().filter(|&(_, c)| c == '\r') {
                ...
                self.dcx().emit_err(crate::rustc_parse::diagnostics::CrDocComment { span, block });
            }
        }
    ...
    fn cook_lexer_literal(
        ...
        kind: crate::rustc_lexer::LiteralKind,
    ) -> (token::LitKind, Symbol) {
        match kind {
            crate::rustc_lexer::LiteralKind::Char { terminated } => {
                if !terminated {
                    let mut err = self
                        .dcx()
            ...
            crate::rustc_lexer::LiteralKind::Byte { terminated } => {
                if !terminated {
                    self.dcx()
                        .struct_span_fatal(
            ...
            crate::rustc_lexer::LiteralKind::Str { terminated } => {
                if !terminated {
                    self.dcx()
                        .struct_span_fatal(
            ...
            crate::rustc_lexer::LiteralKind::ByteStr { terminated } => {
                if !terminated {
                    self.dcx()
                        .struct_span_fatal(
                            self.mk_sp(start + BytePos(1), end),
            ...
            crate::rustc_lexer::LiteralKind::CStr { terminated } => {
                if !terminated {
                    self.dcx()
                        .struct_span_fatal(
                            self.mk_sp(start + BytePos(1), end),
            ...
            crate::rustc_lexer::LiteralKind::RawStr { n_hashes } => {
            ...
            crate::rustc_lexer::LiteralKind::RawByteStr { n_hashes } => {
                if let Some(n_hashes) = n_hashes {
                    let n = u32::from(n_hashes);
            ...
            crate::rustc_lexer::LiteralKind::RawCStr { n_hashes } => {
                if let Some(n_hashes) = n_hashes {
                    let n = u32::from(n_hashes);
            ...
            crate::rustc_lexer::LiteralKind::Int { base, empty_int } => {
                let mut kind = token::Integer;
                if empty_int {
                    let span = self.mk_sp(start, end);
                    let guar = self.dcx().emit_err(crate::rustc_parse::diagnostics::NoDigitsLiteral { span });
                    kind = token::Err(guar);
                    ...
                    for (idx, c) in s.char_indices() {
                        ...
                        if c != '_' && c.to_digit(base).is_none() {
                            let guar = self
                                .dcx()
                                .emit_err(crate::rustc_parse::diagnostics::InvalidDigitLiteral { span, base });
                            kind = token::Err(guar);
                        }
                    }
                }
            ...
            crate::rustc_lexer::LiteralKind::Float { base, empty_exponent } => {
                let mut kind = token::Float;
                if empty_exponent {
                    let span = self.mk_sp(start, self.pos);
                    let guar = self.dcx().emit_err(crate::rustc_parse::diagnostics::EmptyExponentFloat { span });
                    kind = token::Err(guar);
                }
                let base = match base {
                ...
                if let Some(base) = base {
                    ...
                    let guar = self
                        .dcx()
                        .emit_err(crate::rustc_parse::diagnostics::FloatLiteralUnsupportedBase { span, base });
                    kind = token::Err(guar)
                }
                (kind, self.symbol_from_to(start, end))
            }
    ...
    fn report_raw_str_error(&self, start: BytePos, prefix_len: u32) -> ! {
        match crate::rustc_lexer::validate_raw_str(self.str_from(start), prefix_len) {
    ...
    fn report_unknown_prefix(&self, start: BytePos) {
        ...
        if expn_data.edition.at_least_rust_2021() {
            ...
            let sugg = if prefix == "rb" {
                Some(crate::rustc_parse::diagnostics::UnknownPrefixSugg::UseBr(prefix_span))
            } else if prefix == "rc" {
                Some(crate::rustc_parse::diagnostics::UnknownPrefixSugg::UseCr(prefix_span))
                    ...
                    && !self.psess.source_map().is_multiline(start.until(end))
                {
                    ...
                    Some(crate::rustc_parse::diagnostics::UnknownPrefixSugg::MeantStr { start, end })
                } else {
                    Some(crate::rustc_parse::diagnostics::UnknownPrefixSugg::Whitespace(
            ...
            self.dcx().emit_err(crate::rustc_parse::diagnostics::UnknownPrefix {
                span: prefix_span,
                prefix,
                sugg,
            });
    ...
    fn maybe_report_guarded_str(&mut self, start: BytePos, str_before: &'src str) -> TokenKind {
        ...
        let (is_string, span, unterminated) = match cursor.guarded_double_quoted_string() {
            Some(crate::rustc_lexer::GuardedStr { n_hashes, terminated, token_len }) => {
        ...
        if edition2024 {
            ...
            let sugg = if span.from_expansion() {
                ...
                Some(crate::rustc_parse::diagnostics::GuardedStringSugg(space_span))
            };

            // In Edition 2024 and later, emit a hard error.
            let err = if is_string {
                self.dcx().emit_err(crate::rustc_parse::diagnostics::ReservedString { span, sugg })
            } else {
                self.dcx().emit_err(crate::rustc_parse::diagnostics::ReservedMultihash { span, sugg })
            };
    ...
    fn report_too_many_hashes(&self, start: BytePos, num: u32) -> ! {
        self.dcx().emit_fatal(crate::rustc_parse::diagnostics::TooManyHashes {
