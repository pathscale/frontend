...
use crate::rustc_span::{AttrId, Ident, Symbol, sym};
...
macro_rules! declare_lint {
    ...
    ($(#[$attr:meta])* $vis: vis $NAME: ident, $Level: ident, $desc: expr,
     ...
     $($v:ident),*) => (
        $(#[$attr])*
        $vis static $NAME: &$crate::rustc_lint_defs::Lint = &$crate::rustc_lint_defs::Lint {
            name: stringify!($NAME),
            default_level: $crate::rustc_lint_defs::$Level,
            desc: $desc,
            is_externally_loaded: false,
            $($v: true,)*
            $(feature_gate: Some(crate::rustc_span::sym::$gate),)?
            $(future_incompatible: Some($crate::rustc_lint_defs::FutureIncompatibleInfo {
                reason: $reason,
                $($field: $val,)*
                ..$crate::rustc_lint_defs::FutureIncompatibleInfo::default_fields_for_macro()
            }),)?
            $(edition_lint_opts: Some(($crate::rustc_span::edition::Edition::$lint_edition, $crate::rustc_lint_defs::$edition_level)),)?
            $(eval_always: $eval_always,)?
            $(rust_version: Some($crate::rustc_lint_defs::Lint::parse_rust_version($msrv)),)?
            ..$crate::rustc_lint_defs::Lint::default_fields_for_macro()
        };
    );
}
...
macro_rules! impl_lint_pass {
    ($ty:ty => [$($lint:expr),* $(,)?]) => {
        impl $crate::rustc_lint_defs::LintPass for $ty {
            fn name(&self) -> &'static str { stringify!($ty) }
            fn get_lints(&self) -> $crate::rustc_lint_defs::LintVec { vec![$($lint),*] }
        }
        impl $ty {
            #[allow(unused)]
            pub fn lint_vec() -> $crate::rustc_lint_defs::LintVec { vec![$($lint),*] }
        }
    };
}
...
macro_rules! fcw {
    (FutureReleaseError # $issue_number: literal) => {
       $crate::rustc_lint_defs:: FutureIncompatibilityReason::FutureReleaseError($crate::rustc_lint_defs::ReleaseFcw { issue_number: $issue_number })
    };
    (FutureReleaseSemanticsChange # $issue_number: literal) => {
        $crate::rustc_lint_defs::FutureIncompatibilityReason::FutureReleaseSemanticsChange($crate::rustc_lint_defs::ReleaseFcw {
            issue_number: $issue_number,
        })
    };
    ($description: literal # $issue_number: literal) => {
        $crate::rustc_lint_defs::FutureIncompatibilityReason::Custom($description, $crate::rustc_lint_defs::ReleaseFcw {
            issue_number: $issue_number,
        })
    };
    (EditionError $edition_name: tt $page_slug: literal) => {
        $crate::rustc_lint_defs::FutureIncompatibilityReason::EditionError($crate::rustc_lint_defs::EditionFcw {
            edition: fcw!(@edition $edition_name),
            page_slug: $page_slug,
        })
    };
    (EditionSemanticsChange $edition_name: tt $page_slug: literal) => {
        $crate::rustc_lint_defs::FutureIncompatibilityReason::EditionSemanticsChange($crate::rustc_lint_defs::EditionFcw {
            edition: fcw!(@edition $edition_name),
            page_slug: $page_slug,
        })
    };
    (EditionAndFutureReleaseSemanticsChange $edition_name: tt $page_slug: literal) => {
        $crate::rustc_lint_defs::FutureIncompatibilityReason::EditionAndFutureReleaseSemanticsChange($crate::rustc_lint_defs::EditionFcw {
            edition: fcw!(@edition $edition_name),
            page_slug: $page_slug,
        })
    };
    (EditionAndFutureReleaseError $edition_name: tt $page_slug: literal) => {
        $crate::rustc_lint_defs::FutureIncompatibilityReason::EditionAndFutureReleaseError($crate::rustc_lint_defs::EditionFcw {
            edition: fcw!(@edition $edition_name),
            page_slug: $page_slug,
        })
    };
    (@edition 2024) => {
        crate::rustc_span::edition::Edition::Edition2024
    };
    (@edition 2021) => {
        crate::rustc_span::edition::Edition::Edition2021
    };
    (@edition 2018) => {
        crate::rustc_span::edition::Edition::Edition2018
    };
}
pub use crate::declare_lint;
pub use crate::declare_lint_pass;
pub use crate::declare_tool_lint;
pub use crate::fcw;
pub use crate::impl_lint_pass;
pub use crate::pluralize;
