...
use crate::rustc_span::{SpanDecoder, SpanEncoder, Symbol};
...
impl StableCrateId {
    ...
    pub fn new(
        ...
        // during testing.
        if let Some(val) = eko::env::var_os("RUSTC_FORCE_RUSTC_VERSION") {
            // The value is hashed, so the lossy `String` round-trip was only ever a way of
            // getting back to bytes. These are the bytes.
            hasher.write(&val)
        } else {
...
crate::define_id_collections!(DefIdMap, DefIdSet, DefIdMapEntry, DefId);
