...
fn read_binary_file_handles_lying_stat() {
    ...
    // eko::file::read. So we test that we have the same behavior even on files where
    // fs::metadata lies.

    // stat always says that /proc/self/cmdline is length 0, but it isn't.
    let cmdline = Path::new("/proc/self/cmdline");
    let len = eko::file::metadata(cmdline).unwrap().len() as usize;
    let real = eko::file::read(cmdline).unwrap();
    assert!(len < real.len());
    ...
    let len = eko::file::metadata(kernel_max).unwrap().len() as usize;
    let real = eko::file::read(kernel_max).unwrap();
    assert!(len > real.len());
