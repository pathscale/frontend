...
pub trait FileLoader {
    ...
    fn read_file(&self, path: &Path) -> file::Result<String>;

    /// Read the contents of a potentially non-UTF-8 file into memory.
    /// We don't normalize binary files, so we can start in an Arc.
    fn read_binary_file(&self, path: &Path) -> file::Result<Arc<[u8]>>;

    /// Current working directory
    fn current_directory(&self) -> file::Result<PathBuf>;
}

/// A `FileLoader` that reads real files off the disk.
...
impl FileLoader for RealFileLoader {
    ...
    fn read_file(&self, path: &Path) -> file::Result<String> {
        // The size check comes before the read, so an oversized file is refused rather than
        // allocated. `MAX_FILE_SIZE` is a limit of the span encoding, not of the filesystem.
        let size = file::metadata(path).map(|m| m.len()).unwrap_or(0);
        if size > SourceFile::MAX_FILE_SIZE.into() {
            return Err(file::Error::too_large());
        }
        file::read_to_string(path)
    }

    fn read_binary_file(&self, path: &Path) -> file::Result<Arc<[u8]>> {
        // `std`'s version read into an uninitialised `Arc` through the unstable `BorrowedBuf`
        // to avoid zeroing, then probed for extra bytes because the file could have grown since
        // the `metadata` call, falling back to a `Vec` when it had. All of that goes: `file::read`
        // sizes its buffer from `fstat` on the already-open descriptor and keeps reading to EOF,
        // so a file that grows is read whole and one that shrinks costs a short read. The peak
        // memory behaviour the fallback existed to protect is what the plain path now does.
        file::read(path).map(Arc::from)
    }

    fn current_directory(&self) -> file::Result<PathBuf> {
        eko::env::current_dir().map(PathBuf::from_bytes).ok_or_else(file::Error::no_cwd)
    }
}
...
impl core::fmt::Debug for SourceMapFiles {
...
impl SourceMap {
    ...
    pub fn load_file(&self, path: &Path) -> file::Result<Arc<SourceFile>> {
    ...
    /// takes place.
    pub fn load_binary_file(&self, path: &Path) -> file::Result<(Arc<[u8]>, Span)> {
    ...
    pub fn new_source_file(&self, filename: FileName, src: String) -> Arc<SourceFile> {
        self.try_new_source_file(filename, src).unwrap_or_else(|OffsetOverflowError| {
            ...
            crate::rustc_span::fatal_error::FatalError.raise()
        })
    }
