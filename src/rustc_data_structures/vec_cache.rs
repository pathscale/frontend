...
impl SlotIndex {
    ...
    fn initialize_bucket<V>(bucket: &AtomicPtr<Slot<V>>, bucket_idx: BucketIndex) -> *mut Slot<V> {
        // `eko::thread::Mutex::new` is a `const fn`, so it can initialise a `static`.
        static LOCK: eko::thread::Mutex<()> = eko::thread::Mutex::new(());
        ...
        // `eko`'s mutex has no poisoning, so `lock()` hands back the guard directly and
        // the `unwrap_or_else(|e| e.into_inner())` recovery it used to need is gone.
        let _allocator_guard = LOCK.lock();
        ...
        if ptr.is_null() {
            let bucket_layout =
                core::alloc::Layout::array::<Slot<V>>(bucket_idx.capacity()).unwrap();
