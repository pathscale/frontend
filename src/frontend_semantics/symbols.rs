//! Export roots and symbol names required by monomorphization.
...
use crate::rustc_crate_store::CrateDepKind;
...
use crate::rustc_symbol_mangling::is_offload_kernel;
use crate::rustc_target::spec::{Arch, TlsModel};
use tracing::debug;
...
pub fn exported_non_generic_symbols_helper<'tcx>(
    ...
    let is_device_offload = tcx
        ...
        .offload
        .iter()
        .any(|o| matches!(o, crate::rustc_session::config::Offload::Device(_)));
    if is_device_offload {
        let crate_items = tcx.hir_crate_items(());
        let mut seen: crate::rustc_data_structures::fx::FxHashSet<DefId> = symbols
            .iter()
...
fn exported_generic_symbols_provider_local<'tcx>(
    ...
    if export_generics || is_device_offload {
        ...
        use crate::rustc_middle::mono::{MonoItem, Visibility};
...
pub(crate) fn symbol_name_for_instance_in_crate<'tcx>(
    ...
    match symbol {
        ExportedSymbol::NonGeneric(def_id) => {
            crate::rustc_symbol_mangling::symbol_name_for_instance_in_crate(
                tcx,
        ...
        ExportedSymbol::DropGlue(ty) => crate::rustc_symbol_mangling::symbol_name_for_instance_in_crate(
            tcx,
...
pub fn escape_symbol_name(tcx: TyCtxt<'_>, symbol: &str, span: Span) -> String {
    ...
    use crate::rustc_target::spec::{Arch, BinaryFormat};
    if !symbol.is_empty()
