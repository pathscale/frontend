...
use crate::rustc_hir::def_id::LocalDefIdMap;
use crate::rustc_hir::definitions::{DefPathData, PerParentDisambiguatorsMap};
...
use crate::rustc_middle::ty::{self, TyCtxt, TypeSuperVisitable, TypeVisitor, Unnormalized};
...
use crate::rustc_hir_analysis::hir::definitions::PerParentDisambiguatorState;
...
impl<'a, 'tcx> Visitor<'tcx> for BoundVarContext<'a, 'tcx> {
    ...
    fn visit_test_binder_forall(
        &mut self,
        forall: &'tcx crate::rustc_hir::TestBinderForall<'tcx>,
    ) -> Self::Result {
    ...
    fn visit_test_binder_exists(
        &mut self,
        exists: &'tcx crate::rustc_hir::TestBinderExists<'tcx>,
    ) -> Self::Result {
...
impl<'a, 'tcx> BoundVarContext<'a, 'tcx> {
        ...
        let result = loop {
            match *scope {
                ...
                Scope::Binder { ref bound_vars, scope_type, s, where_bound_origin, .. } => {
                    ...
                    {
                        let mut diag: crate::rustc_errors::Diag<'_> =
                            crate::rustc_session::diagnostics::feature_err(
                                &self.tcx.sess,
    ...
    fn emit_opaque_capture_errors(&self) -> Option<ErrorGuaranteed> {
        ...
        let mut span = crate::rustc_errors::MultiSpan::from_span(errors.capture_spans[0]);
        for &capture_span in &errors.capture_spans[1..] {
            span.push_span_label(capture_span, "");
        }
        let decl_span = crate::rustc_errors::MultiSpan::from_spans(errors.decl_spans);
    ...
    fn for_each_trait_bound_on_res(&self, expected_res: Res) -> impl Iterator<Item = DefId> {
        // This was a `gen move {}` block (unstable). It is now an iterator chain that visits the
        // same scopes in the same order and yields the same ids, lazily as before.
        let tcx = self.tcx;
        // The scope chain, innermost first, ending at the root.
        let scopes = core::iter::successors(Some(self.scope), |scope| match **scope {
            Scope::Binder { s, .. }
            | Scope::Body { s, .. }
            | Scope::ObjectLifetimeDefault { s, .. }
            | Scope::Supertrait { s, .. }
        ...
        scopes.flat_map(move |scope| {
            let hir_id = match *scope {
                ...
                Scope::Root { opt_parent_item: Some(parent_def_id) } => {
                    Some(tcx.local_def_id_to_hir_id(parent_def_id))
                }
                Scope::Body { .. }
                | Scope::ObjectLifetimeDefault { .. }
                | Scope::Supertrait { .. }
            ...
            hir_id.into_iter().flat_map(move |hir_id| {
                ...
                let trait_itself = if let Res::SelfTyParam { trait_: _ } = expected_res
                    && let hir::Node::Item(item) = node
                    && let hir::ItemKind::Trait { .. } = item.kind
                {
                    // Yield the trait's def id. Supertraits will be
                    // elaborated from that.
                    Some(item.owner_id.def_id.to_def_id())
                } else {
                    None
                };
                // Only consulted when the trait itself was not yielded, as in the `else if`.
                let generics = if trait_itself.is_none() { node.generics() } else { None };
                let bound_trait_ids = generics
                    .into_iter()
                    .flat_map(|generics| generics.predicates)
                    .filter_map(move |pred| {
                        ...
                        (bounded_path.res == expected_res).then_some(pred.bounds)
                    })
                    .flatten()
                ...
                trait_itself.into_iter().chain(bound_trait_ids)
            })
        })
    }
}
