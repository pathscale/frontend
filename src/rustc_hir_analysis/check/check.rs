...
use crate::rustc_abi::{ExternAbi, FieldIdx, MAX_SIMD_LANES, ScalableElt};
use crate::rustc_data_structures::unord::{UnordMap, UnordSet};
use crate::rustc_errors::codes::*;
use crate::rustc_errors::{Diag, DiagCtxtHandle, Diagnostic, EmissionGuarantee, Level, MultiSpan};
...
use crate::rustc_hir::{Node, find_attr, intravisit};
use crate::rustc_infer::infer::{RegionVariableOrigin, TyCtxtInferExt};
use crate::rustc_infer::traits::{Obligation, ObligationCauseCode, TraitErrors, WellFormedLoc};
use crate::rustc_lint_defs::builtin::{DEAD_CODE, UNINHABITED_STATIC, UNSUPPORTED_CALLING_CONVENTIONS};
...
use super::*;
use crate::rustc_hir_analysis::check::wfcheck::{
...
pub(crate) fn check_item_type(tcx: TyCtxt<'_>, def_id: LocalDefId) -> Result<(), ErrorGuaranteed> {
    ...
    match tcx.def_kind(def_id) {
        ...
        DefKind::Enum => {
            ...
            tcx.ensure_ok().type_of(def_id);
            tcx.ensure_ok().clauses_of(def_id);
            crate::rustc_hir_analysis::collect::check_enum_variant_types(tcx, def_id);
        ...
        def_kind @ (DefKind::Struct | DefKind::Union) => {
            ...
            if let Some((_, ctor_def_id)) = adt.ctor {
                crate::rustc_hir_analysis::collect::check_ctor(tcx, ctor_def_id.expect_local());
            }
            check_variances_for_type_defn(tcx, def_id);
