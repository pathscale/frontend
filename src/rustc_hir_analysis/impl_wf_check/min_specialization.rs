...
//!
//! ```ignore (illustrative)
//! impl<T> SpecExtend<T> for alloc::vec::IntoIter<T> { /* specialized impl */ }
//! impl<T, I: Iterator<Item=T>> SpecExtend<T> for I { /* default impl */ }
//! ```
//!
//! We get that the generic parameters for `impl2` are `[T, alloc::vec::IntoIter<T>]`.
//! `T` is constrained to be `<I as Iterator>::Item`, so we check only
//! `alloc::vec::IntoIter<T>` for repeated parameters, which it doesn't have. The
...
use crate::rustc_hir_analysis::diagnostics::GenericArgsOnOverriddenImpl;
use crate::rustc_hir_analysis::{constrained_generic_params as cgp, diagnostics};
