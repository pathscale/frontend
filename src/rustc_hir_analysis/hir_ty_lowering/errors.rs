...
use crate::rustc_hir_analysis::hir_ty_lowering::{AssocItemQSelf, HirTyLowerer};
...
impl<'tcx> dyn HirTyLowerer<'tcx> + '_ {
    ...
    pub(super) fn report_ambiguous_assoc_item(
        ...
        let qself_str = qself.to_string(tcx);
        let mut err = self.dcx().create_err(crate::rustc_hir_analysis::diagnostics::AmbiguousAssocItem {
            span,
    ...
    // FIXME(fmease): Heavily adapted from `crate::rustc_hir_typeck::method::suggest`. Deduplicate.
    ...
    pub(crate) fn report_unresolved_inherent_assoc_item(
        ...
        // FIXME(fmease): This was copied in parts from an old version of `crate::rustc_hir_typeck::method::suggest`.
        // Either
        ...
        if fulfillment_errors.is_empty() {
            // FIXME(fmease): Copied from `crate::rustc_hir_typeck::method::probe`. Deduplicate.
        ...
        // FIXME(fmease): `crate::rustc_hir_typeck::method::suggest` uses a `skip_list` to filter out some bounds.
...
impl<'a, 'tcx> crate::rustc_errors::Diagnostic<'a, ()> for AmbiguityBetweenVariantAndAssocItem<'tcx> {
    fn into_diag(
        self,
