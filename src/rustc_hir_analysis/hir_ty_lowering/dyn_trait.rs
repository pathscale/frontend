...
use crate::rustc_lint_defs::builtin::{BARE_TRAIT_OBJECTS, UNUSED_ASSOCIATED_TYPE_BOUNDS};
...
use crate::rustc_hir_analysis::diagnostics::DynTraitAssocItemBindingMentionsSelf;
...
impl<'tcx> dyn HirTyLowerer<'tcx> + '_ {
        ...
        for &(projection_bound, span) in projection_bounds.values() {
            ...
            if tcx.generics_require_sized_self(def_id) {
                ...
                tcx.emit_node_span_lint(
                    ...
                    hir_id,
                    span,
                    crate::rustc_hir_analysis::diagnostics::UnusedAssociatedTypeBounds { span },
                );
            }
        }
    ...
    ) -> Option<ty::Region<'tcx>> // if None, use the default
    {
        ...
        if derived_region_bounds[1..].iter().any(|r1| r != *r1) {
            self.dcx().emit_err(crate::rustc_hir_analysis::diagnostics::AmbiguousLifetimeBound { span });
        }
        Some(r)
    }
    ...
    fn prohibit_or_lint_bare_trait_object_ty(
        ...
        if span.edition().at_least_rust_2021() {
            let mut diag = crate::struct_span_code_err!(
                self.dcx(),
                span,
                E0782,
                "{}",
