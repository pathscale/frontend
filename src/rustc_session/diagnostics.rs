...
use crate::rustc_session::lint::builtin::UNSTABLE_SYNTAX_PRE_EXPANSION;
...
pub(crate) struct ProfileUseFileDoesNotExist<'a> {
    pub(crate) path: &'a eko::path::Path,
}
...
pub(crate) struct FileIsNotWriteable<'a> {
    pub(crate) file: &'a eko::path::Path,
}
...
// `FailedToCreateProfiler` ("failed to create profiler: {$err}") was here. There is no profiler
// to fail to create: `-Zself-profile` was implemented by `measureme`, which is std-only and was
// removed. The flag now warns that it does nothing.
#[derive(Diagnostic)]
#[diag(
    "`-Zself-profile` is not supported and produces no profiling data: the `measureme` recorder was removed with `std`"
)]
pub(crate) struct SelfProfileUnsupported;
