...
use crate::rustc_session::session::Session;
...
pub fn extra_compiler_flags() -> Option<(Vec<String>, bool)> {
    ...
    // `eko::env::args()` hands back the raw argv bytes rather than `OsString`s, so the
    // lossy UTF-8 conversion is spelled out here instead of going through `OsStr`.
    let mut args =
        eko::env::args().into_iter().map(|arg| String::from_utf8_lossy(&arg).to_string());
...
pub fn was_invoked_from_cargo() -> bool {
    ...
    *FROM_CARGO.get_or_init(|| eko::env::var_os("CARGO_CRATE_NAME").is_some())
}
