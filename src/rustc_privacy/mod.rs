// `std` re-exports this macro at its root ungated; in `core` it lives behind the feature.
...
use crate::rustc_hir::{self as hir, AmbigArg, ForeignItemId, ItemId, OwnerId, PatKind, find_attr};
...
struct EmbargoVisitor<'tcx> {
    ...
    /// `FxIndexSet`, not `IndexSet`. `indexmap`'s default hasher is `RandomState`, which is
    /// std, so with `indexmap/std` off the type parameter has no default and has to be named.
    queue: FxIndexSet<LocalDefId>,
...
impl<'tcx> crate::rustc_ty_walk::SpannedTypeVisitor<'tcx> for TypePrivacyVisitor<'tcx> {
