...
use crate::rustc_ast_pretty::pprust::{path_to_string, where_bound_predicate_to_string};
use crate::rustc_data_structures::fx::{FxHashMap, FxHashSet, FxIndexMap, FxIndexSet};
use crate::rustc_data_structures::unord::UnordItems;
...
use crate::rustc_hir::def::{CtorKind, CtorOf, DefKind, MacroKinds};
use crate::rustc_hir::def_id::{CRATE_DEF_ID, DefId};
use crate::rustc_hir::{MissingLifetimeKind, PrimTy, find_attr};
...
use crate::rustc_resolve::diagnostics::impls::{ImportSuggestion, LabelSuggestion, TypoSuggestion};
...
use crate::rustc_resolve::ty::fast_reject::SimplifiedType;
...
impl<'ast, 'ra, 'tcx> LateResolutionVisitor<'_, 'ast, 'ra, 'tcx> {
    ...
    fn suggest_range_struct_destructuring(
        ...
        let (struct_path, fields) = match (start_snippet, end_snippet, &end_kind.node) {
            (Some(start), Some(end), ast::RangeEnd::Excluded) => (
                resolve_short_name(sym::Range, "core::ops::Range"),
                vec![field("start", start), field("end", end)],
            ),
            (Some(start), Some(end), ast::RangeEnd::Included(_)) => (
                resolve_short_name(sym::RangeInclusive, "core::ops::RangeInclusive"),
                vec![field("start", start), field("end", end)],
            ),
            (Some(start), None, _) => (
                resolve_short_name(sym::RangeFrom, "core::ops::RangeFrom"),
                vec![field("start", start)],
            ),
            (None, Some(end), ast::RangeEnd::Excluded) => {
                (resolve_short_name(sym::RangeTo, "core::ops::RangeTo"), vec![field("end", end)])
            }
            (None, Some(end), ast::RangeEnd::Included(_)) => (
                resolve_short_name(sym::RangeToInclusive, "core::ops::RangeToInclusive"),
                vec![field("end", end)],
            ),
            _ => return,
        };
    ...
    fn get_single_associated_item(
        ...
        if let crate::rustc_resolve::PathSource::TraitItem(_, _) = source {
            let mod_path = &path[..path.len() - 1];
        ...
        if let Some(open_brace_span) = sm.span_followed_by(span, "{") {
            ...
            let close_brace_span =
                sm.span_to_next_source(open_brace_span).ok().and_then(|next_source| {
                    ...
                    let start = open_brace_span.hi() + crate::rustc_span::BytePos(offset as u32);
                    Some(open_brace_span.with_lo(start).with_hi(start + crate::rustc_span::BytePos(1)))
                });
    ...
    fn suggest_alternative_construction_methods(
        ...
        // suggestion machinery to get `core::default::Default` and perform some
            ...
            .any(|did| did == def_id)
        {
            err.multipart_suggestion(
                ...
                vec![
                    ...
                    (
                        path_span.shrink_to_hi().with_hi(call_span.hi()),
                        " as core::default::Default>::default()".to_string(),
                    ),
                ],
                Applicability::MaybeIncorrect,
            );
