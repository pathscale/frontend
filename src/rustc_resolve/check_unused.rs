...
use crate::rustc_errors::{DiagArgValue, Diagnostic, MultiSpan};
...
use crate::rustc_span::{DUMMY_SP, Ident, Span, kw};

use crate::rustc_resolve::imports::{Import, ImportKind};
use crate::rustc_resolve::{DeclKind, IdentKey, LateDecl, Resolver, diagnostics, module_to_string};
    ...
    fn report_unused_extern_crate_items(
        ...
        for extern_crate in &self.extern_crate_items {
            ...
            if warn_if_unused {
                if let Some(&span) = maybe_unused_extern_crates.get(&extern_crate.id) {
                    self.r.lint_buffer.buffer_lint(
                        ...
                        extern_crate.id,
                        span,
                        crate::rustc_resolve::diagnostics::UnusedExternCrate {
            ...
            self.r.lint_buffer.buffer_lint(
                ...
                extern_crate.id,
                extern_crate.span,
                crate::rustc_resolve::diagnostics::ExternCrateNotIdiomatic {
            ...
            match import.kind {
                    ...
                    if let ImportKind::MacroUse { .. } = import.kind {
                        if !import.span.is_dummy() {
                            self.lint_buffer.buffer_lint(
                                ...
                                import.root_id,
                                import.span,
                                crate::rustc_resolve::diagnostics::MacroUseDeprecated,
                            );
                        }
                    }
                }
                ...
                ImportKind::MacroUse { .. } => {
                    self.lint_buffer.buffer_lint(
                        ...
                        import.root_id,
                        import.span,
                        crate::rustc_resolve::diagnostics::UnusedMacroUse,
                    );
                }
                _ => {}
            }
            ...
            visitor.r.lint_buffer.dyn_buffer_lint_any(
                ...
                move |dcx, level, sess| {
                    ...
                    let test_module_span = test_module_span.map(|span| {
                        sess.downcast_ref::<crate::rustc_session::Session>()
