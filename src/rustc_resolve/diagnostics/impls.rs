...
use crate::rustc_hir::attrs::diagnostic::{CustomDiagnostic, Directive, FormatArgs};
use crate::rustc_hir::attrs::{AttributeKind, CfgEntry, StrippedCfgItem};
use crate::rustc_hir::def::Namespace::{self, *};
use crate::rustc_hir::def::{CtorKind, CtorOf, DefKind, MacroKinds, NonMacroAttrKind, PerNS};
use crate::rustc_hir::def_id::{CRATE_DEF_ID, DefId};
use crate::rustc_hir::{Attribute, PrimTy, Stability, StabilityLevel, find_attr};
...
use crate::bug;
use crate::rustc_middle::ty::{TyCtxt, Visibility};
...
impl<'r, 'ra, 'tcx> CmResolver<'r, 'ra, 'tcx> {
    ...
    pub(crate) fn lint_if_path_starts_with_module(
        ...
        match path.get(1) {
            // If this import looks like `crate::rustc_resolve::...` it's already good
        ...
        // `ExternCrate` (also used for `crate::rustc_resolve::...`) then no need to issue a
        ...
        self.lint_buffer_mut().dyn_buffer_lint_any(
            ...
            move |dcx, level, sess| {
                    ...
                    .span_to_snippet(root_span)
                {
                    ...
                    Err(_) => ("crate::rustc_resolve::<path>".to_string(), Applicability::HasPlaceholders),
                };
            ...
            in_module.for_each_child(self, |this, ident, orig_ident_span, ns, name_binding| {
                    ...
                    && filter_fn(res)
                {
                    ...
                    let mut segms = if lookup_ident.span.at_least_rust_2018() {
                        // crate-local absolute paths start with `crate::rustc_resolve::` in edition 2018
                    ...
                    // meet `core::ops::Range` after `core::range::legacy::Range`,
        ...
        for ns in [Namespace::MacroNS, Namespace::TypeNS, Namespace::ValueNS] {
            ...
            let desc = match binding.res() {
                ...
                Res::Def(DefKind::Trait, trait_def_id) if macro_kind == MacroKind::Derive => {
                    if let crate::rustc_resolve::DeclKind::Import { import, .. } = binding.kind
...
impl<'ra, 'tcx> Resolver<'ra, 'tcx> {
            ...
            if kind != AmbiguityKind::GlobVsGlob {
                if let Scope::ModuleNonGlobs(module, _) | Scope::ModuleGlobs(module, _) = scope {
                    if module == self.graph_root.to_module() {
                        help_msgs.push(format!(
                            "use `crate::rustc_resolve::{ident}` to refer to this {thing} unambiguously"
                        ));
    ...
    fn shorten_import_path(
        ...
        // to the crate root without an explicit `crate::rustc_resolve::` prefix).
        let candidate_names = {
        ...
        // drop the `crate::rustc_resolve::` prefix entirely instead of replacing it with `self::`.
                        ...
                        module_path.map(|module_path| {
                            ...
                            let mut path = Path {
                                ...
                                segments: module_path
                                    .into_iter()
                                    .chain(core::iter::once(ident.name))
                                    .map(|name| {
...
impl<'ra, 'tcx> Resolver<'ra, 'tcx> {
    ...
    fn undeclared_module_exists(&self, ident: Ident) -> Option<eko::path::PathBuf> {
    ...
    /// Suggests a missing `crate::rustc_resolve::` if that resolves to an correct module.
    ///
    /// ```text
    ///    |
    /// LL | use foo::Bar;
...
fn show_candidates(
    ...
    for path_strings in [&mut accessible_path_strings, &mut inaccessible_path_strings] {
        ...
        let foreign_crate_path_strings =
            path_strings.extract_if(.., |p| !p.0.starts_with("crate::rustc_resolve::")).collect::<Vec<_>>();
