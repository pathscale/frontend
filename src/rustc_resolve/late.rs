...
/// Iterates a `Range<NodeId>`. `Range<NodeId>` is not an iterator on stable, because `NodeId`
/// cannot implement the unstable `Step` trait.
fn node_id_range(ids: &Range<NodeId>) -> impl Iterator<Item = NodeId> + use<> {
    (ids.start.as_u32()..ids.end.as_u32()).map(NodeId::from_u32)
}
    ...
    fn visit_foreign_item(&mut self, foreign_item: &'ast ForeignItem) {
        self.with_owner(foreign_item.id, |this| {
            ...
            match foreign_item.kind {
                ...
                ForeignItemKind::Fn(ref fn_item) => {
...
impl<'a, 'ast, 'ra, 'tcx> LateResolutionVisitor<'a, 'ast, 'ra, 'tcx> {
                    ...
                    let guar = if elided {
                        let suggestion = if self.diag_metadata.in_assoc_ty_binding {
                            ...
                            self.lifetime_ribs[i..].iter().rev().find_map(|rib| {
                                ...
                                {
                                    Some(crate::rustc_resolve::diagnostics::ElidedAnonymousLifetimeReportErrorSuggestion {
                        ...
                        if !self.in_func_body
                            ...
                            && let crate::rustc_resolve::ModuleKind::Def(DefKind::Trait, trait_id, _, _) =
                                module.kind
                        {
                            if def_id_matches_path(
                                ...
                                self.r.dcx().emit_err(
                                    crate::rustc_resolve::diagnostics::LendingIteratorReportError {
                                ...
                                let mut err = self.r.dcx().create_err(
                                    crate::rustc_resolve::diagnostics::AnonymousLifetimeNonGatReportError {
                                        lifetime: lifetime.ident.span,
                                        decl,
                                    },
                                );
                            ...
                            let mut err = self.r.dcx().create_err(
                                crate::rustc_resolve::diagnostics::ElidedAnonymousLifetimeReportError {
                                    span: lifetime.ident.span,
                                    suggestion,
                                },
                            );
                        ...
                        self.r.dcx().emit_err(
                            crate::rustc_resolve::diagnostics::ExplicitAnonymousLifetimeReportError {
                                span: lifetime.ident.span,
                            },
                        )
                    };
    ...
    fn resolve_elided_lifetimes_in_path(
        ...
        for (i, segment) in path.iter().enumerate() {
            ...
            if inferred {
                ...
                for id in node_id_range(&node_ids) {
                    self.record_lifetime_use(
                        id,
                        LifetimeRes::Infer,
                        LifetimeElisionCandidate::Ignore,
            ...
            for rib in self.lifetime_ribs.iter().rev() {
                match rib.kind {
                    ...
                    //
                    //     impl Foo for core::cell::Ref<u32> // note lack of '_
                    //     async fn foo(_: core::cell::Ref<u32>) { ... }
                    ...
                    | LifetimeRibKind::Elided { error_in_path: true, .. } => {
                        ...
                        let guar = self.r.dcx().emit_err(
                            crate::rustc_resolve::diagnostics::ImplicitElidedLifetimeNotAllowedHere {
                                span: path_span,
                                subdiag,
                            },
                        );
                ...
                self.sink.lint_buffer.dyn_buffer_lint_any(
                    ...
                    move |dcx, level, sess| {
                        let source_map = sess
                            ...
                            .source_map();
                        crate::rustc_resolve::diagnostics::ElidedLifetimesInPaths {
                ...
                for (_, nodes) in elision_failures {
                    match nodes {
                        ...
                        Either::Right(node_ids) => {
                            for lifetime in node_id_range(&node_ids) {
                                record_res(lifetime)
                            }
                        }
                    }
        ...
        if let [segment, rest @ ..] = use_tree.prefix.segments.as_slice() {
            ...
            let report_error = |this: &Self, ns| {
                if this.should_report_errs() {
                    ...
                    this.r.dcx().emit_err(crate::rustc_resolve::diagnostics::ImportsCannotReferTo {
                        span: ident.span,
                        what,
                    });
                }
    ...
    fn resolve_item(&mut self, item: &'ast Item) {
        ...
        match &item.kind {
            ...
            ItemKind::Fn(fn_item) => {
                let Fn { generics, define_opaque, .. } = &**fn_item;
                self.with_generic_param_rib(
                    &generics.params,
                    RibKind::Item(HasGenericParams::Yes(generics.span), def_kind),
                    item.id,
            ...
            ItemKind::Trait(trait_) => {
                let Trait { generics, bounds, items, impl_restriction, .. } = &**trait_;
            ...
            ItemKind::TraitAlias(trait_alias) => {
                let TraitAlias { generics, bounds, .. } = &**trait_alias;
                // Create a new rib for the trait-wide type parameters.
                self.with_generic_param_rib(
                    &generics.params,
                    RibKind::Item(HasGenericParams::Yes(generics.span), def_kind),
            ...
            ItemKind::Static(static_item) => {
                let ast::StaticItem { ident, ty, expr, define_opaque, eii_impl, .. } =
                    &**static_item;
            ...
            for param in params {
                ...
                if param.ident.name == kw::UnderscoreLifetime {
                    ...
                    let guar = self
                        .r
                        .dcx()
                        .create_err(crate::rustc_resolve::diagnostics::UnderscoreLifetimeIsReserved {
                ...
                if param.ident.name == kw::StaticLifetime {
                    let guar =
                        self.r.dcx().emit_err(crate::rustc_resolve::diagnostics::StaticLifetimeIsReserved {
                            span: param.ident.span,
    ...
    fn resolve_trait_item(&mut self, item: &'ast Item<AssocItemKind>) {
        ...
        match &item.kind {
            AssocItemKind::Const(const_item) => {
                let ast::ConstItem { generics, ty, body, kind, define_opaque, .. } = &**const_item;
                self.with_generic_param_rib(
                    &generics.params,
                    RibKind::AssocItem,
                    item.id,
    ...
    fn resolve_impl_item(
        ...
        match &item.kind {
            ...
            AssocItemKind::Fn(fn_item) => {
                let fn_kind: &'ast Fn = &**fn_item;
                let Fn { ident, generics, define_opaque, .. } = fn_kind;
            ...
            AssocItemKind::Type(ty_alias) => {
                let TyAlias { ident, generics, .. } = &**ty_alias;
    ...
    fn resolve_expr(&mut self, expr: &'ast Expr, parent: Option<&'ast Expr>) {
        ...
        match expr.kind {
            ...
            ExprKind::ForLoop(ref for_loop) => {
                let ForLoop { ref pat, ref iter, ref body, label, kind: _ } = **for_loop;
            ...
            ExprKind::MethodCall(ref method_call) => {
                let MethodCall { ref seg, ref receiver, ref args, .. } = **method_call;
            ...
            ExprKind::Closure(ref closure)
                if matches!(closure.binder, ClosureBinder::For { .. }) =>
            {
                let ClosureBinder::For { ref generic_params, span } = closure.binder else {
                    unreachable!()
                };
                self.with_generic_param_rib(
                    generic_params,
...
impl<'ast> Visitor<'ast> for ItemInfoCollector<'_, 'ast, '_, '_> {
    ...
    fn visit_item(&mut self, item: &'ast Item) {
        ...
        match &item.kind {
            ItemKind::Fn(fn_item) => {
                self.collect_fn_info(&fn_item.sig.decl, item.id);
            }

            ItemKind::ForeignMod(ForeignMod { items, .. }) => {
                for foreign_item in items {
                    if let ForeignItemKind::Fn(fn_item) = &foreign_item.kind {
                        self.collect_fn_info(&fn_item.sig.decl, foreign_item.id);
                    }
                }
            }
    ...
    fn visit_assoc_item(&mut self, item: &'ast AssocItem, ctxt: AssocCtxt) {
        ...
        if let AssocItemKind::Type(ty_alias) = &item.kind {
            let def_id = self.r.owner_def_id(item.id);
            if let Some(suggestion) = required_generic_args_suggestion(&ty_alias.generics) {
