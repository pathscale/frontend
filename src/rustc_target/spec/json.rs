...
use crate::rustc_abi::{Align, AlignFromBytesError};
...
use crate::rustc_target::spec::{AbiMap, LlvmAbi};
...
impl ToJson for Target {
    fn to_json(&self) -> Json {
        ...
        macro_rules! target_option_val {
            ...
            ($attr:ident, $json_name:expr) => {{
                let name = $json_name;                if default.$attr != target.$attr {
                    d.insert(name.to_string(), target.$attr.to_json());
                }
            }};
...
struct EndianWrapper(crate::rustc_abi::Endian);
impl FromStr for EndianWrapper {
    type Err = String;
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        crate::rustc_abi::Endian::from_str(s).map(Self)
    }
}
crate::rustc_target::json::serde_deserialize_from_str!(EndianWrapper);

/// `ExternAbi` is in `rustc_abi`, which doesn't have access to the macro and serde.
struct ExternAbiWrapper(crate::rustc_abi::ExternAbi);
impl FromStr for ExternAbiWrapper {
    type Err = String;
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        crate::rustc_abi::ExternAbi::from_str(s)
            .map(Self)
            .map_err(|_| format!("{s} is not a valid extern ABI"))
    }
}
crate::rustc_target::json::serde_deserialize_from_str!(ExternAbiWrapper);
#[derive(serde_derive::Deserialize)]
