...
use crate::bug;
use crate::rustc_middle::dep_graph::{DepKindVTable, DepNodeKey, KeyFingerprintStyle};
use crate::rustc_middle::query::QueryCache;

use crate::rustc_query_impl::incremental::promote_from_disk_inner;
use crate::rustc_query_impl::query_vtables::GetQueryVTable;
    ...
    DepKindVTable {
        ...
        force_from_dep_node_fn: (can_recover && !is_no_force).then_some(
            |tcx, dep_node, _prev_index| {
                ...
                crate::rustc_query_impl::execution::force_query_dep_node(tcx, query, dep_node)
            },
        ),
...
macro_rules! define_dep_kind_vtables {
    (
        ...
        let q_vtables: [DepKindVTable<'tcx>; _] = [
            $(
                $crate::rustc_query_impl::dep_kind_vtables::make_dep_kind_vtable_for_query::<
                    $crate::rustc_query_impl::query_vtables::$name::VTableGetter,
                >(
                    $cache_on_disk,
                    $eval_always,
                    $no_force,
...
pub(crate) fn make_dep_kind_vtables<'tcx>(arena: &'tcx Arena<'tcx>) -> &'tcx [DepKindVTable<'tcx>] {
    let (nq_vtables, q_vtables) =
        crate::rustc_middle::queries::rustc_with_all_queries! { define_dep_kind_vtables! };
