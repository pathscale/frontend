...
use crate::rustc_errors::{Applicability, Diag, MultiSpan, pluralize, struct_span_code_err};
use crate::rustc_hir as hir;
use crate::rustc_hir::def::{DefKind, Res};
use crate::bug;
use crate::rustc_middle::queries::TaggedQueryKey;
use crate::rustc_middle::query::QueryCycle;
...
pub(crate) fn fn_sig<'tcx>(
    ...
    ty::EarlyBinder::bind(
        tcx,
        ty::Binder::dummy(tcx.mk_fn_sig_safe_rust_abi(core::iter::repeat_n(err, arity), err)),
    )
}
...
pub(crate) fn create_cycle_error<'tcx>(
    ...
    use crate::rustc_query_impl::diagnostics::StackCount;
    ...
    for i in 1..frames.len() {
        ...
        cycle_stack.push(crate::rustc_query_impl::diagnostics::CycleStack {
    ...
    let cycle_usage = usage.as_ref().map(|usage| {
        ...
        crate::rustc_query_impl::diagnostics::CycleUsage {
    ...
    let alias = if !nested {
        if is_all_def_kind(DefKind::TyAlias) {
            Some(crate::rustc_query_impl::diagnostics::Alias::Ty)
        } else if is_all_def_kind(DefKind::TraitAlias) {
            Some(crate::rustc_query_impl::diagnostics::Alias::Trait)
        } else {
            None
        }
    } else {
    ...
    if nested {
        tcx.sess.dcx().create_err(crate::rustc_query_impl::diagnostics::NestedCycle {
            span,
            cycle_stack,
            stack_bottom: crate::rustc_query_impl::diagnostics::NestedCycleBottom { stack_bottom },
            cycle_usage,
            stack_count,
            note_span: (),
        })
    } else {
        tcx.sess.dcx().create_err(crate::rustc_query_impl::diagnostics::Cycle {
            span,
            cycle_stack,
            stack_bottom,
            alias,
