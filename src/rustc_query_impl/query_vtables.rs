...
use crate::rustc_middle::query::{QueryCache, QueryKey, QueryMode, QueryVTable};
...
/// on the type `crate::rustc_query_impl::query_vtables::$name::VTableGetter`.
...
macro_rules! define_queries {
    (
        ...
        // This macro expects to be expanded into `crate::rustc_query_impl::query_vtables`, which is this file.
        $(
            pub(crate) mod $name {
                use super::*;
                ...
                pub(crate) mod execute_query_incr {
                    use super::*;
                    use crate::rustc_middle::queries::$name::{Key, Value};
                    ...
                    pub(crate) fn __rust_end_short_backtrace<'tcx>(
                        ...
                        crate::rustc_query_impl::execution::execute_query_incr_inner(
                ...
                pub(crate) mod execute_query_non_incr {
                    ...
                    pub(crate) fn __rust_end_short_backtrace<'tcx>(
                        ...
                        Some(crate::rustc_query_impl::execution::execute_query_non_incr_inner(
                            &tcx.query_system.query_vtables.$name,
                            tcx,
                            span,
                            key,
                ...
                mod invoke_provider_fn {
                    use super::*;
                    use crate::rustc_middle::queries::$name::{Key, Value, provided_to_erased};
                ...
                pub(crate) fn make_query_vtable<'tcx>(incremental: bool)
                    -> QueryVTable<'tcx, crate::rustc_middle::queries::$name::Cache<'tcx>>
                {
                    use crate::rustc_middle::queries::$name::Value;
                    QueryVTable {
                        name: stringify!($name),
                        eval_always: $eval_always,
                        depth_limit: $depth_limit,
                        feedable: $feedable,
                        dep_kind: crate::rustc_middle::dep_graph::DepKind::$name,
                        ...
                        try_load_from_disk_fn: |tcx, prev_index| {
                            use crate::rustc_middle::queries::$name::{ProvidedValue, provided_to_erased};

                            let loaded_value: ProvidedValue<'tcx> =
                                $crate::rustc_query_impl::incremental::try_load_from_disk(tcx, prev_index)?;
                        ...
                        handle_cycle_error_fn: |tcx, key, cycle, err| {
                            use crate::rustc_middle::query::erase::erase_val;

                            erase_val($crate::rustc_query_impl::handle_cycle_error::$name(tcx, key, cycle, err))
                        },
                        #[cfg(not($handle_cycle_error))]
                        handle_cycle_error_fn: |_tcx, _key, _cycle, err| {
                            $crate::rustc_query_impl::handle_cycle_error::default(err)
                        },
                        ...
                        hash_value_fn: Some(|hcx, erased_value: &erase::Erased<Value<'tcx>>| {
                            ...
                            crate::rustc_middle::dep_graph::hash_result(hcx, &value)
                        }),
                        ...
                        execute_query_fn: if incremental {
                            crate::rustc_query_impl::query_vtables::$name::execute_query_incr::__rust_end_short_backtrace
                        } else {
                            crate::rustc_query_impl::query_vtables::$name::execute_query_non_incr::__rust_end_short_backtrace
                        },
                    }
                }
                ...
                impl<'tcx> GetQueryVTable<'tcx> for VTableGetter {
                    type Cache = crate::rustc_middle::queries::$name::Cache<'tcx>;
        ...
        pub(crate) fn make_query_vtables<'tcx>(incremental: bool)
            -> crate::rustc_middle::queries::QueryVTables<'tcx>
        {
            crate::rustc_middle::queries::QueryVTables {
                $(
                    $name: crate::rustc_query_impl::query_vtables::$name::make_query_vtable(incremental),
                )*
            }
        }
        ...
        macro_rules! for_each_query_vtable {
            ...
            (ALL, $tcx:expr, $closure:expr) => {{
                let tcx: crate::rustc_middle::ty::TyCtxt<'_> = $tcx;
                $(
                    let query: &crate::rustc_middle::query::QueryVTable<'_, _> =
                        &tcx.query_system.query_vtables.$name;
                    $closure(query);
                )*
            }};
...
crate::rustc_middle::queries::rustc_with_all_queries! { define_queries! }
