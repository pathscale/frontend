...
macro_rules! try_visit {
    ($e:expr) => {
        match $crate::rustc_ast_ir::visit::VisitorResult::branch($e) {
            core::ops::ControlFlow::Continue(()) => (),
            #[allow(unreachable_code)]
            core::ops::ControlFlow::Break(r) => {
                return $crate::rustc_ast_ir::visit::VisitorResult::from_residual(r);
            }
        }
    };
}
...
macro_rules! walk_visitable_list {
    ($visitor: expr, $list: expr $(, $($extra_args: expr),* )?) => {
        for elem in $list {
            $crate::try_visit!($crate::rustc_type_ir::TypeVisitable::visit_with(elem, $visitor $(, $($extra_args,)* )?));
        }
    }
}
