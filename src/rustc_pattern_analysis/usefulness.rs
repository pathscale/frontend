...
//!
//! Constructors and relevant operations are defined in the [`crate::rustc_pattern_analysis::constructor`] module. A
//! representation of patterns that uses constructors is available in [`crate::rustc_pattern_analysis::pat`]. The question
...
//! This is done in [`ConstructorSet::split`] and explained in [`crate::rustc_pattern_analysis::constructor`].
//!
//!
//!
...
//! [`crate::rustc_pattern_analysis::constructor`], `Missing` represents the constructors not present in a given column. For
...
// `rustc_hash`'s `FxHashMap`/`FxHashSet` alias `std::collections::HashMap`, which is the one
// std-gated thing in that crate. These are the same table and the same hasher, off `hashbrown`.
...
use crate::rustc_pattern_analysis::constructor::{Constructor, ConstructorSet, IntRange};
use crate::rustc_pattern_analysis::pat::{DeconstructedPat, PatId, PatOrWild, WitnessPat};
use crate::rustc_pattern_analysis::{MatchArm, PatCx, PrivateUninhabitedField, checks};
