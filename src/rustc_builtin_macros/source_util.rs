...
use crate::rustc_lint_defs::builtin::INCOMPLETE_INCLUDE;
use crate::rustc_parse::lexer::StripTokens;
use crate::rustc_parse::parser::{AllowConstBlockItems, ForceCollect};
...
use crate::rustc_builtin_macros::util::{
        ...
        Err(io_err) => {
            ...
            if original_path.is_relative() {
                ...
                let new_path = source_map
                    ...
                    .and_then(|path| String::from_utf8(path.into_os_string()).ok());
...
fn find_path_suggestion(
    ...
    let remove = core::iter::from_fn(|| {
        ...
        Some([
            ...
            (removed != eko::path::Component::ParentDir)
