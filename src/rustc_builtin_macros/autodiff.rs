...
mod llvm_enzyme {
    ...
    use crate::rustc_attr_ir::RustcAutodiff;
    ...
    // ```
    // core::intrinsics::autodiff(source as fn(..) -> .., diff, (args))
    // ```
    fn call_autodiff(
        ecx: &ExtCtxt<'_>,
        primal: Ident,
    ...
    fn gen_enzyme_decl(
        ...
        if x.mode.is_fwd() {
            let ty = match d_decl.output {
                FnRetTy::Ty(ref ty) => ty.clone(),
                FnRetTy::Default(span) => {
                    // We want to return core::hint::black_box(()).
                    let kind = TyKind::Tup(ThinVec::new());
                    let ty = Box::new(crate::rustc_ast::Ty { kind, id: ast::DUMMY_NODE_ID, span });
            ...
            if matches!(x.ret_activity, DiffActivity::Dual | DiffActivity::Dualv) {
                ...
                let ty = Box::new(crate::rustc_ast::Ty { kind, id: ty.id, span: ty.span });
        ...
        if act_ret.len() > 0 {
            let ret_ty = match d_decl.output {
                FnRetTy::Ty(ref ty) => {
                    ...
                    Box::new(crate::rustc_ast::Ty { kind, id: ty.id, span: ty.span })
                }
                FnRetTy::Default(span) => {
                    if act_ret.len() == 1 {
                        act_ret[0].clone()
                        ...
                        let kind = TyKind::Tup(act_ret);
                        Box::new(crate::rustc_ast::Ty { kind, id: ast::DUMMY_NODE_ID, span })
                    }
                }
            };
