...
use crate::rustc_ast::attr::contains_name;
...
use crate::rustc_lint_defs::builtin::UNNAMEABLE_TEST_ITEMS;
...
fn entry_point_type(item: &ast::Item, at_root: bool) -> EntryPointType {
    match &item.kind {
        ast::ItemKind::Fn(fn_) => crate::rustc_ast::entry::entry_point_type(
...
fn get_test_runner(sess: &Session, krate: &ast::Crate) -> Option<ast::Path> {
    match AttributeParser::parse_limited_sym(sess, &krate.attrs, &[sym::test_runner]) {
        Some(crate::rustc_attr_ir::Attribute::Parsed(AttributeKind::TestRunner(path))) => Some(path),
        _ => None,
    }
}
