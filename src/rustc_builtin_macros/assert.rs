...
use crate::rustc_builtin_macros::edition_panic::use_panic_2021;
...
pub(crate) fn expand_assert<'cx>(
    ...
    // `core::panic` and `core::panic` are different macros, so we use call-site
    // context to pick up whichever is currently in scope.
    let call_site_span = cx.with_call_site_ctxt(span);

    let panic_path = || {
        if use_panic_2021(span) {
            // On edition 2021, we always call `$crate::rustc_builtin_macros::panic::panic_2021!()`.
            Path {
                span: call_site_span,
                segments: cx
            ...
            // such that it calls either `core::panic!()` or `core::panic!()`.
    ...
    // string
        ...
        // Pass our own message directly to $crate::rustc_builtin_macros::panicking::panic(),
