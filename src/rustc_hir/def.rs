...
// `crate::rustc_hir`, not `crate`. Upstream this file is in the `rustc_hir` crate and `crate` was
// that crate, so `hir::Safety` named its root. Here `crate` is `frontend` and `rustc_hir` is a
// module of it, so the alias has to move down one level or `hir::` resolves into the crate root
// and finds nothing.
...
pub enum LifetimeRes {
    ...
    Static,
    /// Resolution failure.
    Error(crate::rustc_span::ErrorGuaranteed),
