...
use crate::rustc_ast::token as tk;
...
use crate::rustc_errors::{Diag, ErrorGuaranteed, MultiSpan};
use crate::rustc_parse::lexer::{StripTokens, nfc_normalize};
use crate::rustc_parse::parser::Parser;
use crate::rustc_parse::{exp, new_parser_from_source_str, source_str_to_stream};
...
impl server::Server for Rustc<'_, '_> {
    ...
    fn literal_from_str(&mut self, s: &str) -> Result<Literal<Self::Span, Self::Symbol>, String> {
        ...
        let mut parser = crate::rustc_errors::catch_fatal_errors(|| {
    ...
    fn ts_from_str(&mut self, src: &str) -> Result<Self::TokenStream, String> {
        crate::rustc_errors::catch_fatal_errors(|| {
    ...
    fn ts_expand_expr(&mut self, stream: &Self::TokenStream) -> Result<Self::TokenStream, ()> {
        ...
        let expr = self
            .ecx
            .expander()
            .fully_expand_fragment(crate::rustc_expand::expand::AstFragment::Expr(expr))
            .make_expr();
    ...
    fn symbol_normalize_and_validate_ident(&mut self, string: &str) -> Result<Self::Symbol, ()> {
        ...
        if crate::rustc_lexer::is_ident(sym.as_str()) { Ok(sym) } else { Err(()) }
    }
}
