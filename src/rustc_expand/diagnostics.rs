...
#[note(
    "if there is a `mod {$name}` elsewhere in the crate already, import it with `use crate::rustc_expand::...` instead"
)]
    ...
    impl InvalidIdentReason {
        pub(crate) fn new(symbol: Symbol) -> Self {
            ...
            if let Some(start) = chars.next() {
                if crate::rustc_lexer::is_id_start(start) {
                    let not_continue = chars
                        .find(|c| !crate::rustc_lexer::is_id_continue(*c))
