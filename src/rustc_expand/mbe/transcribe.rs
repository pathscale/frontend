...
use crate::rustc_errors::{Diag, DiagCtxtHandle, PResult, listify, pluralize};
...
use crate::rustc_expand::mbe::macro_parser::NamedMatch;
use crate::rustc_expand::mbe::macro_parser::NamedMatch::*;
use crate::rustc_expand::mbe::metavar_expr::{MetaVarExprConcatElem, RAW_IDENT_ERR};
use crate::rustc_expand::mbe::{self, KleeneOp, MetaVarExpr};
    ...
    match lockstep_iter_size(seq, tscx.interp, &tscx.repeats) {
        LockstepIterSize::Unconstrained => {
            ...
            for ident in meta_vars {
                if let Some(name) = crate::rustc_span::edit_distance::find_best_match_for_name(
                    &repeatable_names[..],
                    ident.name,
                    None,
                ) {
                    typo_repeatable = Some(VarTypoSuggestionRepeatable { span: ident.span, name });
                } else if let Some(name) = crate::rustc_span::edit_distance::find_best_match_for_name(
...
fn transcribe_pnr<'tx>(
    ...
    let tt = match pnr {
        ...
        ParseNtResult::Guard(guard) => {
            ...
            let ts = core::iter::once(TokenTree::token_alone(
...
fn metavar_expr_concat<'tx>(
    ...
    if !crate::rustc_lexer::is_ident(symbol.as_str()) {
