...
use crate::rustc_ast::{self as ast, AttrStyle, Attribute, HasAttrs, HasTokens, NodeId, SyntheticAttr};
use crate::rustc_attr_ir::target::Target;
use crate::rustc_attr_ir::{self as attrs, AttributeKind};
...
impl<'a> StripUnconfigured<'a> {
    ...
    pub(crate) fn expand_cfg_attr(&self, cfg_attr: &Attribute, recursive: bool) -> Vec<Attribute> {
        let Some((cfg_predicate, expanded_attrs)) = crate::rustc_attr_parsing::parse_cfg_attr(
            cfg_attr,
            self.sess,
            self.features,
            self.lint_node_id,
        ...
        if expanded_attrs.is_empty() {
            self.sess.psess.buffer_lint(
                ...
                cfg_attr.span,
                ast::CRATE_NODE_ID,
                crate::rustc_expand::diagnostics::CfgAttrNoAttributes,
            );
        }
