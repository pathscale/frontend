...
use crate::rustc_ast::visit::{AssocCtxt, Visitor, VisitorResult, try_visit, walk_list};
use crate::rustc_ast::{
...
use crate::rustc_attr_parsing::parser::AllowExprMetavar;
...
use crate::rustc_lint_defs::builtin::{UNUSED_ATTRIBUTES, UNUSED_DOC_COMMENTS};
...
use crate::rustc_expand::mbe::diagnostics::annotate_err_with_kind;
use crate::rustc_expand::module::{
    DirOwnership, ParsedExternalMod, mod_dir_path, mod_file_path_from_attr, parse_external_mod,
};
use crate::rustc_expand::placeholders::{PlaceholderExpander, placeholder};
use crate::rustc_expand::stats::*;
...
macro_rules! ast_fragments {
    (
            ...
            fn add_placeholders(&mut self, placeholders: &[NodeId]) {
                ...
                match self {
                    $($(AstFragment::$Kind(ast) => ast.extend(placeholders.iter().flat_map(|id| {
                        // Drives the `$(...)?` without emitting it; see `metavar_ignore!`.
                        $crate::metavar_ignore!([$flat_map_ast_elt]);
        ...
        impl<'a, 'b> MacResult for crate::rustc_expand::mbe::macro_rules::ParserAnyMacro<'a, 'b> {
            $(fn $make_ast(self: Box<crate::rustc_expand::mbe::macro_rules::ParserAnyMacro<'a, 'b>>)
                           -> Option<$AstTy> {
            ...
            InvocationKind::Attr { attr, pos, mut item, derives } => {
                if let Some(expander) = ext.as_attr() {
                    ...
                    let tokens = match &item {
                        ...
                        {
                            crate::rustc_parse::fake_token_stream_for_item(
                                &self.cx.sess.psess,
                                item_inner,
                                Some(&attr),
                            )
                        ...
                        Annotatable::ForeignItem(item_inner) if item_inner.tokens.is_none() => {
                            crate::rustc_parse::fake_token_stream_for_foreign_item(
                                &self.cx.sess.psess,
                                item_inner,
                            )
                        }
...
trait InvocationCollectorNode: HasAttrs + HasNodeId + Sized + DeclaredIdents {
    // Stable Rust has no associated type defaults, so every impl names these two types. The
    // usual choices are `SmallVec<[Self; 1]>` and `ItemKind`.
    type OutputTy;
    type ItemKind;
    ...
    fn as_target(&self) -> Target {
        ...
        Target::GenericParam {
            kind: match &self.kind {
                crate::rustc_ast::GenericParamKind::Lifetime => {
                    crate::rustc_attr_ir::target::GenericParamKind::Lifetime
                }
                crate::rustc_ast::GenericParamKind::Type { default } => {
                    has_default = default.is_some();
                    crate::rustc_attr_ir::target::GenericParamKind::Type
                }
                crate::rustc_ast::GenericParamKind::Const { default, .. } => {
                    has_default = default.is_some();
                    crate::rustc_attr_ir::target::GenericParamKind::Const
                }
            },
            has_default,
        }
        ...
        while let Some(attr) = attrs.next() {
            ...
            if match &attr.kind {
                ...
                self.cx.sess.psess.buffer_lint(
                    ...
                    current_span,
                    self.cx.current_expansion.lint_node_id,
                    crate::rustc_expand::diagnostics::MacroCallUnusedDocComment { span: attr.span },
                );
                continue;
            }
            ...
            match &attr.kind {
                AttrKind::Normal(normal)
                    if crate::rustc_attr_parsing::is_builtin_attr(&normal.item)
                        && !AttributeParser::is_parsed_attribute(&attr.path()) =>
                {
                    ...
                    self.cx.sess.psess.buffer_lint(
                        ...
                        attr.span,
                        self.cx.current_expansion.lint_node_id,
                        crate::rustc_expand::diagnostics::UnusedBuiltinAttribute {
                            attr_name,
