...
impl CurrentDepGraph {
    fn new(
        ...
        #[cfg(debug_assertions)]
        // `eko::env::var` returns `Option<String>`, not `Result`: an unset variable and
        // a variable holding non-UTF-8 are the same `None` here, where `std` distinguished them.
        // Both arms already meant "no filter", so the behaviour at this site is unchanged.
