...
//! during compiler initialization (see [`crate::rustc_interface::passes::DEFAULT_QUERY_PROVIDERS`]).
//!
//! [`rustc_macros`]: https://doc.rust-lang.org/nightly/nightly-rustc/rustc_macros/index.html
//! [`crate::rustc_interface::passes::DEFAULT_QUERY_PROVIDERS`]: ../../rustc_interface/passes/static.DEFAULT_QUERY_PROVIDERS.html
//!
//! ## Query Modifiers
//!
//! Query modifiers are special flags that alter the behavior of a query. They are parsed and processed by the `rustc_macros`
//!
//! For the list of modifiers, see [`crate::rustc_middle::query::modifiers`].
//!
...
//! - A method on [`TyCtxt`] (and [`crate::rustc_middle::query::TyCtxtAt`]) for invoking the query.
...
use crate::rustc_attr_ir::lang_items::{LangItem, LanguageItems};
use crate::rustc_attr_ir::{CanonicalSymbols, EiiDecl, EiiImpl, StrippedCfgItem};
...
use crate::rustc_errors::{ErrorGuaranteed, catch_fatal_errors};
use crate::rustc_hir as hir;
use crate::rustc_hir::def::{DefKind, DocLinkResMap};
use crate::rustc_hir::def_id::{CrateNum, DefId, DefIdMap, LocalDefId, LocalDefIdSet, LocalModId};
use crate::rustc_hir::{ItemLocalId, PreciseCapturingArgKind};
use crate::rustc_index::IndexVec;
use crate::rustc_lint_defs::{LintId, StableLintExpectationId};
...
use crate::rustc_span::def_id::{LOCAL_CRATE, ModId};
use crate::rustc_span::{DUMMY_SP, LocalExpnId, Span, Spanned, Symbol};
use crate::rustc_target::spec::PanicStrategy;

use crate::rustc_middle::infer::canonical::{self, Canonical};
use crate::rustc_middle::lint::LintExpectation;
use crate::rustc_middle::metadata::ModChild;
use crate::rustc_middle::middle::codegen_fn_attrs::{CodegenFnAttrs, SanitizerFnAttrs};
use crate::rustc_middle::middle::dead_code::DeadCodeLivenessSummary;
...
use crate::rustc_middle::middle::resolve_bound_vars::{ObjectLifetimeDefault, ResolveBoundVars, ResolvedArg};
...
use crate::rustc_middle::query::query_api::{define_query_api, maybe_into_query_key};
use crate::rustc_middle::traits::query::{
...
use crate::rustc_middle::ty::util::AlwaysRequiresDrop;
...
rustc_queries! {
    ...
    /// `crate::rustc_interface::passes::write_dep_info` to make that work.
    // An OS string is `[u8]` without `std`; the provider in `crate::rustc_interface::passes` already
    // takes and returns `&'tcx [u8]`, and `QueryKey` is implemented for it in `query/keys.rs`.
    query env_var_os(key: &'tcx [u8]) -> Option<&'tcx [u8]> {
    ...
    query hir_owner(def_id: LocalDefId) -> crate::rustc_middle::hir::ProjectedMaybeOwner<'tcx> {
    ...
    query hir_module_items(key: LocalModId) -> &'tcx crate::rustc_middle::hir::ModuleItems {
        arena_cache
    ...
    query check_liveness(key: LocalDefId) -> &'tcx crate::rustc_index::bit_set::DenseBitSet<abi::FieldIdx> {
        arena_cache
    ...
    query region_scope_tree(def_id: LocalDefId) -> &'tcx crate::rustc_middle::middle::region::ScopeTree {
    ...
    /// <div class="warning">
    ///
    /// Do not use this directly, use [`crate::find_attr`] instead.
    ///
    /// </div>
    query attrs_for_def(def_id: DefId) -> &'tcx [crate::rustc_attr_ir::Attribute] {
    ...
    query dependency_formats(_: ()) -> &'tcx Arc<crate::rustc_middle::middle::dependency_format::Dependencies> {
        arena_cache
    ...
    /// See `crate::rustc_resolve::late::lifetimes` for details.
    ...
    query all_diagnostic_items(_: ()) -> &'tcx crate::rustc_hir::attrs::diagnostic_items::DiagnosticItems {
        arena_cache
        eval_always
    ...
    query diagnostic_items(_: CrateNum) -> &'tcx crate::rustc_hir::attrs::diagnostic_items::DiagnosticItems {
        arena_cache
    ...
    query all_rust_target_features(_: CrateNum) -> &'tcx UnordMap<String, crate::rustc_target::target_features::Stability> {
        arena_cache
        eval_always
