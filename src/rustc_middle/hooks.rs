...
use crate::rustc_middle::query::on_disk_cache::CacheEncoder;
...
declare_hooks! {
    ...
    hook should_codegen_locally(instance: crate::rustc_middle::ty::Instance<'tcx>) -> bool;

    // `alloc_self_profile_query_strings` was declared here and provided by
    // `crate::rustc_query_impl::self_profile`. It went with `measureme`. A hook has no default body -
    // `default_hook` bugs out - so leaving it declared and unprovided would have turned
    // `TyCtxt::finish` into an ICE. Both ends are removed instead.
    ...
    hook validate_scalar_in_layout(scalar: crate::rustc_middle::ty::ScalarInt, ty: Ty<'tcx>) -> bool;
