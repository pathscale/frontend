...
fn incremental_verify_ich_failed<'tcx>(
    ...
    // `std::thread_local!` goes with `std`. `eko::thread::ThreadLocal` is the same
    // thing over `pthread_key_create`; its `with` hands out `&mut T`, so the `Cell` that used
    // to supply interior mutability is no longer needed.
    //
    // Behaviour change: `with` answers `None` when the key could not be created, which is a
    // process out of thread-local slots. That is read here as "not already inside" - the same
    // answer the old thread-local gave on a thread that had never entered this function - so a
    // failure to allocate the slot costs the terse reentrant message, not the diagnostic.
    ...
    // `with` is `#[must_use]` because it can answer `None`; nothing to do if the slot is gone.
