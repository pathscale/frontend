...
// `#![no_std]`: `std::io::Write` becomes `core::fmt::Write`, the only writing trait `core` has.
// Every error these functions can produce comes from a `write!`/`writeln!` on that trait, so the
// error type follows it: the return types below change from `io::Result<()>` to `fmt::Result`.
// `eko::file::File` implements `core::fmt::Write`, so writing a DOT file still works.
...
> GraphvizWriter<'a, G, NodeContentFn, EdgeLabelsFn>
{
    ...
    pub fn write_graphviz<'tcx, W>(&self, tcx: TyCtxt<'tcx>, w: &mut W) -> fmt::Result
    where
        W: Write,
    {
    ...
    pub fn write_node<W>(&self, node: G::Node, dark_mode: bool, w: &mut W) -> fmt::Result
    where
        W: Write,
    {
    ...
    fn write_edges<W>(&self, source: G::Node, w: &mut W) -> fmt::Result
    where
        W: Write,
    {
    ...
    fn write_graph_label<W>(&self, label: &str, w: &mut W) -> fmt::Result
    where
        W: Write,
    {
