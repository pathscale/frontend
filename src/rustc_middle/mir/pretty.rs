...
use crate::rustc_middle::ty::CoroutineArgsExt;
...
impl<'a, 'tcx> MirDumper<'a, 'tcx> {
    ...
    pub fn set_extra_data(
        mut self,
        extra_data: &'a dyn Fn(PassWhere, &mut dyn fmt::Write) -> fmt::Result,
    ) -> Self {
    ...
    /// Render this body and hand the text to the session. Writes nothing anywhere.
    ///
    /// The caller asked for a dump by turning one of the `-Zdump-mir` options on; what happens
    /// to the text after that is the driver's decision, not this function's.
    pub fn dump_mir(&self, body: &Body<'tcx>) {
        let mut text = String::new();
        if self.dump_mir_to_writer(body, &mut text).is_ok() {
            self.tcx().sess.mir_dumps.lock().push((self.dump_name("mir", body), text));
        }

        // `-Zdump-mir-graphviz` rendered the same body as a .dot graph through `gsgdt`, a
        // third-party dot library whose writer is `io::Write` - so a debug view of MIR pulled
        // an external crate and a filesystem-shaped interface into rustc_middle. The text dump
        // above says the same thing, and this compiler is not in the business of drawing.
    }

    /// The name this dump would have had as a file, used as the artifact's name.
    pub fn dump_name(&self, extension: &str, body: &Body<'tcx>) -> String {
        self.dump_path(extension, body).display().to_string()
    }
    ...
    pub fn dump_mir_to_writer(&self, body: &Body<'tcx>, w: &mut dyn fmt::Write) -> fmt::Result {
    ...
    /// Render arbitrary dump text and hand it to the session under this extension.
    ///
    /// Replaces `create_dump_file`, which returned an open file handle and so let every caller
    /// write to disk from inside a pass. Callers now build a `String` and give it a name.
    pub fn record_dump(&self, extension: &str, body: &Body<'tcx>, text: String) {
        self.tcx().sess.mir_dumps.lock().push((self.dump_name(extension, body), text));
    }
}
...
pub fn write_mir_pretty<'tcx>(tcx: TyCtxt<'tcx>, w: &mut dyn fmt::Write) -> fmt::Result {
    ...
    for &def_id in tcx.mir_keys(()) {
        ...
        let render_body = |w: &mut dyn fmt::Write, body| -> fmt::Result {
...
impl<'a, 'tcx> MirWriter<'a, 'tcx> {
    ...
    pub fn write_mir_fn(&self, body: &Body<'tcx>, w: &mut dyn fmt::Write) -> fmt::Result {
...
fn write_mir_sig(tcx: TyCtxt<'_>, body: &Body<'_>, w: &mut dyn fmt::Write) -> fmt::Result {
...
pub fn write_allocations<'tcx>(
    ...
    while let Some(id) = todo.pop() {
        let mut write_allocation_track_relocs =
            |w: &mut dyn fmt::Write, alloc: ConstAllocation<'tcx>| -> fmt::Result {
...
fn pretty_print_const_value_tcx<'tcx>(
    ...
    use crate::rustc_middle::ty::print::PrettyPrinter;
