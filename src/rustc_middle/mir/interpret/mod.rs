...
// Not converted: `write_target_uint`/`read_target_uint` at the bottom of this file rely on
// These were built on `std::io::Write for &mut [u8]` and `Read for &[u8]`, the impls that advance
// slice as they go. `eko` has no equivalent trait and `core::fmt::Write` is a `&str`
// sink, so replacing these means writing the byte copy by hand. Left as one piece because
// converting the `io::Error` in their signatures alone would break the `?` on the calls.
...
pub fn write_target_uint(
    ...
    target: &mut [u8],
    data: u128,
) -> Result<(), TargetUintError> {
    // This u128 holds an "any-size uint" (since smaller uints can fit in it), so only the
    // payload is written, not all sixteen bytes.
    //
    // This used to lean on `std::io::Write for &mut [u8]`, whose `write` copies as much as fits
    // and reslices the destination past it - which is why the original took `mut target` and
    // then asserted the slice had been emptied. The copy is written out here instead: the
    // length is `target.len()` either way, and a short write is now impossible rather than
    // asserted against.
    let n = target.len();
    if n > 16 {
        return Err(TargetUintError);
    }
    match endianness {
        Endian::Little => target.copy_from_slice(&data.to_le_bytes()[..n]),
        Endian::Big => target.copy_from_slice(&data.to_be_bytes()[16 - n..]),
    }
    Ok(())
}

/// A target integer did not fit the buffer it was being read from or written to.
///
/// Replaces the `io::Error` these two used to return. There is no io here - both are a copy
/// between two slices - and the only failure either could ever report was a length mismatch.
#[derive(Copy, Clone, PartialEq, Eq, Debug)]
pub struct TargetUintError;

impl core::fmt::Display for TargetUintError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.write_str("target integer does not fit its buffer")
    }
}

impl core::error::Error for TargetUintError {}

#[inline]
pub fn read_target_uint(endianness: Endian, source: &[u8]) -> Result<u128, TargetUintError> {
    ...
    // Only the payload is read into the u128, not all sixteen bytes. As above, the slice
    // `Read` impl that advanced `source` is replaced by an explicit copy.
    let uint = match endianness {
        ...
        Endian::Big => {
            buf[16 - n..].copy_from_slice(source);
