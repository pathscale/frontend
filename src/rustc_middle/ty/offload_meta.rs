...
use crate::rustc_abi::{BackendRepr, TyAbiInterface};
use crate::rustc_target::callconv::ArgAbi;

use crate::rustc_middle::ty::{self, PseudoCanonicalInput, Ty, TyCtxt, TypingEnv};
...
impl MappingFlags {
    fn from_ty<'tcx>(tcx: TyCtxt<'tcx>, ty: Ty<'tcx>) -> Self {
        ...
        match ty.kind() {
            ...
            | ty::Error(_) => {
                tcx.dcx()
                    .span_err(crate::rustc_span::DUMMY_SP, format!("type `{ty:?}` cannot be offloaded"));
                MappingFlags::empty()
            }
        }
    }
