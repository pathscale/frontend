...
use crate::rustc_type_ir::WithCachedTypeInfo;
...
pub type GenericArgKind<'tcx> = crate::rustc_type_ir::GenericArgKind<TyCtxt<'tcx>>;
pub type TermKind<'tcx> = crate::rustc_type_ir::TermKind<TyCtxt<'tcx>>;
...
impl<'tcx> crate::rustc_type_ir::inherent::GenericArg<TyCtxt<'tcx>> for GenericArg<'tcx> {}

impl<'tcx> crate::rustc_type_ir::inherent::GenericArgs<TyCtxt<'tcx>> for ty::GenericArgsRef<'tcx> {
    fn rebase_onto(
        self,
        tcx: TyCtxt<'tcx>,
...
impl<'tcx> crate::rustc_type_ir::inherent::IntoKind for GenericArg<'tcx> {
...
impl<'tcx, T: TypeVisitable<TyCtxt<'tcx>>> TypeVisitable<TyCtxt<'tcx>> for &'tcx ty::List<T> {
    #[inline]
    fn visit_with<V: TypeVisitor<TyCtxt<'tcx>>>(&self, visitor: &mut V) -> V::Result {
        // `as_slice().iter()`: the inherent `RawList::iter` needs `T: Copy` and this impl is
        // generic over `T: TypeVisitable`. Upstream the slice iterator is reached by deref, which
        // has no such bound; that route is closed here. See `ty/list.rs`.
        walk_visitable_list!(visitor, self.as_slice().iter());
        V::Result::output()
    }
}
