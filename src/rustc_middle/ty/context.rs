...
use crate::rustc_crate_store::{CrateStoreDyn, Untracked};
...
use crate::rustc_hir::definitions::{DefPathData, Definitions, PerParentDisambiguatorState};
use crate::rustc_hir::intravisit::VisitorExt;
use crate::rustc_hir::{self as hir, CRATE_HIR_ID, HirId, Node, TraitCandidate, find_attr};
...
use crate::rustc_middle::dep_graph::dep_node::make_metadata;
use crate::rustc_middle::dep_graph::{DepGraph, DepNodeIndex};
use crate::rustc_middle::hir::{ProjectedMaybeOwner, ProjectedOwnerInfo};
use crate::rustc_middle::ich::StableHashState;
use crate::rustc_middle::infer::canonical::{CanonicalParamEnvCache, CanonicalVarKind};
use crate::rustc_middle::lint::emit_lint_base;
use crate::rustc_middle::metadata::ModChild;
use crate::rustc_middle::middle::codegen_fn_attrs::{CodegenFnAttrs, TargetFeature};
use crate::rustc_middle::middle::resolve_bound_vars;
use crate::rustc_middle::mir::interpret::{self, Allocation, ConstAllocation};
use crate::rustc_middle::mir::{Body, Local, Place, PlaceElem, ProjectionKind, Promoted};
...
impl<'tcx> crate::rustc_type_ir::inherent::DefId<TyCtxt<'tcx>> for DefId {
...
impl<'tcx> crate::rustc_type_ir::inherent::Safety<TyCtxt<'tcx>> for hir::Safety {
    fn safe() -> Self {
        hir::Safety::Safe
    }
...
impl<'tcx> crate::rustc_type_ir::inherent::Span<TyCtxt<'tcx>> for Span {
    fn dummy() -> Self {
        DUMMY_SP
    }
}
...
pub struct GlobalCtxt<'tcx> {
    ...
    pub(crate) hooks: crate::rustc_middle::hooks::Providers,
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    pub fn create_global_ctxt<T>(
        ...
        hooks: crate::rustc_middle::hooks::Providers,
        current_gcx: CurrentGcx,
    ...
    pub fn lang_items(self) -> &'tcx crate::rustc_hir::attrs::lang_items::LanguageItems {
        self.get_lang_items(())
    }
    ...
    pub fn def_key(self, id: impl IntoQueryKey<DefId>) -> crate::rustc_hir::definitions::DefKey {
    ...
    /// Helper to get a tracked environment variable via. [`TyCtxt::env_var_os`], decoded as UTF-8.
    ///
    /// **This returned `Result<&str, VarError>` and now returns `Option`.** `VarError` was the one
    /// thing distinguishing "unset" from "set but not UTF-8", and no caller here looked: the only
    /// one does `.unwrap_or(..)`, which reads the same on either type. The `env_var_os` query it
    /// wraps already hands back `&[u8]` rather than an `OsStr`, so the decode is a plain
    /// `from_utf8` and a non-UTF-8 value now collapses into `None` with the unset case.
    pub fn env_var<K: ?Sized + AsRef<[u8]>>(self, key: &'tcx K) -> Option<&'tcx str> {
        core::str::from_utf8(self.env_var_os(key.as_ref())?).ok()
    }
}
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    pub fn finish(self) {
        // We assume that no queries are run past here. `self.alloc_self_profile_query_strings()`
        // was called at this point to name the queries recorded so far, and anything after it
        // showed up as "<unknown>" in the profiling data. It went with `measureme`, so the
        // assumption is no longer load-bearing on anything here.
...
macro_rules! sty_debug_print {
    ($fmt: expr, $ctxt: expr, $($variant: ident),*) => {{
        ...
        mod inner {
            ...
            use crate::rustc_middle::ty::context::InternedInSet;
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    pub fn metadata_dep_node(self) -> crate::rustc_middle::dep_graph::DepNode {
        make_metadata(self)
    }
