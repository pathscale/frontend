...
use crate::rustc_middle::ty::normalize_erasing_regions::NormalizationError;
use crate::rustc_middle::ty::{self, CoroutineArgsExt, Ty, TyCtxt, TypeVisitableExt, Unnormalized};
...
impl<'tcx> SizeSkeleton<'tcx> {
    ...
    fn compute_inner(
        ...
        if depth >= recursion_limit.0 {
            ...
            let reported =
                tcx.dcx().emit_err(crate::rustc_middle::diagnostics::RecursionLimitReachedSizeSkeleton {
                    span,
                    ty,
                    suggested_limit,
                });
            ...
            match *this.ty.kind() {
                ...
                ty::Ref(_, pointee, _) | ty::RawPtr(pointee, _) => {
                        ...
                        && !pointee.references_error()
                    {
                        ...
                        // offers better information than `core::ptr::metadata::VTable`,
                        // and we rely on this layout information to trigger a panic in
                        // `core::mem::uninitialized::<&dyn Trait>()`, for example.
