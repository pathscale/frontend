...
impl<'tcx, P: Printer<'tcx> + core::fmt::Write> Print<P> for ty::Instance<'tcx> {
...
impl<'tcx, P: Printer<'tcx> + core::fmt::Write> Print<P> for ty::ShimKind<'tcx> {
    fn print(&self, cx: &mut P) -> Result<(), PrintError> {
        match self {
...
impl<T> crate::rustc_type_ir::ir_print::IrPrint<T> for TyCtxt<'_>
where
