...
use crate::rustc_crate_store::{ExternCrate, ExternCrateSource};
...
use crate::rustc_hir::def_id::{DefIdMap, DefIdSet, LOCAL_CRATE, ModId};
use crate::rustc_hir::definitions::{DefKey, DefPathDataName};
use rustc_macros::{Lift, extension};
use crate::rustc_span::{Ident, RemapPathScopeComponents, Symbol, kw, sym};
use crate::rustc_structures::Limit;
use crate::rustc_type_ir::{FieldInfo, Unnormalized, Upcast as _, elaborate};
use smallvec::SmallVec;

// `pretty` is a separate module only for organization.
use super::*;
use crate::rustc_middle::mir::interpret::{AllocRange, GlobalAlloc, Pointer, Provenance, Scalar};
...
define_helper!(
    ...
    /// using `crate::rustc_middle::` for local items.
    ///
    ...
    /// Adds the `crate::rustc_middle::` prefix to paths where appropriate.
    ///
    ...
    /// of various rustc types, for example `alloc::vec::Vec` would be trimmed to `Vec`,
    ...
    fn pretty_print_type(&mut self, ty: Ty<'tcx>) -> Result<(), PrintError> {
        match *ty.kind() {
            ...
            ty::Bound(debruijn, bound_ty) => match bound_ty.kind {
                ty::BoundTyKind::Anon => {
                    crate::rustc_type_ir::debug_bound_var(self, debruijn, bound_ty.var)?
                }
            ...
            ty::Alias(_, ty::AliasTy { kind: ty::Opaque { def_id }, args, .. }) => {
                ...
                // from `crate::rustc_middle::ty` during the computation of `tcx.clauses_of`),
    ...
    fn pretty_print_const(
        ...
        match ct.kind() {
            ...
            ty::ConstKind::Bound(debruijn, bound_var) => {
                crate::rustc_type_ir::debug_bound_var(self, debruijn, bound_var)?
            }
    ...
    fn pretty_print_const_expr(
        ...
        match expr.kind {
            ty::ExprKind::Binop(op) => {
                ...
                let precedence = |binop: crate::rustc_middle::mir::BinOp| binop.to_hir_binop().precedence();
    ...
    fn print_crate_name(&mut self, cnum: CrateNum) -> Result<(), PrintError> {
        ...
        if cnum == LOCAL_CRATE && !with_resolve_crate_name() {
            if self.tcx.sess.at_least_rust_2018() {
                // We add the `crate::rustc_middle::` keyword on Rust 2018, only when desired.
...
impl<'tcx> crate::rustc_errors::IntoDiagArg for TraitRefPrintOnlyTraitPath<'tcx> {
    fn into_diag_arg(self, path: &mut crate::rustc_errors::LongTyPath) -> crate::rustc_errors::DiagArgValue {
        ty::tls::with(|tcx| {
            let trait_ref = tcx.short_string(tcx.lift(self), path);
            crate::rustc_errors::DiagArgValue::Str(alloc::borrow::Cow::Owned(trait_ref))
        })
    }
}
...
/// `alloc::vec::Vec` to just `Vec`, as long as there is no other `Vec` importable anywhere.
///
