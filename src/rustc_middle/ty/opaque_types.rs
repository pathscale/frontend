...
use crate::rustc_middle::diagnostics::ConstNotUsedTraitAlias;
use crate::rustc_middle::ty::{
    self, GenericArg, GenericArgKind, RegionExt, Ty, TyCtxt, TypeFoldable, TypeFolder,
    TypeSuperFoldable,
};

pub type OpaqueTypeKey<'tcx> = crate::rustc_type_ir::OpaqueTypeKey<TyCtxt<'tcx>>;
...
impl<'tcx> TypeFolder<TyCtxt<'tcx>> for ReverseMapper<'tcx> {
    ...
    fn fold_region(&mut self, r: ty::Region<'tcx>) -> ty::Region<'tcx> {
        ...
        match self.map.get(&GenericArg::from(r)).map(|arg| arg.kind()) {
    ...
    fn fold_ty(&mut self, ty: Ty<'tcx>) -> Ty<'tcx> {
        match *ty.kind() {
            ...
            ty::Param(param) => {
                ...
                match self.map.get(&GenericArg::from(ty)).map(|arg| arg.kind()) {
    ...
    fn fold_const(&mut self, ct: ty::Const<'tcx>) -> ty::Const<'tcx> {
        ...
        match ct.kind() {
            ty::ConstKind::Param(..) => {
                // Look it up in the generic parameters list.
                match self.map.get(&GenericArg::from(ct)).map(|arg| arg.kind()) {
                    // Found it in the generic parameters list, replace with the parameter from the
                    // opaque type.
