...
use crate::rustc_middle::ty::{self, ReBound, ReStatic, ReVar, Region, RegionExt, RegionVid, Ty, TyCtxt};
...
use crate::rustc_infer::infer::snapshot::undo_log::{InferCtxtUndoLogs, Snapshot};
use crate::rustc_infer::infer::unify_key::{RegionVariableValue, RegionVidKey};
    ...
    pub fn vars_since_snapshot<'a>(
        ...
        (
            ...
            (range.start.index()..range.end.index())
                ...
                .map(|index| self.storage.var_infos[index].origin)
                .collect(),
        )
    }
