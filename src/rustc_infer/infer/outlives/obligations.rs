...
use crate::rustc_infer::infer::outlives::verify::VerifyBoundCx;
...
impl<'tcx> InferCtxt<'tcx> {
    ...
    pub fn destructure_solver_region_constraints_for_regionck(
        ...
        let assumptions = crate::rustc_type_ir::region_constraint::Assumptions::new(
    ...
    pub fn destructure_solver_region_constraints(
        &self,
        assumptions: crate::rustc_type_ir::region_constraint::Assumptions<TyCtxt<'tcx>>,
        ...
        debug!(?constraint);
        let constraint =
            crate::rustc_type_ir::region_constraint::destructure_type_outlives_constraints_in_root(
                self,
                constraint,
                &assumptions,
            );
        debug!(?constraint);
        let constraint = crate::rustc_type_ir::region_constraint::evaluate_solver_constraint(&constraint);
        debug!(?constraint);

        let mut constraints = vec![constraint];
        while let Some(c) = constraints.pop() {
            use crate::rustc_type_ir::region_constraint::RegionConstraint::*;
    ...
    D: TypeOutlivesDelegate<'tcx>,
{
    ...
    fn alias_ty_must_outlive(
        ...
        let trait_bounds: Vec<_> =
            crate::rustc_type_ir::outlives::declared_bounds_from_definition(self.tcx, alias_ty).collect();
