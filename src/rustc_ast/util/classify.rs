...
use crate::rustc_ast::ast::ExprKind::*;
use crate::rustc_ast::ast::{self, MatchKind};
...
pub fn leading_labeled_expr(mut expr: &ast::Expr) -> bool {
    loop {
        match &expr.kind {
            Block(_, label) | Loop(_, label, _) | While(_, _, label) => {
                return label.is_some();
            }
            ForLoop(for_loop) => {
                return for_loop.label.is_some();
            }
...
///
/// ```ignore (illustrative)
/// ::core::ops::FnOnce(&str) -> fn() -> *const c_void
///                             ^^^^^^^^^^^^^^^^^^^^^
/// ```
