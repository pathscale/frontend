...
impl Features {
    pub fn dump_feature_usage_metrics(
        ...
        // Nanoseconds since the epoch, from whole seconds: the field is a `u128` of nanoseconds
        // and every reader of this file compares two of them for ordering, which second-level
        // resolution answers. The previous version could panic when the system clock was set
        // before 1970, on a metrics dump that has no business killing a compilation.
        let now = || (eko::time::epoch_secs() as u128) * 1_000_000_000;
        ...
        // Serialise to memory and write once. `BufWriter` existed to keep `serde_json`'s many
        // small writes off the descriptor; one `write` does the same thing for a file this size.
        let json = serde_json::to_vec(&feature_usage)?;
        eko::file::write(&metrics_path, &json)?;
