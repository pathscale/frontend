...
use crate::rustc_hir::{ForeignItemId, find_attr};
use crate::rustc_middle::query::{LocalCrate, Providers};
use crate::rustc_middle::ty::{Instance, List, TyCtxt};
use crate::rustc_span::def_id::{DefId, LOCAL_CRATE};
use crate::rustc_span::{Symbol, sym};

use crate::rustc_passes::diagnostics::DuplicateCanonicalSymbolInCrate;
...
fn all_canonical_symbols(tcx: TyCtxt<'_>, (): ()) -> CanonicalSymbols {
    ...
    for cnum in tcx.crates(()).iter().copied().chain(core::iter::once(LOCAL_CRATE)) {
