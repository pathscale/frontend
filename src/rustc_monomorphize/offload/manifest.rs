...
use crate::rustc_hir::def_id::{DefId, DefIndex, LOCAL_CRATE, StableCrateId};
use crate::bug;
...
impl<'a, 'tcx> OffloadManifestEncoder<'a, 'tcx> {
    pub(crate) fn new(path: &'a eko::path::Path, tcx: TyCtxt<'tcx>) -> eko::file::Result<Self> {
    ...
    pub(crate) fn finish(mut self) -> eko::file::Result<()> {
...
impl<'a, 'tcx> SpanEncoder for OffloadManifestEncoder<'a, 'tcx> {
    ...
    fn encode_symbol(&mut self, sym: crate::rustc_span::Symbol) {
    ...
    fn encode_expn_id(&mut self, _expn_id: crate::rustc_span::ExpnId) {
        self.emit_u32(0);
    }
    ...
    fn encode_crate_num(&mut self, crate_num: crate::rustc_span::def_id::CrateNum) {
        self.tcx.stable_crate_id(crate_num).encode(self);
    }

    fn encode_def_index(&mut self, def_index: crate::rustc_span::def_id::DefIndex) {
        def_index.as_u32().encode(self);
    }

    fn encode_def_id(&mut self, def_id: crate::rustc_span::def_id::DefId) {
...
impl<'a, 'tcx> TyEncoder<'tcx> for OffloadManifestEncoder<'a, 'tcx> {
    ...
    fn encode_alloc_id(&mut self, _alloc_id: &crate::rustc_middle::mir::interpret::AllocId) {
        // AllocIds are not expected in the manifest.
    }
}

const UNRESOLVED_DEF_ID: DefId = DefId {
    krate: crate::rustc_span::def_id::CrateNum::MAX,
    index: crate::rustc_span::def_id::DefIndex::from_u32(0),
};
...
impl<'a, 'tcx> BlobDecoder for OffloadManifestDecoder<'a, 'tcx> {
    fn decode_symbol(&mut self) -> crate::rustc_span::Symbol {
        let s: String = Decodable::decode(self);
        crate::rustc_span::Symbol::intern(&s)
    }
    ...
    fn decode_def_index(&mut self) -> crate::rustc_span::def_id::DefIndex {
        let v = self.read_u32();
        crate::rustc_span::def_id::DefIndex::from_u32(v)
    }
}
...
impl<'a, 'tcx> SpanDecoder for OffloadManifestDecoder<'a, 'tcx> {
    ...
    fn decode_expn_id(&mut self) -> crate::rustc_span::ExpnId {
        let _ = self.read_u32();
        crate::rustc_span::ExpnId::root()
    }
    ...
    fn decode_crate_num(&mut self) -> crate::rustc_span::def_id::CrateNum {
        let stable_id: StableCrateId = Decodable::decode(self);
        self.tcx.stable_crate_id_to_crate_num(stable_id)
    }

    fn decode_def_id(&mut self) -> crate::rustc_span::def_id::DefId {
    ...
    fn decode_attr_id(&mut self) -> crate::rustc_ast::AttrId {
...
impl<'a, 'tcx> TyDecoder<'tcx> for OffloadManifestDecoder<'a, 'tcx> {
        ...
        F: FnOnce(&mut Self) -> R,
    {
        ...
        let old_decoder = core::mem::replace(&mut self.decoder, new_decoder);
        let result = f(self);
...
impl<'a, 'tcx> crate::rustc_middle::ty::InternerDecoder for OffloadManifestDecoder<'a, 'tcx> {
...
pub fn write_host_metadata_offload_manifest<'tcx>(tcx: TyCtxt<'tcx>) {
    let Some(path) = tcx.sess.opts.unstable_opts.offload.iter().find_map(|o| {
        if let crate::rustc_session::config::Offload::HostMetadata(p) = o { Some(p) } else { None }
    }) else {
    ...
    if let Err(e) = write_manifest(eko::path::Path::new(path), tcx, &instances) {
        tcx.dcx().emit_fatal(crate::rustc_monomorphize::diagnostics::OffloadManifestWriteError {
            path: path.clone(),
            err: e.to_string(),
        });
    }
...
pub(crate) fn read_manifest<'tcx>(
    path: &eko::path::Path,
    tcx: TyCtxt<'tcx>,
) -> eko::file::Result<Vec<ty::Instance<'tcx>>> {
    let data = eko::file::read(path)?;
    let mut decoder = OffloadManifestDecoder::new(&data, tcx)
        .map_err(|_| eko::file::Error::invalid_data("invalid manifest"))?;
