...
//!
//! The collector and "mentioned items" gathering (which lives in `crate::rustc_mir_transform::mentioned_items`)
...
use crate::rustc_middle::mir::{self, Body, Location, MentionedItem, traversal};
use crate::rustc_middle::mono::{CollectionMode, InstantiationMode, MonoItem, NormalizationErrorInMono};
...
use crate::rustc_session::config::{DebugInfo, EntryFnType, Offload};
use crate::rustc_span::{DUMMY_SP, Span, Spanned, Symbol, dummy_spanned, respan};
use crate::rustc_structures::Limit;
use tracing::{debug, instrument, trace};

use crate::rustc_monomorphize::diagnostics::{
        ...
        match terminator.kind {
            ...
            | mir::TerminatorKind::TailCall { ref func, ref args, .. } => {
                ...
                if let ty::FnDef(def_id, _) = *callee_ty.kind()
                    && self.tcx.is_intrinsic(def_id, crate::rustc_span::sym::offload)
    ...
    match (source_ty.kind(), target_ty.kind()) {
        ...
        (&ty::Adt(source_adt_def, source_args), &ty::Adt(target_adt_def, target_args)) => {
            ...
            let CustomCoerceUnsized::Struct(coerce_index) =
                match crate::rustc_monomorphize::custom_coerce_unsize_info(tcx, source_ty, target_ty) {
                    Ok(ccu) => ccu,
                    Err(e) => {
...
fn create_fn_mono_item<'tcx>(
    ...
    {
        crate::rustc_monomorphize::util::dump_closure_profile(tcx, instance);
    }
...
fn collect_roots(tcx: TyCtxt<'_>, mode: MonoItemCollectionStrategy) -> Vec<MonoItem<'_>> {
    ...
    if let Some(manifest_path) = tcx.sess.opts.unstable_opts.offload.iter().find_map(|o| {
        if let crate::rustc_session::config::Offload::Device(p) = o
            && !p.is_empty()
        {
            Some(p)
        } else {
        ...
        match crate::rustc_monomorphize::offload::manifest::read_manifest(eko::path::Path::new(manifest_path), tcx) {
            Ok(instances) => {
            ...
            Err(e) => {
                tcx.dcx().emit_err(crate::rustc_monomorphize::diagnostics::OffloadManifestReadError {
    ...
    let is_host_metadata = tcx
        ...
        .offload
        .iter()
        .any(|o| matches!(o, crate::rustc_session::config::Offload::HostMetadata(_)));
    if is_host_metadata {
        let crate_items = tcx.hir_crate_items(());
        for id in crate_items.free_items() {
...
pub(crate) fn collect_crate_mono_items<'tcx>(
    ...
    {
        crate::rustc_monomorphize::offload::check_offload_kernels_instantiated(tcx, &mono_items);
    }
