...
use crate::rustc_ast_pretty::pprust::state::MacHeader;
use crate::rustc_ast_pretty::pprust::{Comments, PrintState};
...
use crate::rustc_span::{DUMMY_SP, FileName, Ident, Span, Spanned, Symbol, kw, sym};
    ...
    fn print_attribute_as_style(&mut self, attr: &hir::Attribute, style: ast::AttrStyle) {
        match &attr {
            ...
            hir::Attribute::Parsed(AttributeKind::DocComment { kind, comment, .. }) => {
                self.word(crate::rustc_ast_pretty::pprust::state::doc_comment_to_string(
                    *kind, style, *comment,
                ));
                self.hardbreak()
            }
...
impl<'a> State<'a> {
    ...
    fn print_item(&mut self, item: &hir::Item<'_>) {
        ...
        match item.kind {
            ...
            crate::rustc_hir::ItemKind::TestBinderConstraints { .. } => {
    ...
    fn print_restriction<S: Into<alloc::borrow::Cow<'static, str>>>(
        &mut self,
