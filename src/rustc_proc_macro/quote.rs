...
// `#![no_std]`: `Vec`, `vec!` and `to_string` came from the std prelude and name no path.
use alloc::string::ToString;
use alloc::vec;
use alloc::vec::Vec;

use crate::rustc_proc_macro::{
...
#[doc(hidden)]
pub mod ext {
    // An inline module does not inherit the enclosing module`s `use` items, so the prelude
    // names this one needs are imported again here.
    ...
    use crate::rustc_proc_macro::ToTokens;
...
pub fn quote(stream: TokenStream) -> TokenStream {
    if stream.is_empty() {
        return minimal_quote!(crate::rustc_proc_macro::TokenStream::new());
    }
    let proc_macro_crate = minimal_quote!(crate);
    let mut after_dollar = false;

    let mut tokens = crate::rustc_proc_macro::TokenStream::new();
        ...
        if after_dollar {
            ...
            match tree {
                TokenTree::Group(tt) => {
                    ...
                    minimal_quote!(
                        use crate::rustc_proc_macro::ext::*;
                        (@ if sep_opt.is_some() {
                            minimal_quote!(let mut _i = 0usize;)
                        } else {
                            minimal_quote!(();)
                        })
                        let has_iter = crate::rustc_proc_macro::ThereIsNoIteratorInRepetition;
                    )
                    ...
                    minimal_quote!(let _: crate::rustc_proc_macro::HasIterator = has_iter;)
                        .to_tokens(&mut rep_expanded);
                    ...
                    for meta_var in &meta_vars {
                        minimal_quote!(
                            let (@ meta_var) = match (@ meta_var).next() {
                                Some(_x) => crate::rustc_proc_macro::RepInterp(_x),
                                None => break,
                            };
                        )
                    ...
                    minimal_quote!(
                        (@ if let Some(sep) = sep_opt {
                            minimal_quote!(
                                if _i > 0 {
                                    (@ minimal_quote!(crate::rustc_proc_macro::ToTokens::to_tokens(&crate::rustc_proc_macro::TokenTree::Punct(crate::rustc_proc_macro::Punct::new(
                                        (@ TokenTree::from(Literal::character(sep.as_char()))),
                                        (@ minimal_quote!(crate::rustc_proc_macro::Spacing::Alone)),
                                    )), &mut ts);))
                                }
                                _i += 1;
                            )
                ...
                TokenTree::Ident(_) => {
                    minimal_quote!(crate::rustc_proc_macro::ToTokens::to_tokens(&(@ tree), &mut ts);)
        ...
        match tree {
            TokenTree::Punct(tt) => {
                minimal_quote!(crate::rustc_proc_macro::ToTokens::to_tokens(&crate::rustc_proc_macro::TokenTree::Punct(crate::rustc_proc_macro::Punct::new(
                    ...
                    (@ match tt.spacing() {
                        Spacing::Alone => minimal_quote!(crate::rustc_proc_macro::Spacing::Alone),
                        Spacing::Joint => minimal_quote!(crate::rustc_proc_macro::Spacing::Joint),
                    }),
                )), &mut ts);)
            }
            TokenTree::Group(tt) => {
                minimal_quote!(crate::rustc_proc_macro::ToTokens::to_tokens(&crate::rustc_proc_macro::TokenTree::Group(crate::rustc_proc_macro::Group::new(
                    (@ match tt.delimiter() {
                        Delimiter::Parenthesis => minimal_quote!(crate::rustc_proc_macro::Delimiter::Parenthesis),
                        Delimiter::Brace => minimal_quote!(crate::rustc_proc_macro::Delimiter::Brace),
                        Delimiter::Bracket => minimal_quote!(crate::rustc_proc_macro::Delimiter::Bracket),
                        Delimiter::None => minimal_quote!(crate::rustc_proc_macro::Delimiter::None),
                    }),
                    (@ quote(tt.stream())),
                )), &mut ts);)
            }
            TokenTree::Ident(tt) => {
                ...
                let (literal, ctor) = if let Some(stripped) = literal.strip_prefix("r#") {
                    (stripped, minimal_quote!(crate::rustc_proc_macro::Ident::new_raw))
                } else {
                    (literal.as_str(), minimal_quote!(crate::rustc_proc_macro::Ident::new))
                };
                minimal_quote!(crate::rustc_proc_macro::ToTokens::to_tokens(&crate::rustc_proc_macro::TokenTree::Ident((@ ctor)(
                    (@ TokenTree::from(Literal::string(literal))),
                    (@ quote_span(proc_macro_crate.clone(), tt.span())),
                )), &mut ts);)
            }
            TokenTree::Literal(tt) => {
                minimal_quote!(crate::rustc_proc_macro::ToTokens::to_tokens(&crate::rustc_proc_macro::TokenTree::Literal({
                    let mut iter = (@ TokenTree::from(Literal::string(&tt.to_string())))
                        .parse::<crate::rustc_proc_macro::TokenStream>()
                        .unwrap()
                        .into_iter();
                    if let (Some(crate::rustc_proc_macro::TokenTree::Literal(mut lit)), None) =
                        (iter.next(), iter.next())
                    {
    ...
    minimal_quote! {
        {
            let mut ts = crate::rustc_proc_macro::TokenStream::new();
            (@ tokens)
            ts
        }
    }
