...
// this crate's source lives at `frontend/library/proc_macro/`, while its manifest is
// `frontend/compiler/rustc_proc_macro/Cargo.toml` pointing here with `[lib] path`, so a scan of
// `frontend/compiler/*/src` walks straight past it.
...
// `core::error::Error` is the same trait `std::error::Error` is, re-exported rather than
// redefined, and has been stable since 1.81.
use core::{error, fmt};
// A path here is the bytes the kernel takes. See the module header in `ekostd`.
...
pub mod tracked {
    // An inline module does not inherit the enclosing module's `use` items.
    ...
    /// Why reading an environment variable failed.
    ///
    /// This is `std::env::VarError`, with the one difference the platform forces: a value that
    /// is not UTF-8 is still a real value, and it arrives here as the bytes it is rather than
    /// as an `OsString`, because on this platform an environment value *is* bytes.
    #[derive(Debug, Clone, PartialEq, Eq)]
    pub enum VarError {
        /// The variable is not set.
        NotPresent,
        /// The variable is set, and its value is not UTF-8. Carries the bytes.
        NotUnicode(Vec<u8>),
    }
    ...
    pub fn env_var<K: AsRef<str>>(key: K) -> Result<String, VarError> {
        ...
        let value = match BridgeMethods::injected_env_var(key) {
            ...
            // `env::var` here collapses "unset" and "not UTF-8" into `None`, so the raw bytes
            // are read instead and the two are told apart, which is what `VarError` promises.
