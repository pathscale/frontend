...
use crate::rustc_error_messages::{DiagArgMap, DiagArgName, DiagArgValue, IntoDiagArg};
use crate::rustc_lint_defs::{Applicability, LintExpectationId};
use rustc_macros::{Decodable, Encodable};
use crate::rustc_span::{DUMMY_SP, Span, Spanned, Symbol};
use tracing::debug;

// `ExplicitBug` is no longer imported: with `panic = "abort"` the payload cannot be caught, so
// `BugAbort` names it only through `stringify!`, which does not count as a use.
use crate::rustc_errors::{
    CodeSuggestion, DiagCtxtHandle, DiagMessage, ErrCode, ErrorGuaranteed, Level, MultiSpan,
    StashKey, Style, Substitution, SubstitutionPart, SuggestionStyle, Suggestions,
};
...
impl EmissionGuarantee for BugAbort {
    ...
    fn emit_producing_guarantee(diag: Diag<'_, Self>) -> Self::EmitResult {
        diag.emit_producing_nothing();
        // Was `panic::panic_any(ExplicitBug)`. With `panic = "abort"` there is no unwinding, so
        // nothing can catch the payload and the marker type cannot be recovered; the containment
        // that let a caller downcast to `ExplicitBug` was dropped with the panic runtime.
        panic!("{}", stringify!(ExplicitBug));
    }
}
...
impl DiagStyledString {
    ...
    pub fn compact(&mut self) {
        let segments = core::mem::take(&mut self.0);
...
pub struct DiagInner {
    ...
    /// The full text of an over-long type this diagnostic shortened, if it shortened one.
    ///
    /// Was `Option<PathBuf>` naming a file the compiler had written the type into. See
    /// `crate::rustc_error_messages::LongTyPath`.
    pub long_ty_path: Option<String>,
...
impl DiagInner {
    ...
    pub fn emitted_at_sub_diag(&self) -> Subdiag {
        ...
        Subdiag {
            level: crate::rustc_errors::Level::Note,
...
impl<'a, G: EmissionGuarantee> Diag<'a, G> {
    ...
    pub fn multipart_suggestion_with_style(
        ...
        let mut seen = crate::rustc_errors::FxHashSet::default();
    ...
    fn take_diag(&mut self) -> DiagInner {
        // **The full type goes in the diagnostic, not into a file.**
        //
        // This said "the full name for the type has been written to '<path>'" - a file the
        // compiler had just opened, read back and appended to. For a caller in another process that names
        // a path on the server's disk, and the client that asked the question is somewhere else
        // and cannot open it. So the note carries the type itself.
        if let Some(full) = self.long_ty_path.take() {
            self.note(format!("the full type name is `{full}`"));
        }
    ...
    pub fn long_ty_path(&mut self) -> &mut Option<String> {
        &mut self.long_ty_path
    }

    pub fn with_long_ty_path(mut self, long_ty_path: Option<String>) -> Self {
