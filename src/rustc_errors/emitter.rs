...
// The `Buffy`/`Destination` cluster is gone with the snippet emitter it fed. It existed to give
// `anstream::AutoStream` a buffered stderr so a coloured diagnostic reached the terminal in one
// write; there is no terminal here and no snippet renderer, so nothing constructed a
// `Destination` any more. `anstream`, `anstyle`, `annotate-snippets` and `termize` left the
// manifest with it.
use core::iter;
use eko::path::Path;
...
impl ColorConfig {
    // `to_color_choice` went with `anstream`. Colour is a property of a terminal and this
    // compiler writes to a caller; `ColorConfig` survives only as a `-Z` value that is parsed
...
impl Style {
    // `Style::anstyle` mapped each diagnostic style to an ANSI colour. It had exactly one
    // caller, the snippet renderer, and there is no terminal to colour for.
}
