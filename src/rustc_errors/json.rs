...
// The sink was `std::io::Write` over bytes. There is no byte-writer trait here, and JSON is
// UTF-8 by definition, so the sink is `core::fmt::Write` and each record is serialised to a
// `String` and handed over in one call instead of streamed through `serde_json::to_writer`.
use core::fmt::Write;

use eko::file::{Error, Result};
...
use crate::rustc_span::{FileName, RealFileName, Span};
use serde::Serialize;

use crate::rustc_errors::plain_emitter::PlainEmitter;
use crate::rustc_errors::diagnostic::IsLint;
use crate::rustc_errors::emitter::{
    ColorConfig, Emitter, HumanReadableErrorType, OutputTheme, TimingEvent,
    should_show_source_code,
};
use crate::rustc_errors::formatting::{format_diag_message, format_diag_messages};
use crate::rustc_errors::timings::{TimingRecord, TimingSection};
use crate::rustc_errors::{CodeSuggestion, MultiSpan, SpanLabel, Subdiag, Suggestions, TerminalUrl};
...
pub struct JsonEmitter {
    ...
    // `Write` here is `core::fmt::Write`, not `std::io::Write`.
    #[setters(skip)]
...
impl JsonEmitter {
    ...
    fn emit(&mut self, val: EmitTyped<'_>) -> Result<()> {
        // Serialise to a `String` and write it once. `serde_json::to_writer` wanted a
        // `std::io::Write`; `to_string` is the `alloc`-only spelling and produces the same bytes.
        let mut s = if self.pretty {
            serde_json::to_string_pretty(&val).map_err(|e| Error::other(format!("{e}")))?
        } else {
            serde_json::to_string(&val).map_err(|e| Error::other(format!("{e}")))?
        };
        s.push('\n');
        // `core::fmt::Error` carries nothing, so the sink's failure becomes a bare message.
        // Nothing buffers behind this sink that a separate `flush` would reach.
        self.dst.write_str(&s).map_err(|_| Error::other("failed to write JSON diagnostic"))
    }
}
...
impl Emitter for JsonEmitter {
    fn emit_diagnostic(&mut self, diag: crate::rustc_errors::DiagInner) {
    ...
    fn emit_future_breakage_report(&mut self, diags: Vec<crate::rustc_errors::DiagInner>) {
        let data: Vec<FutureBreakageItem<'_>> = diags
            ...
            .map(|mut diag| {
                ...
                if matches!(diag.level, crate::rustc_errors::Level::Allow | crate::rustc_errors::Level::Expect) {
                    diag.level = crate::rustc_errors::Level::Warning;
                }
                FutureBreakageItem {
    ...
    fn emit_unused_externs(&mut self, lint_level: crate::rustc_lint_defs::Level, unused_externs: &[&str]) {
...
impl Diagnostic {
    ...
    fn from_errors_diagnostic(diag: crate::rustc_errors::DiagInner, je: &JsonEmitter) -> Diagnostic {
        ...
        // The local `BufWriter(Arc<Mutex<Vec<u8>>>)` that used to stand here fed the second
        // render into the `rendered` field. That render is gone (see the comment on `buf`
        // below), nothing constructed the type any more, and its `impl Write` was the last
        // thing in this file relying on `Vec<u8>` being a `std::io::Write`.
        ...
        let code = if let Some(code) = diag.code {
            Some(DiagnosticCode {
                code: code.to_string(),
                // The long-form explanations were deleted with `--explain`; the code remains.
                explanation: None,
            })
        ...
        // The `rendered` field carried the terminal drawing - carets, colour, box characters -
        // produced by rendering the diagnostic a second time into a buffer. There is no terminal
        // here and no snippet renderer any more, so it carries the message instead. A client
        // that wants to draw the snippet has the spans and the source.
        let buf = formatted_message.to_string();
