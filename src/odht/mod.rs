// Vendored from odht 0.3.1, converted to `no_std`. See `vendor/odht/VENDORED.md`.
//
// The conversion is a rename: every `std::` path here was `core`- or `alloc`-reachable except
// `impl std::error::Error for Error` and the `Box<dyn Error>` it was returned as, both replaced
// below. Nothing about the on-disk layout is touched, which is the point - this table is written
// into every `.rmeta` by the upstream rustc that built the sysroot, and it has to be read back
// byte for byte.
...
pub use crate::odht::fxhash::FxHashFn;
pub use crate::odht::unhash::UnHashFn;

use crate::odht::raw_table::{ByteArray, RawIter, RawTable, RawTableMut};
...
impl<C: Config> HashTableOwned<C> {
    ...
    pub fn from_raw_bytes(data: &[u8]) -> Result<HashTableOwned<C>, Box<dyn core::error::Error>> {
...
impl<C: Config> core::fmt::Debug for HashTableOwned<C> {
...
impl<C: Config, D: Borrow<[u8]>> HashTable<C, D> {
    ...
    pub fn from_raw_bytes(data: D) -> Result<HashTable<C, D>, Box<dyn core::error::Error>> {
        let allocation = memory_layout::Allocation::from_raw_bytes(data)?;
...
impl<C: Config, D: Borrow<[u8]> + BorrowMut<[u8]>> HashTable<C, D> {
    pub fn init_in_place(
        ...
        max_load_factor_percent: u8,
    ) -> Result<HashTable<C, D>, Box<dyn core::error::Error>> {
...
mod tests {
    ...
    fn assert_basically_equal(x: usize, y: usize) {
        let larger_number = core::cmp::max(x, y) as f64;
