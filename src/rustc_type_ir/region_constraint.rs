...
use crate::rustc_type_ir::data_structures::IndexMap;
use crate::rustc_type_ir::fold::TypeSuperFoldable;
use crate::rustc_type_ir::inherent::*;
use crate::rustc_type_ir::relate::{Relate, RelateResult, TypeRelation, VarianceDiagInfo};
    ...
    I::BoundVarKinds: StableHash,
{
    ...
    fn stable_hash<CTX: StableHashCtxt>(&self, hcx: &mut CTX, hasher: &mut StableHasher) {
        ...
        core::mem::discriminant(self).stable_hash(hcx, hasher);
        match self {
            Ambiguity(_) => (),
...
impl<I: Interner, S: Clone + core::fmt::Debug> TypeFoldable<I> for RegionConstraint<I, S> {
...
impl<I: Interner, S: core::fmt::Debug> TypeVisitable<I> for RegionConstraint<I, S> {
...
impl<I: Interner, S: Clone + core::fmt::Debug> Default for RegionConstraint<I, S> {
    fn default() -> Self {
        Self::new_true()
    }
}

impl<I: Interner, S: Clone + core::fmt::Debug> RegionConstraint<I, S> {
                    ...
                    for choice in or1 {
                        choices.extend(
                            permutations(rest_ors)
                                .into_iter()
                                .map(|and| core::iter::once(choice.clone()).chain(and).collect()),
                        );
                    }
                    choices
                }
...
fn compute_new_region_constraints<Infcx: InferCtxtLike<Interner = I>, I: Interner>(
    ...
    // `IndexSet::new()` only exists with `indexmap`'s `std` feature, which supplies the
    // `RandomState` default. Naming the hasher is what every other `IndexSet` here already does.
    let mut regions = IndexSet::with_hasher(rustc_hash::FxBuildHasher);
...
#[instrument(level = "debug", ret)]
pub fn evaluate_solver_constraint<I: Interner, S: Clone + core::fmt::Debug>(
    ...
    for HigherRankedAliasMatcher<'a, Infcx, I>
{
    ...
    fn relate_ty_args(
        ...
        crate::rustc_type_ir::relate::relate_args_invariantly(self, a_args, b_args)?;
        Ok(a_ty)
    }
    ...
    fn tys(&mut self, a: I::Ty, b: I::Ty) -> RelateResult<I, I::Ty> {
        crate::rustc_type_ir::relate::structurally_relate_tys(self, a, b)
    }
    ...
    fn consts(&mut self, a: I::Const, b: I::Const) -> RelateResult<I, I::Const> {
        crate::rustc_type_ir::relate::structurally_relate_consts(self, a, b)
    }
