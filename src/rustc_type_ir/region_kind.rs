...
use rustc_macros::GenericTypeVisitable;

use self::RegionKind::*;
use crate::rustc_type_ir::{BoundRegion, BoundVarIndexKind, Interner, LateParamRegion, PlaceholderRegion};
...
/// to index into internal NLL data structures. See `crate::rustc_const_eval::borrow_check`
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ReBound(binder_id, bound_region) => {
                write!(f, "'")?;
                crate::rustc_type_ir::debug_bound_var(f, *binder_id, bound_region)
            }
