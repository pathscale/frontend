...
use crate::rustc_session::diagnostics::report_lit_error;
use crate::rustc_span::{ByteSymbol, DUMMY_SP, DesugaringKind, Ident, Span, Spanned, Symbol, respan, sym};
use thin_vec::{ThinVec, thin_vec};
use visit::{Visitor, walk_expr};

mod closure;

use crate::rustc_ast_lowering::diagnostics::{
...
impl<'hir> LoweringContext<'_, 'hir> {
    ...
    /// Desugar `try { <stmts>; <expr> }` into `{ <stmts>; ::core::ops::Try::from_output(<expr>) }`,
    /// `try { <stmts>; }` into `{ <stmts>; ::core::ops::Try::from_output(()) }`
    ...
    fn lower_expr_try_block(&mut self, body: &Block, opt_ty: Option<&Ty>) -> hir::ExprKind<'hir> {
        ...
        let whole_block = self.with_try_block_scope(new_scope, |this| {
            ...
            let (try_span, tail_expr) = if let Some(expr) = block.expr.take() {
                (
                    this.mark_span_with_reason(
                        ...
                        expr.span,
                        Some(Arc::clone(&this.allow.try_trait)),
                    ),
                    expr,
                )
            } else {
            ...
            // `::core::ops::Try::from_output($tail_expr)`
        ...
        let (inputs, params, task_context): (&[_], &[_], _) = match desugaring_kind {
            hir::CoroutineDesugaring::Async | hir::CoroutineDesugaring::AsyncGen => {
                ...
                let unstable_span = self.mark_span_with_reason(
                    ...
                    Some(Arc::clone(&self.allow.gen_future)),
                );
    ...
    ///             <::core::pin::Pin>::new_unchecked(&mut __awaitee),
    ...
    fn make_lowered_await(
        ...
        let features = match await_kind {
            FutureKind::Future if is_async_gen => Some(Arc::clone(&self.allow.async_gen)),
            FutureKind::Future => None,
            FutureKind::AsyncIterator => Some(Arc::clone(&self.allow.for_await)),
        };
    ...
    /// Desugar `<start>..=<end>` into `core::ops::RangeInclusive::new(<start>, <end>)`.
    ...
    fn lower_expr_yield(&mut self, span: Span, opt_expr: Option<&Expr>) -> hir::ExprKind<'hir> {
        ...
        if is_async_gen {
            ...
            let desugar_span = self.mark_span_with_reason(
                DesugaringKind::Async,
                span,
                Some(Arc::clone(&self.allow.async_gen)),
            );
    ...
    fn lower_expr_try(&mut self, span: Span, sub_expr: &Expr) -> hir::ExprKind<'hir> {
        let unstable_span = self.mark_span_with_reason(
            ...
            span,
            Some(Arc::clone(&self.allow.try_trait)),
        );
    ...
    pub(super) fn expr_struct(
        ...
        self.expr(span, hir::ExprKind::Struct(path, fields, crate::rustc_hir::StructTailExpr::None))
    }
