...
use crate::rustc_errors::StashKey;
...
impl<'hir> LoweringContext<'_, 'hir> {
    ...
    pub(crate) fn lower_qpath(
        ...
        {
            Some(Arc::clone(&self.allow.async_fn_traits))
        } else {
            None
        };
        ...
        // E.g., `core::mem::size_of` or `<I as Iterator>::Item`.
        ...
        // e.g., for `alloc::vec::Vec::<T>::IntoIter::Item::clone`:
        // * base path is `alloc::vec::Vec<T>`
        // * "extensions" are `IntoIter`, `Item` and `clone`
        // * type nodes are:
        //   1. `alloc::vec::Vec<T>` (created above)
        //   2. `<alloc::vec::Vec<T>>::IntoIter`
        //   3. `<<alloc::vec::Vec<T>>::IntoIter>::Item`
        // * final path is `<<<alloc::vec::Vec<T>>::IntoIter>::Item>::clone`
    ...
    fn maybe_insert_elided_lifetimes_in_path(
        ...
        // See crate::rustc_resolve::late::lifetimes::LifetimeContext::add_missing_lifetime_specifiers_label
