...
impl<'hir> LoweringContext<'_, 'hir> {
    ...
    pub(super) fn lower_contract(
        &mut self,
        body: impl FnOnce(&mut Self) -> crate::rustc_hir::Expr<'hir>,
        contract: &crate::rustc_ast::FnContract,
    ) -> crate::rustc_hir::Expr<'hir> {
    ...
    fn lower_decls(&mut self, contract: &crate::rustc_ast::FnContract) -> &'hir [crate::rustc_hir::Stmt<'hir>] {
    ...
    fn lower_precond(&mut self, req: &Box<crate::rustc_ast::Expr>) -> crate::rustc_hir::Stmt<'hir> {
    ...
    fn lower_postcond_checker(
        &mut self,
        ens: &Box<crate::rustc_ast::Expr>,
    ) -> &'hir crate::rustc_hir::Expr<'hir> {
    ...
    fn lower_contract_check_just_precond(
        &mut self,
        contract_decls: &'hir [crate::rustc_hir::Stmt<'hir>],
        precond: crate::rustc_hir::Stmt<'hir>,
    ) -> crate::rustc_hir::Stmt<'hir> {
        let stmts = self
            .arena
        ...
        let precond_check = crate::rustc_hir::ExprKind::If(
    ...
    fn lower_contract_check_with_postcond(
        &mut self,
        contract_decls: &'hir [crate::rustc_hir::Stmt<'hir>],
        precond: Option<crate::rustc_hir::Stmt<'hir>>,
        postcond_checker: &'hir crate::rustc_hir::Expr<'hir>,
    ) -> &'hir crate::rustc_hir::Expr<'hir> {
        let stmts = self
            .arena
            .alloc_from_iter(contract_decls.into_iter().map(|d| *d).chain(precond.into_iter()));
        ...
        let postcond_checker = self.arena.alloc(self.expr_enum_variant_lang_item(
            ...
            crate::rustc_hir::attrs::lang_items::LangItem::OptionSome,
        ...
        let none_expr = self.arena.alloc(self.expr_enum_variant_lang_item(
            postcond_checker.span,
            crate::rustc_hir::attrs::lang_items::LangItem::OptionNone,
            Default::default(),
        ));
        let else_block = self.block_expr(none_expr);
        let else_block = self.arena.alloc(self.expr_block(else_block));

        let contract_check = crate::rustc_hir::ExprKind::If(
    ...
    fn wrap_body_with_contract_check(
        &mut self,
        body: impl FnOnce(&mut Self) -> crate::rustc_hir::Expr<'hir>,
        contract_check: &'hir crate::rustc_hir::Expr<'hir>,
        postcond_span: crate::rustc_span::Span,
    ) -> &'hir crate::rustc_hir::Block<'hir> {
        let check_ident: crate::rustc_span::Ident =
            crate::rustc_span::Ident::from_str_and_span("__ensures_checker", postcond_span);
        let (check_hir_id, postcond_decl) = {
            // Set up the postcondition `let` statement.
            let (checker_pat, check_hir_id) = self.pat_ident_binding_mode_mut(
                postcond_span,
                check_ident,
                crate::rustc_hir::BindingMode::NONE,
            );
            (
                check_hir_id,
                self.stmt_let_pat(
                    ...
                    crate::rustc_hir::LocalSource::Contract,
                ),
            )
        };
    ...
    pub(super) fn checked_return(
        &mut self,
        opt_expr: Option<&'hir crate::rustc_hir::Expr<'hir>>,
    ) -> crate::rustc_hir::ExprKind<'hir> {
        let checked_ret =
        ...
        crate::rustc_hir::ExprKind::Ret(checked_ret)
    }

    /// Wraps an expression with a call to the ensures check before it gets returned.
    pub(super) fn inject_ensures_check(
        &mut self,
        expr: &'hir crate::rustc_hir::Expr<'hir>,
        span: crate::rustc_span::Span,
        cond_ident: crate::rustc_span::Ident,
        cond_hir_id: crate::rustc_hir::HirId,
    ) -> &'hir crate::rustc_hir::Expr<'hir> {
        // {
        //     let ret = { body };
        //
        ...
        //     }
        // }
        let ret_ident: crate::rustc_span::Ident = crate::rustc_span::Ident::from_str_and_span("__ret", span);

        // Set up the return `let` statement.
        let (ret_pat, ret_hir_id) =
            self.pat_ident_binding_mode_mut(span, ret_ident, crate::rustc_hir::BindingMode::NONE);
        ...
        let attrs: crate::rustc_ast::AttrVec = thin_vec![self.unreachable_code_attr(span)];
        self.lower_attrs(contract_check.hir_id, &attrs, span, crate::rustc_hir::Target::Expression);
