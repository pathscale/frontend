...
use crate::rustc_middle::thir::{self, ExprId, LocalVarId, Param, ParamId, PatKind, Thir};
...
use crate::rustc_mir_build::builder::expr::as_place::PlaceBuilder;
...
impl<'a, 'tcx> Builder<'a, 'tcx> {
    ...
    fn dump_for_debugging(&self) {
        ...
        // Rendered, not printed. `-Zdump-mir` names a dump; where it goes is the driver's
        // decision, and a pass writing to stdout is a side effect its signature never declared.
        let writer = pretty::MirWriter::new(self.tcx);
        let mut text = String::new();
        if writer.write_mir_fn(&body, &mut text).is_ok() {
            self.tcx.sess.mir_dumps.lock().push(("built.mir".to_string(), text));
        }
    }
    ...
    fn finish(self) -> Body<'tcx> {
        ...
        for (index, block) in body.basic_blocks.iter().enumerate() {
            if block.terminator.is_none() {
                let mut text = String::new();
                if writer.write_mir_fn(&body, &mut text).is_ok() {
                    self.tcx.sess.mir_dumps.lock().push(("no-terminator.mir".to_string(), text));
                }
