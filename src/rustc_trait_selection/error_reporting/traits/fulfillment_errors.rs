...
use crate::rustc_trait_selection::diagnostics::{
...
use crate::rustc_trait_selection::error_reporting::infer::TyCategory;
use crate::rustc_trait_selection::error_reporting::traits::report_dyn_incompatibility;
use crate::rustc_trait_selection::infer::{self, InferCtxt, InferCtxtExt as _};
                        ...
                        *err.long_ty_path() = long_ty_file;
                        ...
                        if let Some(s) = label {
                            ...
                            if !matches!(leaf_trait_predicate.skip_binder().self_ty().kind(), ty::Param(_))
                                ...
                                // `core::marker::Sized` is not implemented for `T`" as we will point
...
impl<'a, 'tcx> TypeErrCtxt<'a, 'tcx> {
    ...
    fn maybe_detailed_projection_msg(
        ...
        long_ty_path: &mut Option<String>,
    ...
    fn select_transmute_obligation_for_reporting(
        ...
        let Some(assume) = crate::rustc_transmute::Assume::from_const(self.tcx, assume) else {
            return (obligation.clone(), trait_predicate);
        };

        let is_normalized_yes = matches!(
            crate::rustc_transmute::TransmuteTypeEnv::new(self.tcx).is_transmutable(
                trait_ref.args.type_at(1),
                trait_ref.args.type_at(0),
                assume,
            ),
            crate::rustc_transmute::Answer::Yes,
        );
    ...
    fn get_safe_transmute_error_and_reason(
        ...
        self.probe(|_| {
            ...
            let Some(assume) = crate::rustc_transmute::Assume::from_const(self.infcx.tcx, assume) else {
                self.dcx().span_delayed_bug(
                    span,
                    "Unable to construct crate::rustc_transmute::Assume where it was previously possible",
                );
            ...
            match crate::rustc_transmute::TransmuteTypeEnv::new(self.infcx.tcx)
                .is_transmutable(src, dst, assume)
            {
                Answer::No(reason) => {
                    let safe_transmute_explanation = match reason {
                        crate::rustc_transmute::Reason::SrcIsNotYetSupported => {
                            format!("analyzing the transmutability of `{src}` is not yet supported")
                        }
                        crate::rustc_transmute::Reason::DstIsNotYetSupported => {
                            format!("analyzing the transmutability of `{dst}` is not yet supported")
                        }
                        crate::rustc_transmute::Reason::DstIsBitIncompatible => {
                            format!(
                                "at least one value of `{src}` isn't a bit-valid value of `{dst}`"
                            )
                        }
                        crate::rustc_transmute::Reason::DstUninhabited => {
                            format!("`{dst}` is uninhabited")
                        }
                        crate::rustc_transmute::Reason::DstMayHaveSafetyInvariants => {
                            format!("`{dst}` may carry safety invariants")
                        }
                        crate::rustc_transmute::Reason::DstIsTooBig => {
                            format!("the size of `{src}` is smaller than the size of `{dst}`")
                        }
                        crate::rustc_transmute::Reason::DstRefIsTooBig {
                            src,
                            src_size,
                            dst,
                            dst_size,
                        ...
                        crate::rustc_transmute::Reason::SrcSizeOverflow => {
                            format!(
                                "values of the type `{src}` are too big for the target architecture"
                            )
                        }
                        crate::rustc_transmute::Reason::DstSizeOverflow => {
                            format!(
                                "values of the type `{dst}` are too big for the target architecture"
                            )
                        }
                        crate::rustc_transmute::Reason::DstHasStricterAlignment {
                            src_min_align,
                            dst_min_align,
                        } => {
                            format!(
                        ...
                        crate::rustc_transmute::Reason::DstIsMoreUnique => {
                            format!(
                                "`{src}` is a shared reference, but `{dst}` is a unique reference"
                            )
                        }
                        // Already reported by rustc
                        crate::rustc_transmute::Reason::TypeError => {
                            return GetSafeTransmuteErrorAndReason::Silent;
                        }
                        crate::rustc_transmute::Reason::SrcLayoutUnknown => {
                            format!("`{src}` has an unknown layout")
                        }
                        crate::rustc_transmute::Reason::DstLayoutUnknown => {
                ...
                Answer::Yes => span_bug!(
                    span,
                    "Inconsistent crate::rustc_transmute::is_transmutable(...) result, got Yes",
                ),
