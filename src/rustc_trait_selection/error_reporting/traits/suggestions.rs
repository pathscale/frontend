...
use crate::rustc_hir::attrs::lang_items::{self, LangItem};
...
use crate::rustc_middle::ty::adjustment::{Adjust, DerefAdjustKind};
...
impl<'a, 'tcx> TypeErrCtxt<'a, 'tcx> {
    ...
    pub(super) fn suggest_fn_call(
            ...
            && obligation.cause.span.contains(arg_span)
        {
            ...
            let removal_span = if let Ok(snippet) =
                sm.span_to_snippet(arg_span.with_hi(arg_span.hi() + crate::rustc_span::BytePos(1)))
                && snippet.ends_with(' ')
            {
                // There's a space after `||`, include it in the removal
                arg_span.with_hi(arg_span.hi() + crate::rustc_span::BytePos(1))
            } else {
                arg_span
            };
            err.span_suggestion_verbose(
    ...
    ///    |
    ///    = help: within `impl core::future::Future`, the trait `core::marker::Send` is not
    ///            implemented for `Foo`
    ...
    pub fn maybe_note_obligation_cause_for_async_await<G: EmissionGuarantee>(
        ...
        // - `BuiltinDerivedObligation` with `impl core::future::Future` (B)
        // - `BuiltinDerivedObligation` with a coroutine witness (A)
        // - `BuiltinDerivedObligation` with a coroutine (A)
        // - `BuiltinDerivedObligation` with `impl core::future::Future` (A)
    ...
    pub(super) fn suggest_add_result_as_return_type(
        ...
        {
            let mut sugg_spans =
                vec![(ret_span, " -> Result<(), Box<dyn core::error::Error>>".to_string())];
            let body = self.tcx.hir_body(body_id);
    ...
    let mut sugg = if future_output_ty.span.is_empty() {
        vec![
            ...
            (
                ...
                format!(" -> impl core::future::Future<Output = ()>{add_bounds}"),
            ),
        ]
    } else {
        vec![
