...
fn test_input(g: LabelledGraph) -> io::Result<String> {
    // `render` writes through `core::fmt::Write` now, so the sink is a `String` directly and
    // the `Vec<u8>` plus `Read::read_to_string` round trip that turned bytes back into text
    // is gone. Nothing in here can fail any more; the `Result` stays so the call sites do.
...
fn left_aligned_text() {
    ...
    // `render` writes through `core::fmt::Write`, so the sink is a `String` rather than a
    // `Vec<u8>` read back out.
