...
use crate::rustc_middle::ty::{self, RegionExt, TyCtxt};
...
use crate::rustc_borrowck::borrow_set::BorrowSet;
use crate::rustc_borrowck::consumers::RustcFacts;
use crate::rustc_borrowck::diagnostics::RegionErrors;
use crate::rustc_borrowck::handle_placeholders::compute_sccs_applying_placeholder_outlives_constraints;
...
use crate::rustc_borrowck::type_check::MirTypeckRegionConstraints;
use crate::rustc_borrowck::type_check::free_region_relations::UniversalRegionRelations;
use crate::rustc_borrowck::universal_regions::UniversalRegions;
use crate::rustc_borrowck::{
...
pub(crate) fn compute_regions<'tcx>(
    ...
    let polonius_output = polonius_facts.as_ref().and_then(|polonius_facts| {
        // `-Znll-facts` wrote the polonius input relations into a directory for an external
        // solver to read back. Facts are computed and consumed in this process.
...
pub(super) fn dump_nll_mir<'tcx>(
    ...
    let extra_data = &|pass_where, out: &mut dyn core::fmt::Write| {
    ...
    // The region constraint graph and its SCC graph were also written here, as .dot files,
    // straight to disk from inside borrow checking. Drawing is not this compiler's job and a
    // check that writes files is a check with an undeclared side effect. The MIR dump above
    // carries the NLL annotations, which is the part that says anything.
}
...
fn for_each_region_constraint<'tcx>(
    ...
    with_msg: &mut dyn FnMut(String) -> core::fmt::Result,
) -> core::fmt::Result {
