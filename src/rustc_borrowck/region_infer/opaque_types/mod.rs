...
use crate::rustc_borrowck::consumers::RegionInferenceContext;
use crate::rustc_borrowck::session_diagnostics::LifetimeMismatchOpaqueParam;
use crate::rustc_borrowck::type_check::canonical::fully_perform_op_raw;
...
use crate::rustc_borrowck::universal_regions::{RegionClassification, UniversalRegions};
use crate::rustc_borrowck::{BorrowckInferCtxt, CollectRegionConstraintsResult};
        ...
        let expected_ty = ty::fold_regions(
            ...
            |re, _dbi| match re.kind() {
                ty::ReErased => infcx.next_nll_region_var(
                    ...
                    || crate::rustc_borrowck::RegionCtxt::Existential(None),
                ),
                _ => re,
            },
        );
...
pub(crate) fn handle_unconstrained_hidden_type_errors<'tcx>(
    ...
    let mut unconstrained_hidden_type_errors = core::mem::take(unconstrained_hidden_type_errors);
    unconstrained_hidden_type_errors
