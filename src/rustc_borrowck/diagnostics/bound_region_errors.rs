...
use crate::rustc_trait_selection::traits::query::type_op::ascribe_user_type::type_op_ascribe_user_type_with_span;
use crate::rustc_trait_selection::traits::query::type_op::prove_predicate::type_op_prove_predicate_with_cause;
    ...
    T: Copy + fmt::Display + TypeFoldable<TyCtxt<'tcx>> + 'tcx,
{
    ...
    fn nice_error<'diag>(
        ...
        // `crate::rustc_traits::type_op::type_op_normalize` query to allow the span we need in the
...
impl<'tcx> TypeOpInfo<'tcx> for crate::rustc_borrowck::type_check::InstantiateOpaqueType<'tcx> {
    fn fallback_error(&self, tcx: TyCtxt<'tcx>, span: Span) -> Diag<'tcx> {
        // FIXME: This error message isn't great, but it doesn't show up in the existing UI tests,
        // and is only the fallback when the nice error fails. Consider improving this some more.
        tcx.dcx().create_err(HigherRankedLifetimeError { cause: None, span })
