...
use crate::rustc_errors::{Applicability, Diag};
...
use crate::rustc_span::{BytePos, ExpnKind, MacroKind, Span, sym};
use crate::rustc_trait_selection::error_reporting::traits::FindExprBySpan;
use crate::rustc_trait_selection::infer::InferCtxtExt;
use tracing::debug;

use crate::rustc_borrowck::MirBorrowckCtxt;
...
impl<'diag, 'tcx> MirBorrowckCtxt<'_, 'diag, 'tcx> {
    ...
    pub(in crate::rustc_borrowck::diagnostics) fn suggest_clone_of_captured_var_in_move_closure(
        &self,
        err: &mut Diag<'_>,
        upvar_name: &str,
        ...
        match error {
            ...
            GroupedMoveError::OtherIllegalMove { ref original_path, use_spans, .. } => {
                ...
                use_spans.args_subdiag(err, |args_span| {
                    ...
                    crate::rustc_borrowck::session_diagnostics::CaptureArgLabel::MoveOutPlace {
                        place: place_desc.clone(),
                        args_span,
                    }
                });
                if !pointed_at_span && use_span != span {
                    err.subdiagnostic(crate::rustc_borrowck::session_diagnostics::CaptureArgLabel::MoveOutPlace {
                        place: place_desc,
                        args_span: span,
                    });
                }
    ...
    fn add_move_error_details(
        ...
        for (j, local) in binds_to.iter().enumerate() {
            ...
            if binds_to.len() == 1 {
                ...
                err.subdiagnostic(crate::rustc_borrowck::session_diagnostics::TypeNoCopy::LabelMovedHere {
                    ty: bind_to.ty,
