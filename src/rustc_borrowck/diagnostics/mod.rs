...
use crate::rustc_errors::{Applicability, Diag, DiagMessage, EmissionGuarantee, MultiSpan, listify, msg};
...
use crate::rustc_middle::ty::print::{Print, with_no_trimmed_paths};
...
use crate::rustc_span::{DUMMY_SP, Span, Spanned, Symbol, sym};
...
use crate::rustc_borrowck::LocalMutationIsAllowed;
use crate::rustc_borrowck::constraints::OutlivesConstraint;
use crate::rustc_borrowck::nll::ConstraintDescription;
use crate::rustc_borrowck::session_diagnostics::{
...
impl<'diag, 'tcx> BorrowckDiagnosticsBuffer<'diag, 'tcx> {
    ...
    pub(crate) fn emit_errors(&mut self) {
        ...
        for (_, (_, diag)) in core::mem::take(&mut self.buffered_move_errors) {
            // We have already set tainted for this error, so just buffer it.
            self.buffer_error(diag);
        }
        for (_, (mut diag, count)) in core::mem::take(&mut self.buffered_mut_errors) {
            if count > 10 {
...
impl UseSpans<'_> {
    ...
    pub(super) fn var_path_only_subdiag(
        self,
        err: &mut Diag<'_>,
        action: crate::rustc_borrowck::InitializationRequiringAction,
    ) {
        use CaptureVarPathUseCause::*;

        use crate::rustc_borrowck::InitializationRequiringAction::*;
