...
use crate::rustc_metadata::locator::CrateFlavor;
...
pub(crate) struct RegistrationFailedEarlier {
    pub span: Span,
    pub crate_name: Symbol,
    /// The failure that aborted the first registration, when it was recorded.
    pub why: Option<String>,
}

impl<G: EmissionGuarantee> Diagnostic<'_, G> for RegistrationFailedEarlier {
    #[track_caller]
    fn into_diag(self, dcx: DiagCtxtHandle<'_>, level: Level) -> Diag<'_, G> {
        let mut diag = Diag::new(
            dcx,
            level,
            msg!("crate `{$crate_name}` was found but an earlier attempt to load it failed{$why}"),
        );
        diag.arg("crate_name", self.crate_name);
        // The reason goes in the *message* and not only in a note, because the surface that reads
        // these most often - a structured diagnostic table - prints the message and drops
        // the notes, and a reason it cannot print is a reason nobody reads.
        diag.arg("why", match &self.why {
            Some(why) => format!(": {why}"),
            None => String::new(),
        });
        diag.code(E0463);
        diag.span(self.span);
        diag.note(msg!(
            "its file was read and its identity registered, so the file is not the problem"
        ));
        if self.why.is_none() {
            diag.note(msg!(
                "the real failure is whatever stopped that first load, usually one of its own dependencies"
            ));
        }
        diag.span_label(self.span, msg!("this crate loaded partially"));
        diag
    }
}
...
pub(crate) struct CannotFindCrate {
    ...
    pub crate_name: Symbol,
    pub add_info: String,
    /// Every place the locator looked, one note each. Empty when the search did not run.
...
impl<G: EmissionGuarantee> Diagnostic<'_, G> for CannotFindCrate {
    ...
    fn into_diag(self, dcx: DiagCtxtHandle<'_>, level: Level) -> Diag<'_, G> {
        ...
        if self.searched.is_empty() {
            diag.note(msg!("the crate search did not examine any file"));
        } else {
            diag.note(msg!("looked in:"));
