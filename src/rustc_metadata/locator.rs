...
// The `io::` self-import was unused. `Write` is the `write!` trait, which is `core::fmt::Write`
// now; `eko::file::File` and `Stream` both implement it.
use core::fmt::Write;
use eko::file::{Error as IoError, ErrorKind as IoErrorKind, Result as IoResult};
use core::ops::Deref;
use eko::path::{Path, PathBuf};
use core::{cmp, fmt};

use crate::rustc_crate_store::CrateSource;
...
use crate::rustc_errors::{DiagArgValue, IntoDiagArg};
...
pub(crate) struct CrateLocator<'a> {
    ...
    /// Whether `exact_paths` is the only place worth looking.
    ///
    /// For a crate named directly by `--extern` it is: the user said where the crate lives and a
...
impl IntoDiagArg for CrateFlavor {
    fn into_diag_arg(self, _: &mut crate::rustc_errors::LongTyPath) -> crate::rustc_errors::DiagArgValue {
        match self {
...
impl<'a> CrateLocator<'a> {
    pub(crate) fn new(
        ...
        CrateLocator {
            ...
            // one. That reasoning holds for *preferring* the file and not for *ignoring* it: a
            // build that hands the compiler every crate by path and no `-L` directory then has
    ...
    /// Where the search will look, for a diagnostic that has to say why nothing was found.
    fn searched(&self) -> Vec<String> {
        let mut searched: Vec<String> = self
            .exact_paths
            .iter()
            .map(|p| {
                let path = p.original().display();
                if self.exact_paths_are_final {
                    format!("`--extern` path: {path}")
                } else {
                    format!("`--extern` path (candidate for a transitive dependency): {path}")
                }
            })
            .collect();
        for sp in self.filesearch.search_paths(self.path_kind) {
            searched.push(format!("search path: {}", sp.dir.display()));
        }
        searched
    }
    ...
    pub(crate) fn maybe_load_library_crate(
        ...
        if !self.exact_paths.is_empty() {
            ...
            // A transitive dependency. The `--extern` file is a candidate: `crate_matches`
            // still has to accept it, and for a transitive dependency that includes the SVH,
            // so a `--extern` naming a different build of the same crate is rejected here and
            // the search paths get their turn below. Its rejections are kept either way, so a
            // near miss says "found possibly newer version" rather than "can't find crate".
            match self.find_commandline_library(crate_rejections) {
                Ok(Some(library)) => return Ok(Some(library)),
                Ok(None) => {}
                // A `--extern` path that is missing or is not a file is a hard error when the
                // user named the crate directly. Here they named a *different* crate and this
                // file only happens to share its name, so it is one dead candidate and the
                // search paths still get their turn. The path is listed by `searched()` if
                // nothing else works out, so dropping the error here loses nothing.
                Err(CrateError::ExternLocationNotExist(..))
                | Err(CrateError::ExternLocationNotFile(..)) => {}
                Err(e) => return Err(e),
            }
        }
...
fn get_rmeta_metadata_section<'a, 'p>(filename: &'p Path) -> Result<OwnedSlice, MetadataError<'a>> {
    ...
    let file = eko::file::File::open(filename).map_err(|_| {
...
pub fn list_file_metadata(
    ...
    match get_metadata_section(target, flavor, path, metadata_loader, cfg_version, None) {
        ...
        Ok(metadata) => metadata
            .list_crate_metadata(out, ls_kinds)
            .map_err(|_| IoError::new(IoErrorKind::Other, "could not write the metadata listing")),
...
pub(crate) struct CombinedLocatorError {
    ...
    dll_suffix: String,
    crate_rejections: CrateRejections,
    /// Every place the search actually looked, so an error with no rejection to report can say
    /// where it went instead of only that it failed.
    searched: Vec<String>,
}
...
pub(crate) enum CrateError {
    ...
    /// it. Reporting this as "can't find crate" sends the reader looking for a missing file that
    /// is sitting right there, which is what it did until 2026-09-09. The failure that matters
    /// is the earlier one, and it may have been swallowed by `maybe_process_path_extern`.
    RegistrationFailedEarlier(Symbol, Option<String>),
}

impl CrateError {
    /// One line naming this failure, for stashing against a crate whose registration it aborted.
    ///
    /// It is deliberately not the full diagnostic: the full one belongs to the crate that
    /// actually failed, and this is quoted inside a different crate's error as the reason its own
    /// registration never finished.
    pub(crate) fn brief(&self) -> String {
        match self {
            CrateError::NonAsciiName(name) => format!("crate name `{name}` is not ASCII"),
            CrateError::ExternLocationNotExist(name, path) => {
                format!("the `--extern` path for `{name}` does not exist: {}", path.display())
            }
            CrateError::ExternLocationNotFile(name, path) => {
                format!("the `--extern` path for `{name}` is not a file: {}", path.display())
            }
            CrateError::MultipleCandidates(name, flavor, _) => {
                format!("multiple {flavor} candidates for `{name}`")
            }
            CrateError::FullMetadataNotFound(name, flavor) => {
                format!("only a metadata stub was found for `{name}` as a {flavor}")
            }
            CrateError::SymbolConflictsCurrent(name) => {
                format!("`{name}` collides with the crate being compiled")
            }
            ...
            CrateError::RegistrationFailedEarlier(name, why) => match why {
                ...
                None => format!("`{name}` failed to load earlier"),
            },
        }
    }
}
        ...
        match self {
            ...
            CrateError::LocatorCombined(locator) => {
                ...
                if !locator.crate_rejections.via_hash.is_empty() {
                    ...
                    // Nothing was rejected, so the search never got as far as reading a file.
                    // Without the list below the message is "can't find crate" and no more, and
                    // a caller who passed the crate by `--extern` has no way to tell an absent
                    // rule from one the search declined to consult.
            ...
            CrateError::RegistrationFailedEarlier(crate_name, why) => {
                dcx.emit_err(diagnostics::RegistrationFailedEarlier { span, crate_name, why });
            }
        }
    }
}
