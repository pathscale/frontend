...
use crate::rustc_ast::expand::allocator::{ALLOC_ERROR_HANDLER, AllocatorKind, global_fn_name};
use crate::rustc_ast::{self as ast, *};
use crate::rustc_crate_store::{CrateDepKind, CrateSource, ExternCrate, ExternCrateSource};
...
use crate::rustc_lint_defs::builtin::UNUSED_CRATE_DEPENDENCIES;
use crate::bug;
...
pub struct CStore {
    ...
    /// Why a crate's registration failed, the first time it failed.
    ///
    /// Registration reserves the crate's `StableCrateId` before it walks the dependencies, so a
    /// failure part of the way through leaves an entry that is claimed and empty. Every later
    /// load of that crate then stops at the claim rather than at the real problem, and the
    /// speculative `maybe_process_path_extern` has usually thrown the real problem away by then.
    /// This is the copy the second load reports.
    first_load_failure: UnordMap<Symbol, String>,
}
...
impl CStore {
    ...
    fn intern_stable_crate_id<'tcx>(
        ...
        let num = tcx.create_crate_num(root.stable_crate_id()).map_err(|existing| {
            ...
            } else if let Some(crate_name1) = self.metas[existing].as_ref().map(|data| data.name())
            {
                ...
                CrateError::RegistrationFailedEarlier(
                    root.name(),
                    self.first_load_failure.get(&root.name()).cloned(),
                )
            }
        })?;
    ...
    pub fn report_unused_deps(&self, tcx: TyCtxt<'_>) {
        ...
        let level =
            tcx.lint_level_spec_at_node(UNUSED_CRATE_DEPENDENCIES, crate::rustc_hir::CRATE_HIR_ID).level();
    ...
    pub fn new(metadata_loader: Box<MetadataLoaderDyn>) -> CStore {
        CStore {
            ...
            first_load_failure: Default::default(),
        }
    }
    ...
    fn maybe_resolve_crate<'b, 'tcx>(
        ...
        match result {
            ...
            (LoadResult::Loaded(library), host_library) => {
                ...
                let registered = self
                    .register_crate(tcx, host_library, origin, library, dep_kind, name, private_dep);
                // Registration reserves the stable crate id before it resolves the dependencies,
                // so a failure here leaves a claimed and empty entry that every later load of
                // `name` trips over. Keep the reason: the later load has no other way to reach it,
                // and `maybe_process_path_extern` may already have dropped this `Err` on the floor.
                if let Err(e) = &registered {
                    self.first_load_failure.entry(name).or_insert_with(|| e.brief());
                }
                registered
            }
            _ => panic!(),
    ...
    fn report_future_incompatible_deps(&self, tcx: TyCtxt<'_>, krate: &ast::Crate) {
        ...
        if name.as_str() == "wasm_bindgen" {
            // `eko::env::var` already answers `Option`, so the `.ok()` that turned
            // `std`'s `Result` into one is gone. An unset variable still reads as `None`.
            let major =
                env::var("CARGO_PKG_VERSION_MAJOR").and_then(|major| u64::from_str(&major).ok());
            let minor =
                env::var("CARGO_PKG_VERSION_MINOR").and_then(|minor| u64::from_str(&minor).ok());
            let patch =
                env::var("CARGO_PKG_VERSION_PATCH").and_then(|patch| u64::from_str(&patch).ok());
