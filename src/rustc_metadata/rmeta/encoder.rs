...
use eko::file::File;
// `std::io::{Read, Seek, Write}` was dropped: `eko::file::File` carries `read`,
// `seek`, `rewind`, `stream_position` and `write_all` as inherent methods, so no trait is
// in scope for them. The one `write!` target here is a `String`, and the block at
// `meta_stats` already imports `core::fmt::Write` for it.
...
use crate::find_attr;
use crate::rustc_hir_pretty::id_to_string;
...
use crate::rustc_session::config::{OptLevel, TargetModifier};
use crate::rustc_span::def_id::CRATE_MOD_ID;
...
impl<'a, 'tcx> EncodeContext<'a, 'tcx> {
    ...
    fn encode_def_path_table(&mut self) {
        ...
        if self.is_proc_macro {
            for def_id in core::iter::once(CRATE_DEF_ID)
    ...
    fn encode_crate_root(&mut self) -> LazyValue<CrateRoot> {
        ...
        if tcx.sess.opts.unstable_opts.meta_stats {
            ...
            {
                let file = self.opaque.file_mut();
                file.rewind().unwrap();
                // A 64 KiB buffer rather than `BufReader::bytes()`, which produced one `Result`
                // per byte over a whole metadata file.
                ...
                loop {
                    ...
                    zero_bytes += buf[..n].iter().filter(|&&b| b == 0).count();
                }
                file.seek(pos_before_rewind).unwrap();
            }
...
impl EncodedMetadata {
    ...
    ) -> eko::file::Result<Self> {
        let file = eko::file::File::open(&path)?;
...
pub fn encode_metadata(tcx: TyCtxt<'_>, path: &Path, ref_path: Option<&Path>) {
        ...
        && tcx.dep_graph.try_mark_green(tcx, &dep_node).is_some()
    {
        ...
        match crate::rustc_fs_util::link_or_copy(&source_file_in_incr_dir, path) {
            Ok(_) => {}
