...
fn stable_hash_derive_with_mode(
    ...
    s.bound_impl(
        quote!(::frontend::rustc_data_structures::stable_hash::StableHash),
        quote! {
            #[inline]
            fn stable_hash<__Hcx: ::frontend::rustc_data_structures::stable_hash::StableHashCtxt>(
                &self,
                __hcx: &mut __Hcx,
                __hasher: &mut ::frontend::rustc_data_structures::stable_hash::StableHasher
            ) {
                #discriminant
                match *self { #body }
            }
...
fn stable_hash_discriminant(s: &mut synstructure::Structure<'_>) -> proc_macro2::TokenStream {
    match s.ast().data {
        syn::Data::Enum(_) => quote! {
            ::core::mem::discriminant(self).stable_hash(__hcx, __hasher);
        },
