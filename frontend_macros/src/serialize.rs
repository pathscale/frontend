...
pub(super) fn type_decodable_derive(
    ...
    s.add_impl_generic(parse_quote! { #decoder_ty: ::frontend::rustc_middle::ty::codec::TyDecoder<'tcx> });
...
pub(super) fn blob_decodable_derive(
    ...
    let decoder_ty = quote! { __D };
    s.add_impl_generic(parse_quote! { #decoder_ty: ::frontend::rustc_span::BlobDecoder });
...
pub(super) fn decodable_derive(mut s: synstructure::Structure<'_>) -> proc_macro2::TokenStream {
    let decoder_ty = quote! { __D };
    s.add_impl_generic(parse_quote! { #decoder_ty: ::frontend::rustc_span::SpanDecoder });
    s.add_bounds(synstructure::AddBounds::Generics);
...
pub(super) fn decodable_nocontext_derive(
    ...
    let decoder_ty = quote! { __D };
    s.add_impl_generic(parse_quote! { #decoder_ty: ::frontend::rustc_serialize::Decoder });
    s.add_bounds(synstructure::AddBounds::Fields);
...
fn decodable_body(
    ...
    let decode_body = match s.variants() {
        ...
        variants => {
            ...
            let tag = if variants.len() < u8::MAX as usize {
                quote! {
                    ::frontend::rustc_serialize::Decoder::read_u8(__decoder) as usize
                }
            } else {
                quote! {
                    ::frontend::rustc_serialize::Decoder::read_usize(__decoder)
                }
            };
            quote! {
                match #tag {
    ...
    s.bound_impl(
        quote!(::frontend::rustc_serialize::Decodable<#decoder_ty>),
        quote! {
...
fn decode_field(field: &syn::Field) -> proc_macro2::TokenStream {
    ...
    let decode_inner_method = if let syn::Type::Reference(_) = field.ty {
        quote! { ::frontend::rustc_middle::ty::codec::RefDecodable::decode }
    } else {
        quote! { ::frontend::rustc_serialize::Decodable::decode }
    };
...
pub(super) fn type_encodable_derive(
    ...
    s.add_impl_generic(parse_quote! { #encoder_ty: ::frontend::rustc_middle::ty::codec::TyEncoder<'tcx> });
    s.add_bounds(synstructure::AddBounds::Fields);
...
pub(super) fn encodable_derive(mut s: synstructure::Structure<'_>) -> proc_macro2::TokenStream {
    let encoder_ty = quote! { __E };
    s.add_impl_generic(parse_quote! { #encoder_ty: ::frontend::rustc_span::SpanEncoder });
    s.add_bounds(synstructure::AddBounds::Generics);
...
pub(super) fn encodable_nocontext_derive(
    ...
    let encoder_ty = quote! { __E };
    s.add_impl_generic(parse_quote! { #encoder_ty: ::frontend::rustc_serialize::Encoder });
    s.add_bounds(synstructure::AddBounds::Fields);
...
fn encodable_body(
        ...
        [vi] => {
            ...
            let body = vi
                ...
                .map(|binding| {
                    ...
                    let result = quote! {
                        ::frontend::rustc_serialize::Encodable::<#encoder_ty>::encode(
                            #bind_ident,
                            __encoder,
                        );
                    };
            ...
            let disc = {
                ...
                if variant_idx < u8::MAX as usize {
                    quote! {
                        ...
                        ::frontend::rustc_serialize::Encoder::emit_u8(__encoder, disc as u8);
                    }
                } else {
                    quote! {
                        let disc = match *self {
                        ...
                        ::frontend::rustc_serialize::Encoder::emit_usize(__encoder, disc);
                    }
                }
            };
    ...
    s.bound_impl(
        quote!(::frontend::rustc_serialize::Encodable<#encoder_ty>),
        quote! {
            fn encode(
                &self,
