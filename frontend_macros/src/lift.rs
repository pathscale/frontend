...
pub(super) fn lift_derive(mut s: synstructure::Structure<'_>) -> proc_macro2::TokenStream {
    ...
    s.bound_impl(
        quote!(::frontend::rustc_middle::ty::Lift<::frontend::rustc_middle::ty::TyCtxt<'__lifted>>),
        quote! {
            type Lifted = #lifted;

            fn lift_to_interner(self, __tcx: ::frontend::rustc_middle::ty::TyCtxt<'__lifted>) -> #lifted {
                match self { #body }
            }
        },
    )
