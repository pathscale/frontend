...
impl<'a> DiagnosticDerive<'a> {
    ...
    pub(crate) fn into_tokens(self) -> TokenStream {
        ...
        let implementation = each_variant(&mut structure, |mut builder, variant| {
            ...
            let init = quote! {
                let mut diag = frontend::rustc_errors::Diag::new(
                    dcx,
                    level,
                    #message
                );
        ...
        structure.gen_impl(quote! {
            gen impl<'_sess, G> frontend::rustc_errors::Diagnostic<'_sess, G> for @Self
                where G: frontend::rustc_errors::EmissionGuarantee
            {
                #[track_caller]
                fn into_diag(
                    self,
                    dcx: frontend::rustc_errors::DiagCtxtHandle<'_sess>,
                    level: frontend::rustc_errors::Level
                ) -> frontend::rustc_errors::Diag<'_sess, G> {
                    #implementation
                }
            }
        })
