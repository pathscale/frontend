...
impl SubdiagnosticDerive {
    ...
    pub(crate) fn into_tokens(self, mut structure: Structure<'_>) -> TokenStream {
        ...
        let ret = structure.gen_impl(quote! {
            gen impl frontend::rustc_errors::Subdiagnostic for @Self {
                fn add_to_diag<__G>(
                    self,
                    #diag: &mut frontend::rustc_errors::Diag<'_, __G>,
                ) where
                    __G: frontend::rustc_errors::EmissionGuarantee,
                {
                    #implementation
                }
            }
...
impl<'parent, 'a> SubdiagnosticDeriveVariantBuilder<'parent, 'a> {
    ...
    fn generate_field_arg(&mut self, binding_info: &BindingInfo<'_>) -> TokenStream {
        ...
        quote! {
            sub_args.insert(
                ...
                frontend::rustc_errors::IntoDiagArg::into_diag_arg(#field_binding, &mut #diag.long_ty_path)
            );
        }
    }
    ...
    pub(crate) fn into_tokens(&mut self) -> Result<TokenStream, DiagnosticDeriveError> {
        ...
        let plain_args = quote! {
            let mut sub_args = frontend::rustc_errors::DiagArgMap::default();
            #plain_args
        };
        ...
        for (kind, messages) in kind_messages {
            ...
            calls.extend(quote! { let #message = frontend::rustc_errors::format_diag_message(&#message_stream, &sub_args); });
            ...
            let call = match kind {
                SubdiagnosticKind::Suggestion {
                    ...
                    let applicability = applicability
                        ...
                        .unwrap_or_else(|| quote! { frontend::rustc_errors::Applicability::Unspecified });
