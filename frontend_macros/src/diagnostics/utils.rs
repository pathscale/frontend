...
/// `Applicability` of a suggestion - mirrors `frontend::rustc_errors::Applicability` - and used to represent
    ...
    fn to_tokens(&self, tokens: &mut TokenStream) {
        tokens.extend(match self {
            Applicability::MachineApplicable => {
                quote! { frontend::rustc_errors::Applicability::MachineApplicable }
            }
            Applicability::MaybeIncorrect => {
                quote! { frontend::rustc_errors::Applicability::MaybeIncorrect }
            }
            Applicability::HasPlaceholders => {
                quote! { frontend::rustc_errors::Applicability::HasPlaceholders }
            }
            Applicability::Unspecified => {
                quote! { frontend::rustc_errors::Applicability::Unspecified }
            }
        });
    }
}
...
pub(super) fn build_field_mapping(variant: &VariantInfo<'_>) -> FieldMap {
    let mut fields_map = FieldMap::default();
    ...
    pub(crate) fn to_suggestion_style(&self) -> TokenStream {
        match self {
            SuggestionKind::Normal => {
                quote! { frontend::rustc_errors::SuggestionStyle::ShowCode }
            }
            SuggestionKind::Short => {
                quote! { frontend::rustc_errors::SuggestionStyle::HideCodeInline }
            }
            SuggestionKind::Hidden => {
                quote! { frontend::rustc_errors::SuggestionStyle::HideCodeAlways }
            }
            SuggestionKind::Verbose => {
                quote! { frontend::rustc_errors::SuggestionStyle::ShowAlways }
            }
            SuggestionKind::ToolOnly => {
                quote! { frontend::rustc_errors::SuggestionStyle::CompletelyHidden }
            }
        }
    }
        ...
        match kind {
            SubdiagnosticKind::Suggestion {
                ...
                *code_init = if let Some(init) = code.value() {
                    ...
                    quote! { let #code_field = core::iter::empty(); }
                };
            }
            SubdiagnosticKind::MultipartSuggestion {
                suggestion_kind: ref mut kind_field, ..
