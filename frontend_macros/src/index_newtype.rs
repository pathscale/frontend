...
impl Parse for Newtype {
    fn parse(input: ParseStream<'_>) -> Result<Self> {
        ...
        let encodable_impls = if encodable {
            quote! {
                #gate_rustc_only
                impl<D: ::frontend::rustc_serialize::Decoder> ::frontend::rustc_serialize::Decodable<D> for #name {
                    fn decode(d: &mut D) -> Self {
                        Self::from_u32(d.read_u32())
                    }
                }
                #gate_rustc_only
                impl<E: ::frontend::rustc_serialize::Encoder> ::frontend::rustc_serialize::Encodable<E> for #name {
        ...
        let stable_hash_impl = if stable_hash {
            quote! {
                #gate_rustc_only
                impl ::frontend::rustc_data_structures::stable_hash::StableHash for #name {
                    fn stable_hash<
                        __Hcx: ::frontend::rustc_data_structures::stable_hash::StableHashCtxt
                    >(
                        &self,
                        hcx: &mut __Hcx,
                        hasher: &mut ::frontend::rustc_data_structures::stable_hash::StableHasher
                    ) {
        ...
        Ok(Self(quote! {
            ...
            impl ::frontend::rustc_index::Idx for #name {
                #[inline]
